...
/**
 * 终审意见登记自动风险探测场景参数初始化类
 * @author jschen
 * @date:2012-12-24
 * 
 */
public class InitApproveScenario extends Bizlet{
	...
	public BizObject getApprove(String sApproveNo) throws Exception{
		...
		BizObject jboApprove = manager.createQuery("select * from o where SerialNo = '" + sApproveNo + "'").getSingleResult(false);
	    return jboApprove;
	}
	...
	public BizObject getCustomer(String sCustomerID) throws Exception{
		...
		BizObject jboCustomer = manager.createQuery("CustomerID = '" + sCustomerID + "'").getSingleResult(false);
	    return jboCustomer;
	}
	...
	public BizObject[] getGuaranty(String sApproveNo) throws Exception{
		...
		List jboList = q.getResultList(false); //一个BizObject对象可理解为数据中的一行
