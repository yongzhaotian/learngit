...
public class SubJudgeBusinessType extends Bizlet {
	...
	public Object run(Transaction Sqlca) throws Exception {
        ...
        sSql = "select BusinessType,((case when BusinessSum is null then 0 else BusinessSum end)-(case when BailSum is null then 0 else BailSum end))*getERate(BusinessCurrency,'01','') "+
        ...
        rs = Sqlca.getASResultSet(new SqlObject(sSql).setParameter("LimitationSetID", sLimitationSetID).setParameter("LimObjectNo", sBusinessType));
        if(rs.next())
        { 
