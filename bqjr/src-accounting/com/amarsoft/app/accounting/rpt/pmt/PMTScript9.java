 ...
 * 等额本息-气球贷尾款计息
 */
public class PMTScript9 extends BasicPMTScript {
	...
	public double getInstalmentAmount(BusinessObject loan, BusinessObject rptSegment) throws LoanException, Exception {
		...
		if (instalmentAmt <= 0d || updateInstalAmtFlag != null
			...
			double tailRate = Double.valueOf(rptSegment.getString("Remark")) / 100.0;// 临时从rpt表的Remark字段取尾款利率,待修改
			...
			if (totalPeriod == 0) {
				...
				instalmentAmount += (balance - outstandingBalance) * tailRate / 12;
			}
	...
	public double getPrincipalAmount(BusinessObject loan, BusinessObject rptSegment) throws LoanException, Exception {
		...
		if (instalmentAmt <= outstandingBalance * installInterestRate)
			...
			double principalAmt = Arith.round(
					instalmentAmt
							- Arith.round(outstandingBalance * installInterestRate,
									AccountingHelper.getMoneyPrecision(loan))
							- Arith.round(rptSegment.getDouble("SegRPTAmount")
									* Double.valueOf(rptSegment.getString("Remark")) / 100.0 / 12),
					AccountingHelper.getMoneyPrecision(loan));
