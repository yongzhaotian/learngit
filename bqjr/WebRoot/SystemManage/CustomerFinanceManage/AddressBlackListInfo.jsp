...
<%
	...
	String sTempletNo = "AddressBlackListInfo";//模型编号
	ASDataObject doTemp = new ASDataObject(sTempletNo,Sqlca);
	if(sTemp.equals("modify")){
		doTemp.setReadOnly("AreaName", true);
	}
...
<script type="text/javascript">
	...
	function saveRecord(sPostEvents){
		...
		if(sArea==null) sArea="";
		if(sTown==null) sTown="";
		if(sVillege==null) sVillege="";
		if(sCell==null) sCell="";
		if(sHouseNo==null) sHouseNo="";
	    var sCnt=RunMethod("BlackListModel","checkAddressMulti",sArea+","+sTown+","+sVillege+","+sCell+","+sHouseNo);
			if(sCnt!="Null"){
				alert("列表中已有重复的地址记录,请重新输入");
				return;
			}
	...
	function goBack(){
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/AddressBlackListList.jsp","","_self");
	}
	...
	function initRow(){
		if (getRowCount(0)==0){//如当前无记录，则新增一条
			as_add("myiframe0");
			var sSerialNo = getSerialNo("AddressBlackList","SerialNo");// 获取流水号
