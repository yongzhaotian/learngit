...
function downExcelMould() {
	var file = "<%=basePath %>/FormatDoc/Excel/graylisthometel.xls";
	...
	function newRecord(){
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListHomeTelInfo.jsp","","_self","");
	}
	...
	function deleteRecord(){
		...
		if(confirm("您真的想删除该信息吗？")){
			RunMethod("GrayList_MODEL","delData","GrayListHomeTel,"+sSerialNo);
			//as_del("myiframe0");
	...
	function viewAndEdit(){
		...
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListHomeTelInfo.jsp","SERIALNO="+sSerialNo,"_self","");
	}
	function importRecords(){
		...
		RunJavaMethodSqlca("com.amarsoft.app.billions.ExcelDataImport", "importGrayListHomeTelModel", "filePath="+sFilePath+",userid="+"<%=CurUser.getUserID()%>"+",orgid="+"<%=CurUser.getOrgID()%>"+",inputdate="+"<%=StringFunction.getTodayNow()%>");
		//处理导入了重复的数据
	    RunMethod("GrayList_MODEL","delMulti_1","GrayListHomeTel,TEL");
		reloadSelf();
	}
