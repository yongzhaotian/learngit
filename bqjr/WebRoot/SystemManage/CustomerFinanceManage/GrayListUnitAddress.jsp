<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	String sTempletNo = "GrayListUnitAddress";//模型编号
...
function downExcelMould() {
	var file = "<%=basePath %>/FormatDoc/Excel/graylistunitaddress.xls";
	...
	function newRecord(){
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListUnitAddressInfo.jsp","","_self","");
	}
	...
	function deleteRecord(){
		...
		if(confirm("您真的想删除该信息吗？")){
			RunMethod("GrayList_MODEL","delData","GrayListUnitAddress,"+sSerialNo);
			//as_del("myiframe0");
	...
	function viewAndEdit(){
		...
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListUnitAddressInfo.jsp","SERIALNO="+sSerialNo,"_self","");
	}
	function importRecords(){
		...
		RunJavaMethodSqlca("com.amarsoft.app.billions.ExcelDataImport", "importGrayListUnitAddressModel", "filePath="+sFilePath+",userid="+"<%=CurUser.getUserID()%>"+",orgid="+"<%=CurUser.getOrgID()%>"+",inputdate="+"<%=StringFunction.getTodayNow()%>");
		//处理导入了重复的数据
	    RunMethod("GrayList_MODEL","delMulti_7","GrayListUnitAddress,PROVINCE,CITY,AREA,TOWN,VILLEGE,CELL,HOUSENO");
		reloadSelf();
	}
