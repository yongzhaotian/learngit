<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	String sTempletNo = "GrayListOfficeTel";//模型编号
...
function downExcelMould() {
	var file = "<%=basePath %>/FormatDoc/Excel/graylistofficetel.xls";
	...
	function newRecord(){
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListOfficeTelInfo.jsp","","_self","");
	}
	...
	function deleteRecord(){
		...
		if(confirm("您真的想删除该信息吗？")){
			RunMethod("GrayList_MODEL","delData","GrayListOfficeTel,"+sSerialNo);
			//as_del("myiframe0");
	...
	function viewAndEdit(){
		...
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListOfficeTelInfo.jsp","SERIALNO="+sSerialNo,"_self","");
	}
	function importRecords(){
		...
		RunJavaMethodSqlca("com.amarsoft.app.billions.ExcelDataImport", "importGrayListOfficeTelModel", "filePath="+sFilePath+",userid="+"<%=CurUser.getUserID()%>"+",orgid="+"<%=CurUser.getOrgID()%>"+",inputdate="+"<%=StringFunction.getTodayNow()%>");
		//处理导入了重复的数据
	    RunMethod("GrayList_MODEL","delMulti_1","GrayListOfficeTel,TEL");
		reloadSelf();
	}
