<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	String sTempletNo = "GrayListUnitName";//模型编号
...
function downExcelMould() {
	var file = "<%=basePath %>/FormatDoc/Excel/graylistunitname.xls";
	...
	function newRecord(){
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListUnitNameInfo.jsp","","_self","");
	}
	...
	function deleteRecord(){
		...
		if(confirm("您真的想删除该信息吗？")){
			RunMethod("GrayList_MODEL","delData","GrayListUnitName,"+sSerialNo);
			//as_del("myiframe0");
	...
	function viewAndEdit(){
		...
		AsControl.OpenView("/SystemManage/CustomerFinanceManage/GrayListUnitNameInfo.jsp","SERIALNO="+sSerialNo,"_self","");
	}
	function importRecords(){
		...
		RunJavaMethodSqlca("com.amarsoft.app.billions.ExcelDataImport", "importGrayListUnitNameModel", "filePath="+sFilePath+",userid="+"<%=CurUser.getUserID()%>"+",orgid="+"<%=CurUser.getOrgID()%>"+",inputdate="+"<%=StringFunction.getTodayNow()%>");
		//处理导入了重复的数据
	    RunMethod("GrayList_MODEL","delMulti_1","GrayListUnitName,COMPANY");
		reloadSelf();
	}
