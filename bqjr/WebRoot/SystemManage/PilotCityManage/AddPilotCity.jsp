...
<%
    ...
    String sPilotType = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("PilotType"));
    if(sPilotType == null) sPilotType = "";
...
<%
	    ...
	    String queryStr = " select itemno,itemname from code_library " +
	              " where itemno not in (select city from pilot_city p where p.verifytype='"+sPilotType+"' and p.subproducttype='"+sSubProductType+"') and CodeNo='AreaCode' and IsInuse='1' and (length(sortNo)=4 and substr(SortNo,1,2) not in ('11','12','31','50') or SortNo in ('11','12','31','50')) order by itemno ASC";
		...
		queryStr = "  select itemno,itemname from code_library where 1=2";
		ASResultSet rs1 = Sqlca.getASResultSet(new SqlObject(queryStr));
		num = 0;
		while(rs1.next()){
...
<script type="text/javascript">
   ...
   function pushButton(buttonName,push){
	...
	function doQuery(){
		...
		RunJavaMethodSqlca("com.amarsoft.proj.action.PilotCityCommon", "addPilotCitys", "pilotType="+sPilotType+" ,subProductType="+sSubProductType+",citys="+vTemp+",userID="+userID);
