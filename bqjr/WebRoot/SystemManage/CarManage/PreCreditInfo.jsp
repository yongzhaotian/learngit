...
<%
	...
	// 获取最新预审数据
	String sSql = "select SerialNo from Basedataset_Info where TypeCode='"+sTypeCode+"' order by UpdateDate desc ";
	String sSerialNo = DataConvert.toString( Sqlca.getString(sSql));//modify  by jli5 增加空值处理
	
	
	// 通过DW模型产生ASDataObject对象doTemp
	String sTempletNo = "PreCreditInfo";//模型编号
...
<script type="text/javascript">
	...
	function goBack(){
		AsControl.OpenView("/SystemManage/CarManage/PreCreditList.jsp","","_self");
	}
	...
	function initRow(){
		if (getRowCount(0)==0){//如当前无记录，则新增一条
			...
			setItemValue(0,getRow(),"TypeCode","<%=sTypeCode %>");
			setItemValue(0, 0, "InputOrg", "<%=CurOrg.orgID %>");
