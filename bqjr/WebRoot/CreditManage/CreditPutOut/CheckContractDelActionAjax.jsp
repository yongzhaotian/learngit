...
<% 
	...
	/*sSql = 	" select FinishDate,PigeonholeDate,ManageUserID,BusinessType,ReinforceFlag "+
	    ...
	    if(sFinishDate.equals("")) //未终结
	    {
	    	if(sPigeonholeDate.equals("")) //未完成放贷
	    	{
	    		if(sBusinessType.substring(0,1).equals("3")) //授信额度
	    		{
		    		...
		    		{
		    			if(!sManageUserID.equals(CurUser.getUserID())) //其他信贷人员管户
		    			{
		    				sReturnValue = "Other";
		    			}
		    		} 
	    		}else
	    		{
			    	...
			    	so = new SqlObject(sSql).setParameter("ContractSerialNo",sObjectNo);		
		    		rs = Sqlca.getASResultSet(so);
		    		if(rs.next())
		    			iCount = rs.getInt(1);
		    		rs.getStatement().close();
