...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 业务申请新增的担保信息所对应的质物信息列表;
		Input Param:
			ObjectType：对象类型
			ObjectNo: 对象编号
			ContractNo：担保信息编号
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord(){
		...
		OpenPage("/CreditManage/GuarantyManage/ApplyImpawnInfo1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo=<%=sContractNo%>&ImpawnType="+sImpawnType,"_self");
	}
	
	/*~[Describe=引入抵押物;InputParam=无;OutPutParam=无;]~*/
	...
	function viewAndEdit(){
		...
		if(typeof(sGuarantyID)=="undefined" || sGuarantyID.length==0){
			...
			OpenPage("/CreditManage/GuarantyManage/ApplyImpawnInfo1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo=<%=sContractNo%>&GuarantyID="+sGuarantyID+"&ImpawnType="+sImpawnType,"_self");
		}
	}
	</script>
<%/*~END~*/%>
