...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	//设置过滤条件	
	sTempletFilter = " (ColAttribute like '%ApproveApply%' ) ";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function goBack()
	{
		OpenPage("/CreditManage/GuarantyManage/ApprovePawnList1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo=<%=sContractNo%>","_self","");
	}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{				
		...
		{
			sGuarantyID = getItemValue(0,getRow(),"GuarantyID");//担保信息流水号
