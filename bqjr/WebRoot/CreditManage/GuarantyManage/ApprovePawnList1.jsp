...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 最终审批意见新增的担保信息所对应的抵押物信息列表;
		Input Param:
			ObjectType：对象类型
			ObjectNo: 对象编号
			ContractNo：担保信息编号
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
	String sButtons[][] = {
		{"true","All","Button","新增","新增抵押物信息","newRecord()",sResourcesPath},
		{"true","All","Button","引入","引入已经登记的抵押物信息","importGuaranty()",sResourcesPath},
		{"true","","Button","详情","查看抵押物信息详情","viewAndEdit()",sResourcesPath},
		{"true","All","Button","删除","删除抵押物信息","deleteRecord()",sResourcesPath}
	};
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord(){
		...
		sPawnType = sReturn[0];	
		OpenPage("/CreditManage/GuarantyManage/ApprovePawnInfo1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo=<%=sContractNo%>&PawnType="+sPawnType,"_self");
	}
	...
	function viewAndEdit(){
		...
		if(typeof(sGuarantyID)=="undefined" || sGuarantyID.length==0){
			...
			OpenPage("/CreditManage/GuarantyManage/ApprovePawnInfo1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo=<%=sContractNo%>&GuarantyID="+sGuarantyID+"&PawnType="+sPawnType,"_self");
		}
	}
	</script>
<%/*~END~*/%>
