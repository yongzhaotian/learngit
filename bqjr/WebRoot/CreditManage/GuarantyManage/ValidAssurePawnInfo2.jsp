...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 最高额担保合同所对应的抵押物基本信息详情（有效的）;
		Input Param:
			PawnType：抵押物类型				
		    ContractNo: 担保合同编号
			GuarantyID：抵押物编号
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	sGuarantyStatus = Sqlca.getString(new SqlObject("select GuarantyStatus from GUARANTY_INFO where GuarantyID = :GuarantyID").setParameter("GuarantyID",sGuarantyID));
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function goBack()
	{
		OpenPage("/CreditManage/GuarantyManage/ValidAssurePawnList2.jsp?ContractNo=<%=sContractNo%>","_self","");
	}
