...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "质物信息变更"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	ASDataObject doTemp = new ASDataObject("ImpawnChangeInfo",sTempletFilter,Sqlca);
	...
	//设置出质人选择框
/* 	doTemp.setUnit("NewCertID"," <input type=button value=.. onclick=parent.selectCustomer()><font color=red>(存在的客户请选择,否则请输入)</font>"); */
/* 	doTemp.setHTMLStyle("NewCertType,NewCertID"," onchange=parent.getCustomerName() ");
	...
	doTemp.setHTMLStyle("OldEvalOrgName"," style={width:200px} "); */
	
/* 	doTemp.setUnit("NewEvalOrgName"," <input type=button value=.. onclick=parent.selectEvalOrgName()>");		 */
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function goBack()
	{
		OpenPage("/CreditManage/GuarantyManage/ChangeImpawnList.jsp","_self","");
	}
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	{			
		...
		//校验新出质人贷款卡编号
		sNewLoanCardNo = getItemValue(0,getRow(),"NewLoanCardNo");//新出质人贷款卡编号	
		if(typeof(sNewLoanCardNo) != "undefined" && sNewLoanCardNo != "" )
		{
			
			//检验新出质人贷款卡编号唯一性
			sNewOwnerName = getItemValue(0,getRow(),"NewOwnerName");//新出质人名称	
			sReturn=RunMethod("CustomerManage","CheckLoanCardNo",sNewOwnerName+","+sNewLoanCardNo);
			if(typeof(sReturn) != "undefined" && sReturn != "" && sReturn == "Many") 
			{
				alert(getBusinessMessage('244'));//该新出质人贷款卡编号已被其他客户占用！							
				return false;
			}						
		}
			
		//检查输入的新出质人是否建立信贷关系，如果未建立，需要新获取新出质人的客户编号
