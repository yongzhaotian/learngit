...
<%
	...
	String imgpath = CurConfig.getConfigure("ImageFolder");
  String renzhengbao="K";//是否已经调用了认证宝
	...
	if ("NCIIC信息自动检查".equals(sCheckPhaseName)) {
		...
		sRet = RunID5.runParserId5(Sqlca, sCustomerID, reqHeader, reqData, savepath, "010",imgpath);
		}
	} else if (("ID5办公电话检查".equals(sCheckPhaseName) || "ID5办公电话核查".equals(sCheckPhaseName)) && bUserId5) {
		//-- add by 农民产品无办公电话报错修改 tangyb 20160108 --//
		...
		if (reqData != null && !"".equals(reqData)) {
			...
			if(!"timeOut".equals(sRet)){
				sRet = RunID5.runParserId5(Sqlca, sCustomerID, reqHeader, reqData, savepath, "020",imgpath);
			}
		} else {
			reqData = "";
			reqHeader = "1C1G01";
    ...
    function doSubmit()
	{
		...
		if(typeof(sPhaseInfo)=="undefined" || sPhaseInfo=="" || sPhaseInfo==null || sPhaseInfo=="null" || sPhaseInfo=="_CANCEL_") return;
			...
			if(isSameUser=="Yes"){
				...
				parent.parent.reloadSelf();//CCS-485   --无预约现金贷附条件审批   --ybpan
			}else{
				window.returnValue = "NotSameUser";
				parent.parent.parent.reloadSelf();
			}
			 var sssphaseNO = RunMethod("WorkFlowEngine", "GetPhaseNo", sObjectType+","+sObjectNo);
			if(sssphaseNO=="2000"){
				window.close();
				parent.parent.parent.reloadSelf();
			} 
