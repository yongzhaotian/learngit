...
<script type="text/javascript">	
	...
	AsControl.OpenView("/CreditManage/CashLoan/CashLoanOpinionInfoNew.jsp","","frameright",OpenStyle);
	setTimeout(
			function ID5(){
				...
				if(sRzb=="N"){
					if(sPhaseName=="NCIIC信息自动检查"){
						...
						sReqHeader = "1A020201";
						//sCount = RunMethod("公用方法","GetColValue", "ID5_XML_ELE_VAL,count(1),serialno='"+sReqHeader+"#"+sReqData+"'");
						...
						sReqData = sReqData.replace("-", "");
						sReqHeader = "1C1G01";
						//sCount = RunMethod("公用方法","GetColValue", "ID5_XML_ELE_VAL,count(1),serialno='"+sReqHeader+"#"+sReqData+"' and (TO_DATE(TO_CHAR(SYSDATE,'yyyy/mm/dd'),'yyyy/mm/dd')-(TO_DATE(SUBSTR(INPUTDATE,1,10), 'yyyy/mm/dd')+30))>0");
						sCount = RunJavaMethodSqlca("com.amarsoft.biz.workflow.action.CheckID5TimeOut", "CheckTimeOut", "ReqHeader="+sReqHeader+",ReqData="+sReqData);
						sReqData = "WORKTEL";
					...
					if((sCount!=""||sCount.length!=0)&&sCount=="0"&&sReqData!=""){
						AsControl.OpenView("/CreditManage/CashLoan/CashLoanOpinionInfoNew.jsp","ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&FlowNo=<%=sFlowNo%>&PhaseNo=<%=sPhaseNo%>&Ret="+sRet,"frameright",OpenStyle);
					}
				}
			}, 10000);
</script>	
