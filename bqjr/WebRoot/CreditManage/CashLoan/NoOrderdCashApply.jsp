...
<%
	String PG_TITLE = "无预约现金贷款管理"; // 浏览器窗口标题 <title> PG_TITLE </title>
...
<script type="text/javascript">
	...
	function newApply(){
		...
		if(confirm("当前登录所在门店为：\n\r"+sStore+"\n\r是否确认在该门店发起申请？")){
			var rValues=RunJavaMethodSqlca("com.amarsoft.app.billions.RetailStoreCommon", "getCreditID", "ProductType=2,citys="+ssCity);
			if(rValues=="false"){
	...
	function doSubmit(){
		...
		var sObFlowNo = RunMethod("公用方法", "GetColValue", "FLOW_OBJECT,OBJATTRIBUTE5,OBJECTNO='"+sObjectNo+"'");
		 var sUserID = "<%=CurUser.getUserID()%>"; //登录人账号
		...
		var sOtherLoanMassageTemp = RunMethod("公用方法", "GetColValue", "OTHER_LOAN_MASSAGE,COUNT(*),CONTRACTNO='"+sContratNo+"'");
		var sAssistTempFlag=RunMethod("公用方法", "GetColValue", "ASSISTINVESTIGATE,TEMPSAVEFLAG,OBJECTNO='"+sContratNo+"'");

		//add by tangyb 20150522 无预约现金贷 (外部名单客户)---
		//查询是否需要无预约现金贷外部名单客户检查[0:否,1:是]
		var isOutuser = RunJavaMethodSqlca("com.amarsoft.app.billions.OutUserCheck", "queryOutuserPara", "userID="+sUserID+",businessType="+sBusinessType);
		//查询是否上传个人征信授权书[0:否,1:是]
		var isUpload = RunJavaMethodSqlca("com.amarsoft.app.billions.OutUserCheck", "queryCreditAuthorization", "objectNo="+sContratNo);
		...
		//update by tangyb 20150522 无预约现金贷 (外部名单客户)---
		if(isOutuser != "1"){ //外部名单客户其他贷款信息选填
			if (sOtherLoanMassageTemp == "0.0") {	// 校验合同信息是否已经保存
				alert("请先保存其他贷款信息再提交！");
				return;
			}
		}
		...
		//add by tangyb 20150522 无预约现金贷 (外部名单客户)---
		if(isOutuser == "1"){ //外部名单客户必须上传个人征信授权书
			if(isUpload == "0"){
				alert("请先上传个人征信授权书再提交！");
				return;
			}
		}
		...
		var sPhaseInfo = "";
		//若流程阶段为附条件批准，则提交后走提交选择窗口     add  by ybpan at 20150424 CCS-485   无预约现金贷附条件批准功能
		if(sPhaseNo=='2000'){
			//查询规则引擎返回的期数金额以及合同表中的期数金额及产品编号
			var sSubConditionCapitalPeriod = RunJavaMethodSqlca("com.amarsoft.proj.action.ProductChange","subConditionCapitalPeriod","businessContractSerialNo="+sObjectNo);
			var sReturnString = sSubConditionCapitalPeriod.split("@");
			var  sCprincipal = sReturnString[0];//规则引擎返回的金额
			var  sCperiod = sReturnString[1];//规则引擎返回的期数
			var  sBusinessCprincipal = sReturnString[2];//合同中的金额
			var  sBusinessCperiod = sReturnString[3];//合同中的期数
			//var sBusinessType =sReturnString[4] ;//产品代码
			sCprincipal=parseInt(sCprincipal);
			sCperiod=parseInt(sCperiod);
			var sCprincipalNew	=parseInt(sBusinessCprincipal)+sCprincipal;   //改变后的金额
			var sCperiodNew	=parseInt(sBusinessCperiod)+sCperiod;  //改变后的期数
			var sPrincipalFlag = RunJavaMethodSqlca("com.amarsoft.proj.action.ProductChange","cprincipalMapping","BusinessType="+sBusinessType+",CprincipalNew="+sCprincipalNew);
			if(sPrincipalFlag=="FAIL"){
				alert("修改后的金额超出产品金额界限,提交失败");
				return;
			}
			//查询有没有符合条件的产品
			var msg = sPrincipalFlag.split("@");
			var  sTallPrincipal = msg[0];
			var  sLowPrincipal = msg[1];
			var  sProductSeriesNo = msg[2];
			
			var sNewBusinessType = RunJavaMethodSqlca("com.amarsoft.proj.action.ProductChange","checkBusinessType","CprincipalNew="+sCprincipalNew+",CperiodNew="+sCperiodNew+",productSeriesNo="+sProductSeriesNo);
			if(sNewBusinessType==""){
		    	alert("没有复合条件的产品");
		    	return;
		    } 
			...
			sPhaseInfo = PopPage("/Common/WorkFlow/SubmitDialog.jsp?SerialNo="+sTaskNo+"&objectType="+sObjectType+"&objectNo="+sObjectNo+"&oldPhaseNo="+sPhaseNo,"","dialogWidth=580px;dialogHeight=420px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
       		//如果销售人员选择的是客户同意则对合同表进行修改
       		var sSubContractStatus = RunMethod("WorkFlowEngine","GetPhaseNo",sObjectType+","+sObjectNo);
       		if(sSubContractStatus=="1000"){
       			RunJavaMethodSqlca("com.amarsoft.proj.action.ProductChange","changeBusinessType","oldCprincipal="+sBusinessCprincipal+",oldCperiod="+sBusinessCperiod+",CprincipalNew="+sCprincipalNew+",CperiodNew="+sCperiodNew+",businessContractSerialNo="+sObjectNo+",BusinessType="+sBusinessType+",NewBusinessType="+sNewBusinessType);
       			var sReturn_Message = PopComp("RecountCreditInfo","/CreditManage/CreditApply/RecountCreditInfo.jsp","ObjectNo="+sContratNo+"&ObjectType=BusinessContract","dialogWidth=0;dialogHeight=0;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
				if("Success" == sReturn_Message) {
					// 重新生成还款计划 added by huangshuo
					params = "objectNo=" + sContratNo + ",userID=<%=CurUser.getUserID()%>," 
						+ ",org=<%=CurUser.getOrgID()%>";
					...
					if(res == null || res.length == 0 || res == "Error") {
						...
						alert("生成还款信息成功！合同金额与期限修改成功！");
					} 
				}
				
       			RunMethod("LoanAccount","deleteFormatdocRecord",sObjectNo+",ApplySettle");
       		}
			//end by ybpan 20150427 CCS-485   无预约现金贷附条件批准功能
			}else
				{
		...
		if(typeof(sPhaseInfo)=="undefined" || sPhaseInfo=="" || sPhaseInfo==null || sPhaseInfo=="null" || sPhaseInfo=="_CANCEL_") return;
				...
				if(sSubContractStatus!="1000"&&sSubContractStatus!="8000"){
					RunMethod("公用方法","UpdateColValue","BUSINESS_CONTRACT,ContractStatus,070,SerialNo = '"+sObjectNo+"'");
				}else if(sSubContractStatus=="1000"){
					var sSubContractStatus = RunMethod("BusinessManage","UpdateContractStatus",sObjectNo+",080");
				}else if(sSubContractStatus=="8000"){
					var sSubContractStatus = RunMethod("BusinessManage","UpdateContractStatus",sObjectNo+",010");
				}
			alert(getHtmlMessage('18'));//提交成功！
			reloadSelf();
		}else if (sPhaseInfo == "Failure"){
...
//  ============================== start  打印格式化报告 ============================================================
	...
	function viewApprove(){
		...
		PopPage("/CreditManage/CashLoan/ApproveCondition.jsp?BusinessContractSerialno="+SerialNo,"myprint10","dialogWidth=30;dialogHeight=5;status:no;center:yes;help:no;minimize:yes;maximize:no;border:thin;statusbar:no");//CCS-485 add by ybpan 无预约附条件批准 at 20150424
	}
