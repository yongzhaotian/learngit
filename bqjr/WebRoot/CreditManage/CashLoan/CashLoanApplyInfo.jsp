...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "CashNewLoanApplyInfo";
    ...
    String InsuranceNo=Sqlca.getString(new SqlObject("select sp.ins_serialno from bq_insurance_info sp,insurancecity_info ii "+
    			"where sp.ins_serialno = ii.insuranceno and sp.ins_status = '1' and ii.cityno='"+sCity+"' and ii.subproducttype='1'"));
    ...
    String sSql="select sp.serialno as SerialNo, sp.serviceprovidersname as ServiceProvidersName "+
	            "from Service_Providers sp,ProvidersCity pc where sp.customertype1 = '06' and pc.ProductType = '1' "+
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function doReturn(){
		...
		RunMethod("AssistInvestigateManage","AddAssistInvestigate",sSerialNo+","+sObjectNo); */
	...
	//   alert("Sum:"+sSum+"ProductSum:"+sProductSum+"PaymentSum:"+sPaymentSum+"Shuofuratio:"+sShoufuratio1);
	    ...
	    //该客户对应有效活动（状态为有效且申请日期在活动起止日期范围内）下该客户信用额度
	    var sCreditlimit = RunMethod("BusinessManage", "GetCashLoanInfo", "bcr.creditlimit,"+sCustomerID);
	    if(null == sCreditlimit || "" == sCreditlimit || "undefined" == sCreditlimit || "Null" == sCreditlimit)
	    {
		    alert("该客户没有对应的有效活动，请检查！");
		    return;
		} 
	    if(sBusinessSum > sCreditlimit)
	    {
		    alert("超出客户信用限额！");
		    return;
		}
	  //add by phe 2015/03/31 CCS-572 PRM-254 安硕系统中限制销售代表为自己申请员工贷
		...
		sParaString = "pID"+","+sProductID+","+"Sum"+","+sSum+","+"cID"+","+sCustomerID+","+"SNo,<%=sNo%>,eventattribute,01";
		//设置返回参数 
		//serialno@eventname@productid@productname@TypeNo@TypeName@Term
		setObjectValue("SelProductForCashLoanInfo",sParaString,"@EventID@0@EventName@1@BusinessType@4@ProductName@5@Periods@6",0,0,""); 
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"SubProductType","1");
		}
