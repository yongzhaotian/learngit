...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 最终审批意见所对应的新增的担保信息列表（一个保证合同对应多个保证人）;
		Input Param:
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord()
	{
		...
		{
			OpenPage("/CreditManage/CreditAssure/ApproveAssureInfo3.jsp?GuarantyType="+sGuarantyType,"right");
		}
	}
	...
	function viewAndEdit()
	{
		...
		}else 
		{
			...
			OpenPage("/CreditManage/CreditAssure/ApproveAssureInfo3.jsp?SerialNo="+sSerialNo+"&GuarantyType="+sGuarantyType,"right");
		}
	}
	...
	function mySelectRow()
	{
		...
		}else
		{
			...
			if (sGuarantyType.substring(0,3) == "010") //保证
				OpenPage("/CreditManage/GuarantyManage/ApproveGuarantorList1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
			else if (sGuarantyType.substring(0,3) == "050") //抵押
				OpenPage("/CreditManage/GuarantyManage/ApprovePawnList1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
			else //质押
				OpenPage("/CreditManage/GuarantyManage/ApproveImpawnList1.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
		}
	}
