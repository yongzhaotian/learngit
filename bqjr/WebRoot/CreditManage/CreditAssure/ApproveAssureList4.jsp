...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 最终审批意见所对应的拟引入的担保合同列表（一个保证合同对应多个保证人）;
		Input Param:
...
<%
	...
	sSql =  " select GC.SerialNo,GC.CustomerID,GC.GuarantyType, "+
			...
			" where  exists (Select AR.ObjectNo from APPROVE_RELATIVE AR where "+
	...
	//获取业务申请人代码
	sSql = " select CustomerID from BUSINESS_APPROVE where SerialNo =:SerialNo ";
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//6.资源图片路径
	String sButtons[][] = {
		{"true","All","Button","引入","引入担保合同信息","importRecord()",sResourcesPath},
		{"true","","Button","详情","查看担保合同信息详情","viewAndEdit()",sResourcesPath},
		{"true","All","Button","删除","删除担保合同信息","deleteRecord()",sResourcesPath},
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function importRecord(){
	    //传入当前的条件即可
		sParaString = "RelativeTableName,APPROVE_RELATIVE RT,ObjectNo,<%=sObjectNo%>,ContractStatus,020,ContractType,020,OrgID,<%=CurUser.getOrgID()%>";
	...
	function viewAndEdit(){
		...
		if(typeof(sSerialNo)=="undefined" || sSerialNo.length==0) {
			...
			OpenPage("/CreditManage/CreditAssure/ApproveAssureInfo4.jsp?SerialNo="+sSerialNo+"&GuarantyType="+sGuarantyType,"right");
		}
	}
	...
	function mySelectRow(){
		...
		if(typeof(sSerialNo)=="undefined" || sSerialNo.length==0) {
			...
			if (sGuarantyType.substring(0,3) == "010") //保证
				OpenPage("/CreditManage/GuarantyManage/ApproveGuarantorList2.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
			else if (sGuarantyType.substring(0,3) == "050") //抵押
				OpenPage("/CreditManage/GuarantyManage/ApprovePawnList2.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
			else //质押
				OpenPage("/CreditManage/GuarantyManage/ApproveImpawnList2.jsp?ObjectType=<%=sObjectType%>&ObjectNo=<%=sObjectNo%>&ContractNo="+sSerialNo,"DetailFrame","");
		}
	}
	</script>
<%/*~END~*/%>
