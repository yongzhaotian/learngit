...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "CreditNewLineApplyCreationInfo";
    ...
    String InsuranceNo=Sqlca.getString(new SqlObject("select sp.ins_serialno from bq_insurance_info sp,insurancecity_info ii "+
    			"where sp.ins_serialno = ii.insuranceno and sp.ins_status = '1' and ii.cityno='"+sCity+"' and ii.subproducttype='0'"));
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function InsertCustomer(){
		...
		if(parseInt(sReturnBmd) > 0){
			setItemValue(0,0,"IsBaimingdan","1");//白名单标志
			var sParam = "";
			/*****************update huzp 二段式提单需求修改 begin*******************/
			if("1"=="<%=switch_status%>"){
			...
			var sReturnCID = RunMethod("CustomerManage","AddCustomerAction4Bmd",sParam);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function doCreation()
	{
		...
		if(sProductID=="020"){
			...
			var CertID = getItemValue(0,0,"CertID");//身份证号
			var beginStr = BusinessType.substring(0,4);
			var flag = "1";
			if(beginStr == "DYYH"){
				flag = RunMethod("公用方法", "GetClerkNumber", CustomerName+","+CertID);
			}else if(beginStr == "YGYH"){
				flag = RunMethod("公用方法", "GetEmployeeNumber",CustomerName+","+CertID);
			}else{
			}
			if(parseInt(flag) == 0){
				alert("没有备案店员或员工信息，或者录入信息与备案信息不符。");
				return;
			}else{
	...
	function selectProductID()
	{
	 ...
	 if (typeof(sProductSum)=="undefined" || sProductSum=="_CLEAR_" || sProductSum.length==0 
	  ...
	  // add by phe 2015/03/31 CCS-572 PRM-254 安硕系统中限制销售代表为自己申请员工贷
		...
		sParaString = "ProductID"+","+sProductID+","+"Sum"+","+sSum+","+"ProductSum"+","+sProductSum+","+"PaymentSum"+","+sPaymentSum+","+"ProductCategory"+","+sProductCategory+","+"Shoufuratio"+","+sShoufuratio1+",SNo,<%=sNo%>";
		
		//设置返回参数 
		var result = setObjectValue("SelectPosBusinessInfo",sParaString,"@BusinessType@0@ProductName@1@Periods@2",0,0,""); 
...
//是否投保，联动计算每月还款额
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"SubProductType","0");
