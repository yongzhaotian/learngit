<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	...
	String sTempletNo = "ImageListByProductType";//模型编号
	ASDataObject doTemp = new ASDataObject(sTempletNo,Sqlca);

 	doTemp.generateFilters(Sqlca);
 	doTemp.setFilter(Sqlca, "020", "IMAGE_TYPE_NAME", "Operators=EqualsString,BeginsWith;");
...
//	doTemp.setReadOnly("TypeNo", true);
	...
	for(int i=0;i<vTemp.size();i++) {
 	...
 	String sButtons[][] = {
			{"true","","Button","影像维护","影像维护","maintainImage()",sResourcesPath},
	}; 
...
<script type="text/javascript">
	...
	function maintainImage(){
		var sProductTypeId = getItemValue(0,getRow(),"PRODUCT_TYPE_ID");
		var re = PopPage("/ImageManage/AddImageFile.jsp?ProductTypeId="+<%=productTypeId%>+"&compNo=2","","dialogWidth=550px;dialogHeight=350px;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;");
        if(re==1){
