...
//	doTemp.setReadOnly("TypeNo", true);
	...
	for(int i=0;i<vTemp.size();i++) {
 	...
 	String sButtons[][] = {
			{"true","","Button","贷后资料维护","贷后资料维护","maintainImage()",sResourcesPath},
	}; 
...
<script type="text/javascript">
	...
	function maintainImage(){
		//获取产品类型编号
		var sProductTypeId = getItemValue(0,getRow(),"PRODUCT_TYPE_ID");
		var sProductTypeID = "<%=productTypeId%>";
		var re = PopPage("/ImageManage/AddImageFile.jsp?ProductTypeId="+sProductTypeID+"&compNo=3","","dialogWidth=550px;dialogHeight=350px;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;");
		parent.reloadSelf();
		if(re==1){
	...
	function maintainTimeAndCity(){
		var sProductTypeId = getItemValue(0,getRow(),"PRODUCT_TYPE_ID");
		var sProductTypeID = "<%=productTypeId%>";
		var re = PopPage("/ImageManage/MaintainProductForUpload.jsp?ProductTypeId="+sProductTypeID,"","dialogWidth=550px;dialogHeight=350px;status:no;center:yes;help:no;minimize:no;maximize:no;border:thin;");
        if(re==1){
			window.location.reload();
        }
	}
