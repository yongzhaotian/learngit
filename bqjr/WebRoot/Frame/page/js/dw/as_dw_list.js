...
function tableSearchFromInput0(obj,from,params,clearSort){
	TablePage.from = from;
	var tableIndex = 0;
...
function openFilterArea(divid,tableIndex,evt){
	...
	for(var i=0;i<aDWfilterTitles[tableIndex].length;i++){
		...
		if(DZ[tableIndex][1][iColIndex][20] && DZ[tableIndex][1][iColIndex][20]!=""){
			//插入操作符号
			var sInputOptions = "<select name='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP' id='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP'><option value='In' "+getFilterOptionSelection(tableId,iColIndex,'In')+">等于　　　</option></select>";
			...
			if(dateString.length>0 || (DZ[tableIndex][1][iColIndex][12]==5 || DZ[tableIndex][1][iColIndex][12]==2 || DZ[tableIndex][1][iColIndex][12]==6 || DZ[tableIndex][1][iColIndex][12]==7 || DZ[tableIndex][1][iColIndex][12]>10)){//日期控件和数字控件 需要重新设置调整键
				sInputOptions = "<select name='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP' id='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP' onchange=showFM('"+tableId+"',"+iColIndex+",this.value,'"+filterNames[tableIndex][i]+"')><option value='Equals' "+getFilterOptionSelection(tableId,iColIndex,'Equals')+">等于　　　</option><option value='Not' "+getFilterOptionSelection(tableId,iColIndex,'Not')+">不等于　</option><option value='BigThan' "+getFilterOptionSelection(tableId,iColIndex,'BigThan')+">大于</option><option value='BigEqualsThan' "+getFilterOptionSelection(tableId,iColIndex,'BigEqualsThan')+">大于等于</option><option value='LessThan' "+getFilterOptionSelection(tableId,iColIndex,'LessThan')+">小于</option><option value='LessEqualsThan' "+getFilterOptionSelection(tableId,iColIndex,'LessEqualsThan')+">小于等于</option><option value='Area' "+getFilterOptionSelection(tableId,iColIndex,'Area')+">在…之间</option>";
			}
				...
				sInputOptions = "<select name='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP' id='DOFILTER_DF_"+ filterNames[tableIndex][i] +"_1_OP' onchange=showFM('"+tableId+"',"+iColIndex+",this.value,'"+filterNames[tableIndex][i]+"')><option value='In' "+getFilterOptionSelection(tableId,iColIndex,'In')+">等于　　</option><option value='Like' "+getFilterOptionSelection(tableId,iColIndex,'Like')+">包含　　</option>";
...
function change_height(){	
	...
	try {
		...
		oldLeftTD = document.getElementById("Left_TD_myiframe0").offsetWidth;
		//else document.getElementById("Left_TD_myiframe0").style.width = oldLeftTD;
		document.getElementById("Right_TD_myiframe0").style.width = document.body.offsetWidth - oldLeftTD - 10;
