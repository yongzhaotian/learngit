...
TableBuilder.subListLeftColspan = 1;//子 表左侧区域空几列
TableBuilder.pageDepth =Math.floor(Math.random()*10000);//使用随机数作为深度，尽可能保证下层次的编号与本层次不冲突
...
TableBuilder.expandSubTable = function(obj,rowindex){
	var controlobj = document.getElementById("SUBTR_"+TableBuilder.pageDepth+"_" + rowindex);
	if(obj.getAttribute("isEpanded")=="true"){
		obj.setAttribute("isEpanded","false");
		obj.className = "list_gridCell_narrow tree_grid_folder_collapse";
		if(controlobj)controlobj.style.display="none";
	}
	else{
		obj.setAttribute("isEpanded","true");
		obj.className = "list_gridCell_narrow tree_grid_folder_expand";
		if(controlobj)controlobj.style.display="block";
		if(obj.getAttribute("funloaded")!="true" && window.displaySubTable){
			displaySubTable(rowindex,"SUBIFR_"+TableBuilder.pageDepth+"_" +rowindex);
			obj.setAttribute("funloaded","true");
		}
	}
}
...
TableBuilder.prototype.createTableHTML = function(){
	...
	sbResult.append('<th><div class="list_left_border"><div class="list_gridCell_narrow">&nbsp;</div></div></th>');
	if(this.uiparams.multySelect && this.uiparams.multySelect==1){
		sbResult.append('<th class="list_gridCell_narrow"><div class="list_gridCell_narrow"><input class="list_left_cbInput" type="checkbox" id="DW_CheckAll_'+ this.tableId +'" name="CheckAll_'+ this.tableId +'" onclick="TableBuilder.SelAll(\''+ this.tableId +'\',this)"></div></th>');
	}
	//输出行号标题：空格
	sbResult.append('<th class="list_gridCell_narrow"><div class="list_gridCell_narrow">&nbsp;</div></th>');
	...
	sbResult.append('</div>');
	//创建活动数据列
	sbResult.append('<div id="'+this.tableId+'_cells" style="  width:100%;height:'+ this.floatHeight +'px;overflow-x:auto;overflow-y:auto;" onscroll="document.getElementById(\''+ this.tableId +'_float\').scrollLeft=this.scrollLeft">');
	sbResult.append('<table border="0" cellspacing="0" cellpadding="0">');
	...
	for(var i=0;i<tableDatas[this.tableId].length;i++){
		...
		//输出+
		sbResult.append('<td class="list_checkbox_td"><div class="list_gridCell_narrow tree_grid_folder_collapse" onclick="TableBuilder.expandSubTable(this,'+i+')">&nbsp;</div></td>');
		//输出checkbox
		if(this.uiparams.multySelect && this.uiparams.multySelect==1)
		...
		sbResult.append('</tr>');
		var iColspan = DZ[0].length;//tableDatas[this.tableId].length;
		if(this.uiparams.multySelect && this.uiparams.multySelect==1){
			iColspan+=TableBuilder.subListLeftColspan+1;
		}
		else
			iColspan+=TableBuilder.subListLeftColspan;
		if(TableBuilder.subListLeftColspan>0){
			sbResult.append('<tr id="SUBTR_'+TableBuilder.pageDepth+'_'+i+'" style="display:none">');
			sbResult.append('<td class="list_all_no" colspan='+ TableBuilder.subListLeftColspan +'>&nbsp;</td>');
			sbResult.append('<td colspan="'+iColspan+'" class="list_all_no" ><iframe id="SUBIFR_'+TableBuilder.pageDepth+'_'+i+'" name="SUBIFR_'+TableBuilder.pageDepth+'_'+i+'" width="100%" height="10" frameborder="0" onload="try{reloadFrame(this.id)}catch(e){}"></iframe></td></tr>');
		}
		else
			sbResult.append('<tr id="SUBTR_'+TableBuilder.pageDepth+'_'+i+'" style="display:none"><td colspan="'+(iColspan+1)+'" class="list_all_no" ><iframe id="SUBIFR_'+TableBuilder.pageDepth+'_'+i+'" name="SUBIFR_'+TableBuilder.pageDepth+'_'+i+'" width="100%" height="10" frameborder="0" onload="try{reloadFrame(this.id)}catch(e){}"></iframe></td></tr>');
	}
	//输出右侧小计数据
	if(this.clientCount){
		var countArr = this.clientCount[1].split(",");
...
TableBuilder.setColumnWidth = function(tableId,colindex,width){
	...
	//alert("colindex = " + colindex+",lockCols="+lockCols);
	if(colindex<lockCols){//冻结区域
		headTh = document.getElementById("TH_Left_" + tableId);
