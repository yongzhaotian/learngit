...
TableBuilder.setDisplayRule = function(tableId,displayRule){
	//alert(displayRule);
	for(var i=0;i<displayRule.length;i++){
		if(displayRule[i]==undefined)continue;
		//alert(displayRule[i]);
		//获得第i列的所有规则
		var regulars = displayRule[i];
		var index = TableFactory.getTableColIndexFromDZ(tableId,i);
		for(var j=0;j<regulars.length;j++){
			//获得该列下每一行的值
			for(var r=0;r<tableDatas[tableId].length;r++){
				var value = tableDatas[tableId][r][index];
				//alert("value0 = " + value);
				//获得每一项的规则
				for(var t=0;t<regulars[j].length;t+=2){
					//alert("regulars[j][t] = " + regulars[j][t]);
					//实现值替换
					var replaceValue = regulars[j][t].replace(/\{VALUE\}/,"'" + value + "'");
					//运算
					//alert(replaceValue);
					if(t==regulars[j].length-1){//如果是最后一个，那么就是else情况
						TableBuilder.setItemStyle(tableId,r,index,regulars[j][t]);
						break;
					}
					else if(regulars[j][t]=="" || eval("("+ replaceValue +")") ){
						TableBuilder.setItemStyle(tableId,r,index,regulars[j][t+1]);
						break;
					}
				}
			}
...
TableBuilder.prototype.initValue = function(){
	//如果没有冻结列会自动设lockCols为1
	if(this.depth && this.lockCols==0){
		this.lockCols = 1;
	}
...
TableBuilder.prototype.getFilterHtml = function(index){
	...
	if(DZ[this.tableId.substring(8)][1][iDZColIndex][21]=="" && TableBuilder.isStrInArray2(filterNames[this.tableId.substring(8)],sColName)>-1  && this.depth==undefined){
		...
		sortHtml =  sortHtml + '<span isfilter=\'true\' id=\''+sColName.toUpperCase()+'_SEARCH\' class=\'list_search_span\'>&nbsp;&nbsp;</span>';
	}
	//alert(sortHtml);
	return sortHtml;
};
...
TableBuilder.prototype.createTableHTML = function(){
	...
	sbResult.append('<table border="0" cellspacing="0" cellpadding="0" width="100%"  id="'+ this.tableId +'" origFloatWidth="'+ this.floatWidth +'" origFloatHeight="'+ this.floatHeight +'" lockCols="'+ this.lockCols +'" onselectstart="javascript:if(event.shiftKey)return false;">');
	//alert(300);
	sbResult.append('<tr><td id="Left_TD_'+this.tableId+'" valign="top">');
	//创建冻结栏目标题
	//冻结栏
	
	sbResult.append('<div class="list_topdiv_header">');
	//这里的class处理显示左边框
	sbResult.append('<table style="width:100%" class="list_left_border" border="0" cellspacing="0" cellpadding="0">');
	//输出合并标题
	if(this.combineLeft){
		sbResult = this.getCombineHtml(sbResult,this.combineLeft);
	}
	sbResult.append('<thead id="TH_Left_'+ this.tableId +'">');
	//输出最左侧选择栏
	//alert("this.uiparams.multySelect = " + this.uiparams.multySelect);
	if(this.uiparams.multySelect && this.uiparams.multySelect==1)
		sbResult.append('<th><div class="list_gridCell_narrow"><input class="list_left_cbInput" type="checkbox" id="DW_CheckAll_'+ this.tableId +'" name="CheckAll_'+ this.tableId +'" onclick="TableBuilder.SelAll(\''+ this.tableId +'\',this)"></div></th>');
	//输出行号标题：空格
	sbResult.append('<th><div class="list_gridCell_narrow">&nbsp;</div></th>');
	//根据冻结的列数，输出标题
	for(var i=0;i<this.lockCols;i++){
		sbResult.append('<th');
		...
		sbResult.append('><div id="Header_'+ this.tableId + "_" + i +'" style="'+this.getDefaultWidth(i)+'" class="list_gridCell_lock">'+ this.heads[i] +' '+ this.getFilterHtml(i) +'</div></th>');
	}
	sbResult.append('</thead>');
	sbResult.append('</table>');
	sbResult.append('</div>');
	//创建冻结数据列
	sbResult.append('<div style="height:'+ this.floatHeight +'px;overflow-x:scroll;overflow-y:hidden" id="'+ this.tableId +'_static" onmousewheel="document.getElementById(\''+ this.tableId +'_cells\').scrollTop  = document.getElementById(\''+ this.tableId +'_cells\').scrollTop-event.wheelDelta;">');
	//sbResult.append('<div style="overflow-x:auto;overflow-y:hidden" id="'+ this.tableId +'_static" onmousewheel="document.getElementById(\''+ this.tableId +'_cells\').scrollTop  = document.getElementById(\''+ this.tableId +'_cells\').scrollTop-event.wheelDelta;">');
	
	sbResult.append('<table style="width:100%"  border="0" cellspacing="0" cellpadding="0">');
	sbResult.append('<tbody id="'+ this.tableId +'_order_GridBody_Locks">');
	//alert(this.depth);
	for(var i=0;i<tableDatas[this.tableId].length;i++){
		//alert(this.depth[i]);
		sbResult.append('<tr tableId="'+ this.tableId +'" id="TR_Left_'+ this.tableId +'_'+ i +'" onMouseOver="TableBuilder.TRMouseOver('+i+',this,undefined,\''+ this.colors[3] +'\',event)" onmousedown="TableBuilder.TRClick('+i+',this,undefined,\''+ this.colors[3] +'\',event)" onMouseOut="TableBuilder.TRMouseOut('+i+',this,undefined,\''+ this.colors[3] +'\',event)">');
		//输出checkbox
		if(this.uiparams.multySelect && this.uiparams.multySelect==1)
			sbResult.append('<td class="list_checkbox_td"><div  class="list_gridCell_narrow" ><input class="list_left_cbInput" type="checkbox" name="check_S_'+ this.tableId +'" onclick="TableBuilder.iCurrentRow[\''+ this.tableId +'\'] = '+i+';TableBuilder.displaySelectedRows(\''+ this.tableId +'\')"></div></td>');
		//输出序号列
		var iCurPage = s_c_p[this.tableId.substring(8)];
		var iPageSize = s_p_s[this.tableId.substring(8)];
		sbResult.append('<td class="list_all_no"  onDblClick=""><div class="list_gridCell_narrow" >'+ (i+1 + iCurPage*iPageSize) +'</div></td>');
		//根据冻结的列数，输出其他列:如果有层次结构，那么在第0列体现层次结构
		//alert(this.lockCols);
		for(var j=0;j<this.lockCols;j++){
			if(this.depth)
				sbResult.append('<td '+ this.getIntervalColor(i) +' class="list_all_td" onDblClick="TableBuilder.displayChild(\''+ this.tableId +'\','+i+',\''+ this.depth.toString() +'\',event)"');
			else
				sbResult.append('<td '+ this.getIntervalColor(i) +' class="list_all_td"');
			...
			if(j==0 && this.depth){//体现层次结构
				var k = 0;
				var pre = "";
				while(k<this.depth[i]){
					pre += "　";
					k++;
				}
				//alert("this.depth["+ i +"] = " + this.depth);
				if(k==this.depth[i]){
					if(this.depth[i]<this.depth[i+1]){//树干节点
						//sValue = pre + '<img onclick="TableBuilder.displayChild(\''+ this.tableId +'\','+i+',\''+ this.depth.toString() +'\',event)" id="IMG_'+ this.tableId + '_' + i +'" src="'+ sSkinPath +'/images/dw/folder.gif" counts="0">' + sValue;
						sValue = pre + '<span onclick="TableBuilder.displayChild(\''+ this.tableId +'\','+i+',\''+ this.depth.toString() +'\',event)" id="IMG_'+ this.tableId + '_' + i +'" class="tree_grid_folder_expand" counts="0">&nbsp;&nbsp;</span>' + sValue;
					}
					else//叶子节点
						sValue = pre + '<span class="list_treespan_leaf">&nbsp;&nbsp;</span>' + sValue;
				}
			}
			sbResult.append('<div id="DIV_Data_'+ this.tableId + "_" + i + "_" + j +'" class="list_gridCell_lock" style="'+this.getDefaultWidth(j)+'">'+ sValue +'</div>');
			//alert('DIV_Data_'+ this.tableId + '_' + i + '_' + j +'');
			sbResult.append('</td>');
		}
		sbResult.append('</tr>');
	}
	...
	if(this.clientCount){
		...
		if(this.uiparams.multySelect && this.uiparams.multySelect==1)
			sbResult.append('<td class="list_all_no_2"><div  class="list_gridCell_narrow">&nbsp;</div></td>');
		sbResult.append('<td class="list_all_no"><div  class="list_gridCell_narrow">'+this.clientCount[0]+'</div></td>');
		sbResult = this.getCountInfo(sbResult,0,this.lockCols-1);
		sbResult.append('</tr>');
	}
	//输出总计标题以及左侧数据
	if(this.serverCount){
		...
		if(this.uiparams.multySelect && this.uiparams.multySelect==1)
			sbResult.append('<td class="list_all_no_2"><div  class="list_gridCell_narrow">&nbsp;</div></td>');
		sbResult.append('<td class="list_all_no"><div  class="list_gridCell_narrow">'+this.serverCount[0]+'</div></td>');
		for(var t=0;t<this.lockCols;t++){
			...
			sbResult.append('<td class="list_all_td"><div class="list_gridCell_standard list_div_totalcount" style="'+ this.getDefaultWidth(t) +';'+TableBuilder.getAlign(this.tableId.substring(8),TableFactory.getDZColIndexFromTalbe(this.tableId,t))+';">'+ TableBuilder.getFomatedNumber(sValue,sCheckFormat) +'</div></td>');
		}
		sbResult.append('</tr>');
		//alert(sbResult.toString());
	}
	sbResult.append('</tbody>');
	sbResult.append('</table>');
	sbResult.append('</div>');
	sbResult.append('</td>');
	sbResult.append('<td id="Right_TD_'+ this.tableId +'" valign="top" width="100%">');
	//创建活动栏目标题
	//sbResult.append('<div style="width:100%; height:100%;overflow-y:auto;overflow-x:hidden" id="'+ this.tableId +'_float" class="list_topdiv_header"');
	sbResult.append('<div style=" width:100%;overflow-y:auto; overflow-x:hidden" id="'+ this.tableId +'_float" class="list_topdiv_header"');
	//添加标题拖拉事件
	sbResult.append(' onmousedown="TableBuilder.header_onmousedown(this,event)" onmouseup="TableBuilder.header_onmouseup(this,event)" onmousemove="TableBuilder.header_onmousemove(this,event)"');
	sbResult.append('>');
	sbResult.append('<table border="0" cellspacing="0" cellpadding="0">');
	//输出合并标题
	if(this.combineRight){
		sbResult = this.getCombineHtml(sbResult,this.combineRight);
	}
	
	sbResult.append('<thead id="TH_Right_'+ this.tableId +'">');
	//输出标题
	for(var i=this.lockCols;i<this.heads.length;i++){
		sbResult.append('<th');
		if(this.htmlStyle && this.htmlStyle[i] && /[\{'"]width\:[0-9]+px[\}'"]/ig.test(this.htmlStyle[i])){
			//alert(this.htmlStyle[i].match(/\{width\:[0-9]+px\}/));
			var search = this.htmlStyle[i].match(/[\{'"]width\:[0-9]+px[\}'"]/);
	...
	sbResult.append('</div>');
	//创建活动数据列
	//sbResult.append('<div id="'+this.tableId+'_cells" style="width:100%; height:'+ this.floatHeight +'px;overflow-x:auto;overflow-y:auto;" onscroll="document.getElementById(\''+ this.tableId +'_static\').scrollTop=this.scrollTop;document.getElementById(\''+ this.tableId +'_float\').scrollLeft=this.scrollLeft">');
	sbResult.append('<div id="'+this.tableId+'_cells" style="  width:100%;height:'+ this.floatHeight +'px;overflow-x:scroll;overflow-y:scroll;" onscroll="document.getElementById(\''+ this.tableId +'_static\').scrollTop=this.scrollTop;document.getElementById(\''+ this.tableId +'_float\').scrollLeft=this.scrollLeft">');
	//sbResult.append('<div id="'+this.tableId+'_cells" style="  width:100%;overflow-x:auto;overflow-y:auto;" onscroll="document.getElementById(\''+ this.tableId +'_static\').scrollTop=this.scrollTop;document.getElementById(\''+ this.tableId +'_float\').scrollLeft=this.scrollLeft">');
	...
	for(var i=0;i<tableDatas[this.tableId].length;i++){
		...
		for(var j=this.lockCols;j<tableDatas[this.tableId][i].length;j++){
			//对齐方式
			var sAlign = "";
			if(this.colAlign && this.colAlign[j])
			...
			if(this.uiparams && this.uiparams.colReadOnly && this.uiparams.colReadOnly[j]=="0"){
				...
				sValue = TableBuilder.convertJS2HTML(tableDatas[this.tableId][i][j],sCheckFormat);
				sValue = '<span id="INPUT_'+ this.tableId + "_" + this.uiparams.colNames[j] + "_" + i+ "_" + j +'" class="list_event_width">'+ this.getTrueValue(j,sValue) + "</span>" + sUnit;
			}
			if(this.htmlStyle && this.htmlStyle[j])
...
TableBuilder.TRMouseOut = function(index,objleft,objright,visitedColor,evt){
	...
	if(objright){//选中右边的情况
		...
		//获得冻结区域对应的行对象
		objLeft = document.getElementById("TR_Left_" + objright.getAttribute("tableId") + "_" + index);
		for(var i=1;i<objLeft.childNodes.length;i++){
			if(objLeft.childNodes[i].getAttribute("ignore_select_color")!="1")
				objLeft.childNodes[i].style.backgroundColor = showColor;
		}
	}
};
//表格鼠标划过效果
TableBuilder.TRMouseOver = function(index,objleft,objright,visitedColor,evt){
	...
	if(objright){//选中右边的情况
		...
		for(var i=1;i<objLeft.childNodes.length;i++){
			if(objLeft.childNodes[i].getAttribute("ignore_select_color")!="1")
				objLeft.childNodes[i].style.backgroundColor = objright.getAttribute("lightColor");
		}
	}
};
...
TableBuilder.displaySelectedRowsColor= function(tableId,dataSize,visitedColor){
	...
	for(var i=0;i<dataSize;i++){
		...
		//获得冻结区域对应的行对象
		var ol = document.getElementById("TR_Left_" + tableId + "_" + i);
		for(var j=1;j<ol.childNodes.length;j++){
			if(ol.childNodes[i].getAttribute("ignore_select_color")!="1")
				ol.childNodes[j].style.backgroundColor = showColor;
		}
	}
};
//给当前点中行赋颜色
TableBuilder.displayCurrentRowColor = function(tableId,dataSize,visitedColor){
	for(var i=0;i<dataSize;i++){
		...
		var ol = document.getElementById("TR_Left_" + tableId + "_" + i);
		for(var j=1;j<ol.childNodes.length;j++){
			if(ol.childNodes[j].getAttribute("ignore_select_color")!="1")
				ol.childNodes[j].style.backgroundColor = showColor;
		}
	}
};
...
TableBuilder.TRClick = function(index,objleft,objright,visitedColor,evt){
	...
	mySelectRow();
	/*
	if(evt.ctrlKey){//按住ctrl键的情况
		var iL = TableBuilder.isStrInArray(selectedRows[tableId],index);
		if(iL>-1){//如果index 已经存在于数组中则要从数组中排除
			for(var i=iL;i<selectedRows[tableId].length-1;i++){
				selectedRows[tableId][i] = selectedRows[tableId][i+1];
			}
			selectedRows[tableId].length = selectedRows[tableId].length-1;
		}
		else
			selectedRows[tableId][selectedRows[tableId].length] = index;
	}
	else if(evt.shiftKey){//按住shift键的情况
		var firstIndex = selectedRows[tableId][0];
		var lastIndex = index;
		selectedRows[tableId] = [];
		for(var i=firstIndex;i<=lastIndex;i++){
			selectedRows[tableId][i-firstIndex] = i;
		}
	}
	else{
		selectedRows[tableId] = [];
...
TableBuilder.showFilterDialog = function(obj,tableId,index,range,evt){
	...
	div.colIndex = index;
	div.scrollTop = 0;//防止因滚动造成的显示不全的问题
	if(range!=""){//有取值范围的
		//首先增加滚动条
