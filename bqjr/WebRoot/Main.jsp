...
<%
	...
	//取门店选择的参数,如果首次登陆的话没有值   add by awang
	String sStoreChosen=DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("StoreChosen"));
	if(sStoreChosen==null) sStoreChosen="";
...
// 判定该销售是否已经关联门店
ASResultSet rs = Sqlca.getResultSet(new SqlObject("select sno,sname from store_info where identtype='01' and status='05' and sno in (Select sno from STORERELATIVESALESMAN where (Stype is null or Stype='' ) and SalesManNo='"+CurUser.getUserID()+"')"));
//判定该销售是否已经关联展厅
...
//消费金融、门店 
//CCS-541 //end by jshu 20150310      1626  1628  --add by yzhang9   CCS-571 
if (CurUser.hasRole(new String[]{"1006","1620","1622","1626","1628","1624"})) { %>
...
if("false"==str){
	...
	<%if("".equals(sStoreChosen)){%>
	var sRetVal = AsControl.PopView("/AppMain/ModifySotres.jsp", "identtype=01", "dialogWidth=800px;dialogHeight=250px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
...
if("false"==st){
	...
	var sRetVal = AsControl.PopView("/AppMain/ModifySotres.jsp", "identtype=02", "dialogWidth=800px;dialogHeight=250px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
	if(typeof(sRetVal)=='undefined' || sRetVal.length==0){
		alert("请重新登录并选择关联展厅！");
		window.open("index.html","_top");
	}
...
<script type="text/javascript">
	//add by ybpan at 20150323 --CCS-332   PRM-74 合同出现非关键错误和关键错误时，质量标注在销售ID界面显示
	//查询是否存在关键错误的合同 
	if(<%=CurUser.hasRole("1005")%>){
		var sIsPrimaryError = RunJavaMethodSqlca("com.amarsoft.app.billions.PrimaryErrorContract","checkSaleMangerQuality","userID="+'<%=CurUser.getUserID()%>'); 
		if("true"==sIsPrimaryError){
			//打开关键错误合同列表
			PopPage("/DeskTop/PrimaryListForSM.jsp","","dialogWidth=1000px;dialogHeight=500px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
		}
	}else if(<%=CurUser.hasRole(new String[]{"1006","1620","1622","1626","1628","1624"})%>){
		var sIsPrimaryError = RunJavaMethodSqlca("com.amarsoft.app.billions.PrimaryErrorContract","checkHavedError","userID="+'<%=CurUser.getUserID()%>');
		if("true"==sIsPrimaryError){
			//打开关键错误合同列表
			PopPage("/DeskTop/PrimaryList.jsp","","dialogWidth=1000px;dialogHeight=500px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
		}
	}
	
	//add by ybpan at 20150323 --CCS-332
</script>
