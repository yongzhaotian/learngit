...
<%!
    String flag = "Counts";//判断所显示的图形是关于数值型还是百分比型，默认为数值型
	CategoryDataset createDataset(String itemStr, String orgStr, String valueStr) {
        // row keys...
        ...
        for (int i1 = 0; i1 < items.length; i1 ++) {
        	//for (int i2 = orgs.length - 1; i2 >= 0; i2 --) {
        	for (int i2 = 0; i2 < orgs.length; i2 ++) {
        		double d = 0; //确省值用0代替
        		try {
        			d = Double.parseDouble(values[i1*orgs.length+i2]);
        		}
        		catch (Exception e) {
        		}
        		dataset.addValue(d,items[i1],orgs[i2]);
        	}
        }
        return dataset;
    }
    ...
    JFreeChart createBarChart(CategoryDataset dataset) {
        ...
        if(flag == "Counts"){
            ...
            vName = "百分比(补登完成或未完成笔数  / 总补登笔数)";
        }
        JFreeChart chart = ChartFactory.createBarChart3D(
                "补登情况统计",       // chart title
                "",               // domain axis label
                vName,    // range axis label
                dataset,                  // data
