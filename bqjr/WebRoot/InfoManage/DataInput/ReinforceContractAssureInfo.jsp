...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info00;Describe=注释区;]~*/%>
	...
	/*
		...
		Tester:
		Describe: 业务合同补登新增的担保合同详情（一个保证合同对应一个保证人）;
		Input Param:
			ObjectNo：对象编号（合同流水号）
			SerialNo：担保合同编号			
			GuarantyType：担保方式
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function goBack()
	{
		OpenPage("/InfoManage/DataInput/ReinforceContractAssureList.jsp","_self","");
	}
		...
		if(sGuarantyType == '050' || sGuarantyType == '060' || sGuarantyType.substring(0,3) == '010')
		{
			...
			if(typeof(sLoanCardNo) != "undefined" && sLoanCardNo != "" )
			{
				...
				if(typeof(sReturn) != "undefined" && sReturn != "" ){
					...
					sReturn = sItems[1]; //贷款卡号
					if(sReturn == "") {
						alert("该保证人基本信息不全，不能作为该业务的保证人！");
						return false;
					}
					if(sReturn != sLoanCardNo){
						alert("所填贷款卡号并非该客户的贷款卡!");//所填贷款卡并非该客户的贷款卡!		
						return false;
					}
				}
