...
<%/*~END~*/%>
 ...
 <%
	/*
		页面说明: 示例列表页面
	 */
	String PG_TITLE = "合同档案登记 ";
	 String sTempletNo="";
	//通过DW模型产生ASDataObject对象doTemp
	if(sTemp.equals("1")){
		 sTempletNo = "ContractRegistration";
	}else{
	    sTempletNo = "EndContractRegistration";
	}
	...
	String sButtons[][] = {
			{"false","","Button","信息补登","信息补登","informationBoard()",sResourcesPath},
			{"true","","Button","加入合同档案登记","加入合同档案登记","contractRegistration()",sResourcesPath}
	};
...
//			return;
//		}
		if(confirm("您真的确定加入合同档案登记吗？")){
			RunMethod("ModifyNumber","GetModifyNumber","business_contract,contractRegistration='01',serialNo="+sSerialNo);// 修改合同档案登记状态 
			RunMethod("ModifyNumber","GetModifyNumber","business_contract,boxID=<%=sBoxID%>,serialNo="+sSerialNo);    //修改合同存放的位置 
			as_save("myiframe0");  
