...
<%	
	ASDataObject doTemp = new ASDataObject("BoxInfo",Sqlca);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function saveRecord()
	{
		...
		RunMethod("ModifyNumber","GetModifyNumber","archives_Warehouse,boxNumber='"+sSCount+"',cabinetID='"+sCabinetID+"'"); //修改该柜子中已存在多少箱子
	}

    /*~[Describe=保存;InputParam=后续事件;OutPutParam=无;]~*/
	function saveRecordAndBack()
	{
		OpenPage("/AppConfig/Document/BoxList.jsp","_self","");
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function initRow(){
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"trueFalseFile", "02");
