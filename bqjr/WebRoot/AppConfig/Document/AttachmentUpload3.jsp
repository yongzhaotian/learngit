...
<%@ page import="com.amarsoft.app.als.image.ImageUtil" %>
      	...
      	if (!myAmarsoftUpload.getFiles().getFile(0).isMissing()){
     		try {
     			...
     			if("1".equals(uploadPeriod)){
     				sObjectType = "BusinessLoan";
     			}else {
     				sObjectType = "Business";
     			}
     			sNewSql = "insert into ECM_PAGE(ObjectType,ObjectNo,TypeNo,DocumentId,ModifyTime,ImageInfo,OperateUser,OperateOrg,PageNum) values(:ObjectType,:ObjectNo,:TypeNo,:DocumentId,:ModifyTime,:ImageInfo,:OperateUser,:OperateOrg,:PageNum)";
            	so = new SqlObject(sNewSql).setParameter("ObjectType",sObjectType).setParameter("ObjectNo",sDocNo).setParameter("TypeNo", sTypeNo)
            				.setParameter("ModifyTime", DateX.format(new java.util.Date(),"yyyy/MM/dd hh:mm:ss")).setParameter("ImageInfo", Sqlca.getString(new SqlObject("Select TypeName from Ecm_Image_Type where TypeNo=:TypeNo").setParameter("TypeNo", sTypeNo)))
            				.setParameter("OperateUser", CurUser.getUserID()).setParameter("OperateOrg", CurOrg.orgID).setParameter("PageNum", "2");
        		...
        		String sFileSaveMode = CurConfig.getConfigure("FileSaveMode");
             	if(sFileSaveMode.equals("Disk")){ //存放文件服务器中
					...
					//CCS-1208 add by zty 
					if("20002".equals(sTypeNo)){//客户现场照片
						ImageUtil.isPhotoProcessing(sFullPath,Sqlca,sTempNo);
					}
					//add by zty end
            	...
            	//操作上传状态start
            	String uploadSQl = "select a.IMAGE_TYPE_NO  from PRODUCT_ECM_UPLOAD a where "+
                    	"a.PRODUCT_TYPE_ID in (select p.PRODUCT_TYPE_ID from PRODUCT_TYPE_CTYPE p where p.PRODUCT_ID = (select t.businesstype from business_contract t where t.serialno = '"+docno+"'))";
                	rs=Sqlca.getASResultSet(uploadSQl);
                	if(rs.next()){
    	            	String flag = "1";
    	            	String updateSql = "update business_contract t set t.uploadFlag='"+flag+"',"+"t.checktime='"+sToday+"',"+"t.checkstatus='2',"+
...
%>          
	          <script type="text/javascript">
	        	...
	        	AsControl.OpenView("/ImageManage/ImageViewNewFrame.jsp","DocNo="+<%=docno%>+"&TypeNo="+<%=sTypeNo%>,"right");
