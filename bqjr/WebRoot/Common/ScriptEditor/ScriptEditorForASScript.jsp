<%@ page contentType="text/html; charset=GBK"%><%@ include file="/IncludeBeginMD.jsp"%><%
	...
	while(rs.next()){
	    ...
	    sSql2 = "select distinct MethodName,'<font color=red>'||ReturnType ||'</font>&nbsp;&nbsp;!'||ClassName ||'.'||MethodName||'('|| MethodArgs ||')',MethodDescribe,MethodCode from CLASS_METHOD "+
	            "where ClassName=:ClassName order by MethodName";
            ...
            <table>
                <tr>
                    ...
                    <td id="showTreeButtonTD">
                        <%=ASWebInterface.generateControl(SqlcaRepository,CurComp,sServletURL,"","Button","显示方法树","显示方法树","showTree()",sResourcesPath)%>
                    </td>
                </tr>
            </table>
	    </td>
