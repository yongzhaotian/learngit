		...
		<td colspan=4>
			<table width="100%">
				<tr>
					...
					<%=HTMLControls.generateButton("&nbsp;取&nbsp;消&nbsp;","取消","javascript:self.returnValue='';self.close();",sResourcesPath)%></td>
				</tr>
			</table>
		</td>
	</tr>
...
<script type="text/javascript">
	...
	function doConfig(){
   		...
   		var node = getCurTVItem(); //树图标识的节点
   		//alert(typeof(node)+"|"+node.id+"|"+node.name+"|");
   		if(node.id == "root"){
   			alert("请选择要选择的城市！");
   			return;
   		} 
   		...
   		if (node.id.length==2 && !inArray(node.id, arryInclude)) {
   			alert("请选择省份下面的城市！");
   			return;
   		};
   		self.returnValue=node.id+"@"+node.name;
	    top.close();
	}
	...
	<%
		...
		tviTemp.MultiSelect = false;
		tviTemp.initWithSql("ItemNo","ItemName","SortNo","","from code_library where codeno='AreaCode1' and isinuse='1' and (length(SortNo)!=6 and substr(SortNo,1,2) not in ('11','12','15','31','50') or SortNo in ('11','12','15','31','50'))",Sqlca);
