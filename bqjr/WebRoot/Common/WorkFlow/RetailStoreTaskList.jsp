...
<script type="text/javascript">
	...
	function doSubmit(){
		...
		// 检查是否申请下的所有门店意见意见都签署了
		var dStoreCnt = RunMethod("公用方法", "GetCountByWhereClause", "Store_Info,SerialNo,RSSerialNo='"+sObjectNo+"'");
		var sSignRetVal = RunJavaMethodSqlca("com.amarsoft.app.billions.RetailStoreCommon", "signAllStoreOpinion", "objectNo="+sObjectNo);
		if (dStoreCnt>0.0 && sSignRetVal.length>0) {
			alert("以下门店未签署意见,门店编号,门店名称:\n"+sSignRetVal);
			return;
		}
		...
		var sPhaseInfo = PopPage("/Common/WorkFlow/SubmitDialog_RetailStore.jsp?SerialNo="+sSerialNo+"&objectType="+sObjectType+"&objectNo="+sObjectNo+"&oldPhaseNo="+sPhaseNo,"","dialogWidth=580px;dialogHeight=420px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;");
		if(typeof(sPhaseInfo)=="undefined" || sPhaseInfo=="" || sPhaseInfo==null || sPhaseInfo=="null" || sPhaseInfo=="_CANCEL_") return;
		else if (sPhaseInfo == "Success"){
			alert(getHtmlMessage('18'));//提交成功！
			// 提交完成阶段，根据门店通过状态判断大商户准入或者拒绝
			if ( dStoreCnt>0.0 && (getItemValue(0, getRow(), "PERMITTYPE")=="01")) RunJavaMethodSqlca("com.amarsoft.app.billions.RetailStoreCommon", "retailApplyResult", "objectNo="+sObjectNo+",retailNo="+getItemValue(0, getRow(), "RSERIALNO"));
			
			RunJavaMethodSqlca("com.amarsoft.app.billions.GenerateSerialNo", "batchGenerateStoreNo", "serialNo="+sObjectNo);
	...
	function getTask(){
		// 是否该该用户在任务池还有用户
		var sCntRetVal = RunJavaMethodSqlca("com.amarsoft.app.billions.BqjrFlowAction", "getTaskPoolCount", "flowNo="+sFlowNo+",phaseNo="+sPhaseNo+",userId=<%=CurUser.getUserID()%>");

		if ("Failure" ==  sCntRetVal) {
			alert("没有可以获得的任务！");
			return;
		}
		
		var sRetVal = AsControl.PopView("/BusinessManage/ChannelManage/RetailStoreTaskPoolList.jsp", "FlowNo="+sFlowNo+"&PhaseNo="+sPhaseNo);
		//alert(sRetVal+"|"+typeof(sRetVal)+"|"+(typeof(sRetVal)=='undefined'));
		if (typeof(sRetVal)=='undefined' || sRetVal.length==0) {
			return;
		}
		
		var sRetValArray = sRetVal.split("@");
		// 开始获取任务
		RunJavaMethodSqlca("com.amarsoft.app.billions.BqjrFlowAction", "getTask", "serialNo="+sRetValArray[0]+",objectNo="+sRetValArray[1]+",phaseNo="+sRetValArray[2]+",objectType="+sRetValArray[3]);
	...
	function returnToPool(){
		...
		if (typeof(sObjectNo)=='undefined' || sObjectNo.length=="") {
			alert("请选择一条记录！");
			return;
		}
	...
	function signCheckOpinion(){
		...
		var sCompURL = "/Common/WorkFlow/SignTaskOpinionInfoNew.jsp";
	...
	function viewTab(){
        ...
        //alert(getItemValue(0, getRow(), "APPLYTYPE"));
		AsControl.PopView(sUrl, "RSSerialNo="+getItemValue(0, getRow(), "RSSERIALNO")+"&RSerialNo="+getItemValue(0, getRow(), "RSERIALNO")+"&ViewId=001&ApplyType=<%=CommonConstans.ReTAILSTORE_APPROVE_TYPE %>", "");
		reloadSelf();
	}
