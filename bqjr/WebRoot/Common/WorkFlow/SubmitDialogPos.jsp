...
<% 
	...
	String inputOrg=CurOrg.getOrgID();
	...
	System.out.println("inputTime"+inputTime+"inputOrg="+inputOrg+"inputUser="+inputUser+"sSerialNo="+sSerialNo+",oldPhaseNo="+oldPhaseNo+",objectType="+objectType+",objectNo="+objectNo);
		...
		<tr>
			<td>
				<table>
					<tr>
						<td  width="70px"></td>
						<td  width="70px"><p align="left" >备注(限100个汉字)</p></td>
						<td colspan=10 width="500px">
						
							<input type="text" id="remark"  size=8 style="width:80%;height:100px;" <%=sSelectStyle%>  />
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function doActionList(){
		...
		for(i = 0;i <= iLength - 1;i++){
			...
			if (stdItem.selected){
				...
				if(vItem != ""){
		    		if ((	 vItem.search("提交") >= 0
		    				
								|| vItem.search("同意") >= 0
								|| vItem.search("销售运营初审") >= 0
								//vItem.search("退回") < 0
								)
								&& roleSubmit == 0
		    	    	){
		    		...
		    		}else{	//如果为退回一类的意见选择，则不显示动作列表
		    			document.getElementById("selectlist").style.display = "none";
		    			document.getElementById("PhaseAction").value = "";
		    			bActionReqiured = false;	//不需要再选择Action了
		    		}
		    		if(roleSubmit==1){
		    			getNextPaseInfo(3);	//找下一阶段信息
			    	}else{
	...
	function getNextPaseInfo(type){
		...
		if(type != 3 && (phaseOpinion1.length == 0 ||(phaseAction==""  && (phaseOpinion1.substring(0,2)=="提交" || phaseOpinion1.substring(0,2)=="补充" || phaseOpinion1.substring(0,2)=="重新")))){				
			//alert("请先进行提交意见选择 !");
			document.getElementById("nextPaseInfo").innerHTML="";
		}else{
	...
	function commitAction(){
		...
		var ssSerialNo="<%=sSerialNo%>";
		var sObjectNo="<%=objectNo%>";
		var sObjectType="<%=objectType%>";
		var sOldPhaseNo="<%=oldPhaseNo%>";
		var sInputUser="<%=inputUser%>";
		var sInputOrg="<%=inputOrg%>";
		var sInputTime="<%=inputTime%>";
...
//		alert(thisPhaseAction);
//		alert(thisPhaseOpinion1);
		...
		if(havaBackTask=="1")
		{
			alert("该任务的其他分支正在退回补充资料中，请等待补充完全再提交！");
			return;
		}
		...
		if(thisPhaseOpinion1.length == 0){				
			...
			var sRemarks=document.getElementById("remark").value;
	//		alert(sRemarks);
			
	//		alert("SerialNo"+ssSerialNo+","+"ObjectNo"+sObjectNo+","+"PhaseOpinion"+thisPhaseOpinion1+","+"InputOrg"+sInputOrg+","+"InputUser"+sInputUser+","+"InputTime"+sInputTime+","+"Remarks"+sRemarks);
//			alert("SerialNo="+ssSerialNo+","+"ObjectNo="+sObjectNo+","+"PhaseOpinion="+thisPhaseOpinion1+","+"InputOrg="+sInputOrg+","+"InputUser="+sInputUser+","+"InputTime="+sInputTime+","+"Remarks="+sRemarks+","+"PhaseNo="+sOldPhaseNo+","+"ObjectType="+sObjectType);
			if (confirm("该笔业务的"+document.getElementById("nextPaseInfo").innerHTML+"\r\n你确定提交吗？")){
				...
				RunMethod("审批流程","InsertFlowOpinionForPos",ssSerialNo+","+sOpinionNo+","+sObjectNo+","+thisPhaseOpinion1+","+sInputOrg+","+sInputUser+","+sInputTime+","+sRemarks+","+sOldPhaseNo+","+sObjectType);
				if(sOldPhaseNo=="0030"&&thisPhaseOpinion1=="批准"){
					RunMethod("公用方法","UpdateColValue","MOBILEPOS_INFO,Status,05,SerialNo = '"+sObjectNo+"'");
					var MobilePosNo = RunMethod("公用方法","GetColValue","MOBILEPOS_INFO,MobliePosNo,SerialNo = '"+sObjectNo+"'");
					if(MobilePosNo.length==0||MobilePosNo==null||MobilePosNo==""){
					RunJavaMethodSqlca("com.amarsoft.app.billions.UpdateMobilePosNo","UpdatePosNo","SerialNo="+sObjectNo);
					}
				}else if((sOldPhaseNo=="0030"||sOldPhaseNo=="0020")&&thisPhaseOpinion1=="否决"){
					RunMethod("公用方法","UpdateColValue","MOBILEPOS_INFO,Status,04,SerialNo = '"+sObjectNo+"'");
				}
