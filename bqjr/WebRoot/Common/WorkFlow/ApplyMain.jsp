...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Main02;Describe=定义变量，获取参数;]~*/%>
    ...
    if("CashLoanApply".equals(sApplyType) || "CarCashLoanApply".equals(sApplyType)){
        if(sPhaseType == null) sPhaseType = "1010";//CCS-1334 update huzp 二段式提单需求修改默认选中第一个菜单
    }else{
        if("1".equals(switch_status)){
        	if(sPhaseType == null) sPhaseType = "1001";//CCS-1334 update huzp 二段式提单需求修改默认选中第一个菜单
        }else{
            if(sPhaseType == null) sPhaseType = "1010";//CCS-1334 update huzp 二段式提单需求修改默认选中第一个菜单
        }
    }
...
<%/*~BEGIN~可编辑区[Editable=true;CodeAreaID=Main05;Describe=树图事件;]~*/%>
    ...
    function TreeViewOnClick()
    {
        ...
        if(sCurItemDescribe1 != "root")
        {
            ...
            AsControl.OpenComp(sCurItemDescribe1,"ComponentName=授信申请列表&ApplyType=<%=sApplyType%>&PhaseType="+getCurTVItem().id+"&ViewId="+sCurItemDescribe3,"right");
