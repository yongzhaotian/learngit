...
<script type="text/javascript">
	...
	function doSubmit(){
		...
		// 判定是否已签署意见
		var sColVal = RunMethod("公用方法", "GetColValue", "FLOW_OPINION,SerialNo,SerialNo='"+sSerialNo+"'");
		//alert(sColVal+"|"+typeof(sColVal));
		//return;
		if ("Null" == sColVal) {
			alert("请先签署意见再提交！");
			return;
		...
		//判断检查是否全部已完成
		var isSubmit = RunJavaMethodSqlca("com.amarsoft.proj.action.CheckOfApprove","isSubmit","SerialNo="+sSerialNo+"");
		...
		//判断是否需要进行家访
		var returnValue = RunJavaMethodSqlca("com.amarsoft.proj.action.CheckOfApprove","insertHomeInfo","objectNo="+sObjectNo+"");
		if((returnValue)!="undefined" && returnValue=='true'){
			var isHomeCheck = RunJavaMethodSqlca("com.amarsoft.proj.action.CheckOfApprove","isHomeCheck","objectNo="+sObjectNo+"");
			if(typeof(isHomeCheck)!="undefined" && isHomeCheck=="true"){
			}else{
				alert("该笔申请还未完成家访，不能提交");
				return;
			}
		}
		...
		var sPhaseInfo = "";
		if(sFlowNo=="CarFlow" || sFlowNo=="CarFlowNew"){
	...
	function getTask(){
		...
		var sReturn = RunJavaMethodSqlca("com.amarsoft.biz.workflow.action.FlowAction","getTaskCar","flowNo="+sFlowNo+",oldPhaseNo="+sPhaseNo+",userID="+sUserID);
		if(sReturn == "Success"){
		//刷新件数及页面
		OpenComp("ApproveMain","/Common/WorkFlow/ApproveMain.jsp","ComponentName=授信业务审批&ComponentType=MainWindow&ApproveType=<%=sApproveType%>","_top","");
		alert("获取任务成功！");
...
// 		alert(sObjectNo+","+sObjectType);
	...
	function viewOpinions(){
		...
		popComp("ViewApplyFlowOpinions","/Common/WorkFlow/CarViewApproveFlowOpinions.jsp","FlowNo="+sFlowNo+"&PhaseNo="+sPhaseNo+"&ObjectType="+sObjectType+"&ObjectNo="+sObjectNo,"");
	}
