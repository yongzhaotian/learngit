...
<%
	...
	sSql = 	" select FO.CustomerName,getItemName('Currency',FO.BusinessCurrency) as BusinessCurrencyName, "+
			...
			" FT.BeginTime,FT.EndTime,FT.PhaseChoice,FO.PhaseOpinion,FO.PhaseOpinion1,FO.PhaseOpinion2,FO.PhaseOpinion3,FO.PhaseOpinion4, "+
			" FM.Attribute3 as OpinionRightType,FM.Attribute4 as OpinionRightPhases,FM.Attribute5 as OpinionRightRoles,FO.opinion_remark "+
  ...
  <table width="100%"  height="100%" cellpadding="3" cellspacing="0" border="0" style="overflow:auto;overflow-x:visible;overflow-y:visible">
    ...
    <%
		while (rs.next()){
			...
			//修改为参照SignTaskImageInfo.jsp的写法,add by daihuafeng,20150707--begin
			...
			//修改为参照SignTaskImageInfo.jsp的写法,add by daihuafeng,20150707--end
			
			// 家庭成员外呼核查   意见选项
			//String sOpSql = "select ItemName from Code_library where codeno = (select Coleditsource from dataobject_library where dono = '"+sDoNo+"' and Colheader = '意见详情') and ItemNo = '"+rs.getString("PhaseOpinion")+"'";
			/* if ("0080".equals(sPhaseNo1) && "WF_HARD".equals(sFlowNo1) && "HomePhoneInfoOpinionInfo".equalsIgnoreCase(sDoNo)) {
				sOpSql = "select ITEMNAME from CODE_LIBRARY where CODENO = 'FamilyMemberPhoneInfoCheck' and itemno='"+rs.getString("PhaseOpinion")+"'";
			} */
			//String sOpinon = Sqlca.getString(sOpSql);
        ...
        <tr id=<%=iRow++%>>
            <td  colspan=2 align=center>
                <textarea type=textfield  bgcolor="#FDFDF3" readonly style='width:100%;height:100px;resize: none;'>
                     ...
                     %>
                     <%
                     	String FlowNo =DataConvert.toString(rs.getString("FlowNo")).trim();
                     	String PhaseNo=DataConvert.toString(rs.getString("PhaseNo")).trim();
                     	boolean flag = false;
                     	String p_remak = DataConvert.toString(rs.getString("opinion_remark")).trim();
                     	if(FlowNo.equals("WF_MEDIUM") && PhaseNo.equals("0170")){
                     		flag= true;
                     	} 
                     	if(flag){ %>
                     		<%="\r\n【备注】"%>
                     	<%}else{ %>
                     		<%="\r\n【备注】"+StringFunction.replace(DataConvert.toString(rs.getString("opinion_remark")).trim(),"\\r\\n","\r\n")%>
                     	<%}
                     %>
                </textarea>
            <input type=hidden value='<%="\r\n【意见】"+StringFunction.replace(DataConvert.toString(rs.getString("PhaseOpinion")).trim(),"\\r\\n","\r\n")%>' name=<%="R"+String.valueOf(iRow)+"L"%>>
            <input type=hidden value='<%="\r\n【备注】"+p_remak%>' name=<%="R"+String.valueOf(iRow)+"L"%>>
