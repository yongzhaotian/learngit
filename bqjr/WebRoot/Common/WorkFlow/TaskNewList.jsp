...
<%
	...
	System.out.println("=======================111111111111111111=============="+sFlowNo);
	//将空值转化成空字符串
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List03;Describe=定义数据对象;]~*/%>
	...
	if(sFinishFlag.equals("N")) //当前工作
		sButton = Sqlca.getString("select Attribute1 from FLOW_MODEL where FlowNo = 'WF_MEDIUM' and PhaseNo = '1000'");
	System.out.println(sButton);
	if(sFinishFlag.equals("Y")) //已完成工作
		sButton = Sqlca.getString("select Attribute2 from FLOW_MODEL where FlowNo = 'WF_MEDIUM' and PhaseNo = '1000'");
	//将空值转化成空字符串
	...
	sWhereClause1 = StringFunction.replace(sWhereClause1,"#FlowNo","FlowNo in "+sFlowNo+" ");
	...
	//TODO 暂在这里添加flowNo
	doTemp.WhereClause += "and flowNo in "+ sFlowNo + ""; 
	...
	//生成HTMLDataWindow
/***==开始====================quliangmao==========优化设置两秒只能不能请求两次**/
	String  SPhase_Time= (String)session.getAttribute("SPhase_Time");
	String SPhase="N";
	Vector vTemp=null;
	long  strTime = new java.util.Date().getTime();  
	if(SPhase_Time != null ){
		if((strTime - Long.parseLong(SPhase_Time) ) < 2000){
			SPhase="Y";
		}else{
			session.setAttribute("SPhase_Time",strTime+"");
		}
	}else{
		    session.setAttribute("SPhase_Time",strTime+"");
	}
	if("Y".equals(SPhase)){
		vTemp = (Vector)session.getAttribute("SPhase_DataWindow");
	}else{
	 vTemp = dwTemp.genHTMLDataWindow("'','"+sPhaseType+"','"+CurUser.getUserID()+"'");
	 session.setAttribute("SPhase_DataWindow",vTemp);
	}
	/**************优化结束*********************************************/
