...
<%!
	...
	String getBranchOrgID(String sOrgID,Transaction Sqlca) throws Exception {
		...
		int sLevel = Integer.parseInt(Sqlca.getString(new SqlObject("select OrgLevel from Org_Info where OrgID=:OrgID").setParameter("OrgID",sOrgID)));
		while (sLevel > 3) {
			sUpperOrgID = Sqlca.getString(new SqlObject("select RelativeOrgID from Org_Info where OrgID=:OrgID").setParameter("OrgID",sOrgID));
			if (sUpperOrgID == null) break;
			sOrgID = sUpperOrgID;
			sLevel = Integer.parseInt(Sqlca.getString(new SqlObject("select OrgLevel from Org_Info where OrgID=:OrgID").setParameter("OrgID",sOrgID)));
		}
		return sOrgID;
	}
%>
...
<%
	...
	sSql =  " select DirName,DirID" +
			" from FORMATDOC_DEF "+
			" where DocID =:DocID "+
			" and DirID not in ("+sDirID+") "+
			" and JSPFileName <> 'NULL' ";
