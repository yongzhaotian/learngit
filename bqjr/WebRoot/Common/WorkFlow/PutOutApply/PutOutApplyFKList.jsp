...
<script type="text/javascript">
	...
	function newApply(){
		...
		//弹出新增申请参数对话框
		var sReturn= setObjectValue("SelectPutOutInfo","","",0,0,"");
	    ...
	    sObjectNo=sReturn[0];  
		RunMethod("WorkFlowEngine","InitializeFlow",sObjectType+","+sObjectNo +","+sApplyType+","+sInitFlowNo+","+sInitPhaseNo+",<%=CurUser.getUserID() %>,<%=CurOrg.getOrgID() %>");
		RunMethod("ModifyNumber","GetModifyNumber","business_contract,ContractStatus='180',serialNo='"+sObjectNo+"'");//修改合同状态为带放款
		window.close();
        reloadSelf();
	}
...
//			sPhaseInfo = RunJavaMethodSqlca("com.amarsoft.biz.workflow.action.FlowAction","AutoCommitWithRule","SerialNo="+sTaskNo+",objectType="+sObjectType+",objectNo="+sObjectNo+",oldPhaseNo="+sPhaseNo+",userID="+sUserID);
	...
	function checkFile(){
		...
		AsControl.OpenView("/Common/WorkFlow/PutOutApply/PutOutImageList.jsp","ObjectNo="+sObjectNo,"_blank","");		
	}
	</script>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function initRow(){
		...
		var sInitPhaseNo = "<%=sInitPhaseNo%>";
		//弹出新增申请参数对话框
//		var  sReturn=RunMethod("InsertFlowEngine", "GetInitializeFlow",sObjectType+","+sApplyType+","+sInitFlowNo+","+sInitPhaseNo+",<%=CurUser.getUserID()%>,<%=CurOrg.getOrgID()%>" );
	}
	</script>
<%/*~END~*/%>
