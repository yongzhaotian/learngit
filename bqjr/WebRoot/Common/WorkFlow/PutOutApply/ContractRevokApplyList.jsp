...
<script type="text/javascript">
	...
	function newApply(){
		...
		//if(sApplyType == "CreditLineApply"){
			sCompID = "CreditContractApplyInfo";
			sCompURL = "/Common/WorkFlow/PutOutApply/CreditContractApplyInfo.jsp";		
...
//			sPhaseInfo = RunJavaMethodSqlca("com.amarsoft.biz.workflow.action.FlowAction","AutoCommitWithRule","SerialNo="+sTaskNo+",objectType="+sObjectType+",objectNo="+sObjectNo+",oldPhaseNo="+sPhaseNo+",userID="+sUserID);
	...
	/*~[Describe=创建新合同 ;InputParam=无;OutPutParam=无;]~*/
	function newContract(){
		...
		if(confirm("您真的确定创建新合同吗？")){
//			sReturn=RunMethod("InsertContractInfo", "GetInsertContractInfo",sObjectNo+","+sObjectType);
			sReturn = RunMethod("WorkFlowEngine","CopyApplyFlow",sObjectType+","+sObjectNo);   //复制合同
			RunMethod("BusinessManage","UpdateApplyPhaseType",sReturn+","+sObjectType+","+"1060");//修改阶段类型
			if(typeof(sReturn)!="undefined" && sReturn.length!=0){
				alert("创建新合同成功");
				reloadSelf();
			}
	...
	/*~[Describe=提交到放款;InputParam=无;OutPutParam=无;]~*/
	function doSubPutOut(){
		var sObjectNo = getItemValue(0,getRow(),"ObjectNo");
		var cObjectType = getItemValue(0,getRow(),"ObjectType");
		var sFlowNo ="PutOutFlow";
		var sApplyType="PutOutApply";
		var sObjectType ="PutOutApply";
		if (typeof(sObjectNo)=="undefined" || sObjectNo.length==0){
			alert(getHtmlMessage('1'));//请选择一条信息！
			return;
		}
		if(confirm("您真的确定创建新合同吗？")){
			RunMethod("WorkFlowEngine","InitializeFlow",sObjectType+","+sObjectNo +","+sApplyType+","+sFlowNo+",0010,<%=CurUser.getUserID() %>,<%=CurOrg.getOrgID() %>");
			RunMethod("ModifyNumber","GetModifyNumber","business_contract,ContractStatus='180',serialNo='"+sObjectNo+"'");//修改合同状态
			RunMethod("BusinessManage","UpdateApplyPhaseType",sObjectNo+","+cObjectType+","+"1070");//
		}
		reloadSelf();
	}
