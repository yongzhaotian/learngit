...
//  	    }
    ...
    String sTempletNo="";
	sTempletNo="DownloadReportList1";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function initRow(){
		...
		for(var iMSR = 0; iMSR < getRowCount(0) ; iMSR++){
			var sSerial = getItemValue(0,iMSR,"SerialNo");
			//alert(sSerial);
			//根据合同号查询record_data里面的记录
			var returnVal=RunMethod("BusinessManage","SelectRecordData",sSerial);
			//var returnVall =returnVal.split("@");
			//alert("1");
			var SerialNoqg=getItemValue(0,iMSR,"SerialNoqg");
			//alert(returnVall.length);
			//alert(returnVal.indexOf(SerialNoqg));
			if(returnVal.indexOf(SerialNoqg)==-1){
				setItemValue(0,iMSR,"IsFinally","历史");
			}else{
				setItemValue(0,iMSR,"IsFinally","最终");
			}
