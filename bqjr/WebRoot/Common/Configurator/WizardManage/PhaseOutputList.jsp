...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "组件输出参数列表"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%	
	...
	sSql = "select "+
		   ...
		   "ParameterID,"+
		   "ParameterName "+
		  "from WZD_PHASE_OUTPUT Where WizardNo = '"+sWizardNo+"' and PhaseNo = '"+sPhaseNo+"' order by WizardNo,PhaseNo,ParameterID";

	ASDataObject doTemp = new ASDataObject(sSql);
	doTemp.UpdateTable = "WZD_PHASE_OUTPUT";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function newRecord()
	{
        sReturn=popComp("PhaseOutputInfo","/Common/Configurator/WizardManage/PhaseOutputInfo.jsp","WizardNo=<%=sWizardNo%>~PhaseNo=<%=sPhaseNo%>~IsNew=Y","");
        reloadSelf();           
	}
	...
	function viewAndEdit()
	{
        ...
        sReturn=popComp("PhaseOutputInfo","/Common/Configurator/WizardManage/PhaseOutputInfo.jsp","WizardNo="+sWizardNo+"~PhaseNo="+sPhaseNo+"~ParameterID="+sParameterID+"~IsNew=N","");
	}
