...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "新增账户信息"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	sTempletNo = "CarFactoryDetailsLisInfo";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord()
	{
		...
		var sCount=RunMethod("Unique","uniques","account_information,count(1),relativeSerialNo='<%=sSerialNo %>' and accountType='" +sAccountType+"'");
		bIsInsert = false;
		if(sCount!="0.0"){
			alert("列表中已存在放款帐号或收款账号 ！");
			return;
		}
	    insertaccount();
		as_save("myiframe0");
	...
	function insertaccount(){
		//插入还款、放款账户
		var SerialNo = getSerialNo("ACCT_DEPOSIT_ACCOUNTS","SerialNo","");
		var sAccountNo =getItemValue(0,getRow(),"accountNo");//账号
		var sAccountType =getItemValue(0,getRow(),"accountType");//账户类型
		var sAccountName =getItemValue(0,getRow(),"dealerName");//账户名称
		var sAccountIdicator = "";
		if(sAccountType=="01"){
			sAccountIdicator="00";
		}else if(sAccountType=="02"){
			sAccountIdicator="01";
		}
		sReturn = RunMethod("PublicMethod","CreateDistinctAccount",SerialNo+","+sAccountNo+","+sAccountIdicator+","+sAccountName);
	}
    
	 /*~[Describe=保存;InputParam=后续事件;OutPutParam=无;]~*/
	function saveRecordAndBack()
	{
		OpenPage("/CustomerManage/CooperationCustomerManage/CarFactoryDetailsList.jsp","_self","");
	}
	...
	function selectBankCode(){
		...
		if(typeof(sBranch)=="undefined" || sBranch=="" || sBranch=="_CANCEL_" || sBranch=="undefined") return;
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function initRow(){
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"serialNo", getSerialNo("account_Information", "serialNo", " "));
