...
<%
	...
	String sRefVersionSeq = groupcustomer.getRefVersionSeq(); //正在维护的集团家谱版本编号
	String sCurrentVersionSeq = groupcustomer.getCurrentVersionSeq(); //正在使用的集团家谱版本编号
...
<script type="text/javascript" language="javascript">
	...
	function reloadTree(){		
			...
			var sTreeViewDetail1="<%=sFamilyMapStatus%>";
			var sTreeViewDetail2="<%=sRefVersionSeq%>";
		   	if(<%=!Version.isVersion(sGroupID,sRefVersionSeq)%>)
	   			OpenPage("/AppMain/Blank.jsp?TextToShow=该集团客户的系统信息暂时还未建好，请稍后...","right");
		   	else
		   		openIFram("sys","right","TreeViewDetail1="+sTreeViewDetail1+"&TreeViewDetail2="+sTreeViewDetail2+"&RightType="+sRightType+"&FamilyMapStatus="+<%=sFamilyMapStatus%>+"&GroupID=<%=sGroupID%>&GroupName=<%=sGroupName%>&KeyMemberCustomerID=<%=sKeyMemberCustomerID%>&RefVersionSeq=<%=sRefVersionSeq%>&CurrentVersionSeq=<%=sCurrentVersionSeq%>&GroupType1=<%=sGroupType1%>");
	}
	//仅查看系统信息
    function systemFamily(){
		...
		var sTreeViewDetail1="<%=sFamilyMapStatus%>";
		var sTreeViewDetail2="<%=sRefVersionSeq%>";
		var sParam = "TreeViewDetail1="+sTreeViewDetail1+"&TreeViewDetail2="+sTreeViewDetail2+"&RightType=<%=sRightType%>&FamilyMapStatus=<%=sFamilyMapStatus%>&GroupID=<%=sGroupID%>&GroupName=<%=sGroupName%>&KeyMemberCustomerID=<%=sKeyMemberCustomerID%>&RefVersionSeq=<%=sRefVersionSeq%>&CurrentVersionSeq=<%=sCurrentVersionSeq%>&GroupType1=<%=sGroupType1%>";
		if(<%=!Version.isVersion(sGroupID,sRefVersionSeq)%>){
