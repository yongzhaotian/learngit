...
<%
    ...
    // CCS-1275 农民产品 ??将职务固定为“农民”并匹配产品代码       获取 新增合同  否农民产品，产品代码 FM 开头
    String fmProduct = Sqlca.getString("select count(1) from Business_Contract where SerialNo ='"+sObjectNo+"' and CustomerID = '"+sCustomerID+"' and contractstatus='060' and BusinessType like 'FM%'");
...
//	String sNo = CurARC.getAttribute(request.getSession().getId()+"city");
	...
	if(sCertType.equals("Ent02")){
	 ...
	 /********CCS-1334,二段式提单开关现金贷可以录客户信息不受预申请***************************/
	...
	if("1".equals(switch_status)){
		if("020".equals(sProductID)){
			doTemp.setReadOnly("MobileTelephone",false);
			doTemp.setReadOnly("WorkCorp",false);
			doTemp.setReadOnly("KinshipName",false);
			doTemp.setReadOnly("KinshipTel",false);
			doTemp.setReadOnly("SelfMonthIncome",false);
			doTemp.setReadOnly("OtherContact",false);
			doTemp.setReadOnly("Contactrelation",false);
			doTemp.setReadOnly("ContactTel",false);
			doTemp.setReadOnly("RelativeType",false);
		}else{
			doTemp.setReadOnly("MobileTelephone",true);
			doTemp.setReadOnly("WorkCorp",true);
			doTemp.setReadOnly("KinshipName",true);
			doTemp.setReadOnly("KinshipTel",true);
			doTemp.setReadOnly("SelfMonthIncome",true);
			doTemp.setReadOnly("OtherContact",true);
			//doTemp.setReadOnly("Contactrelation",true);
			doTemp.setReadOnly("ContactTel",true);
			//doTemp.setReadOnly("RelativeType",true);
		}
	}else{
		doTemp.setReadOnly("MobileTelephone",false);
		doTemp.setReadOnly("WorkCorp",false);
	...
	//update CCS-150 记录每个合同的客户详情信息(新发生状态下的合同，在维护客户信息时将客户信息拷贝到新建立的合同个人客户信息表中)
	//add 现金贷-是否第一次录入客户信息标志（字段IsCashLoanTemp空或1表示第一次录入，0为已录入过）
	if(sCustomerType.substring(0,2).equals("03") || sCustomerType.equals("03"))
	{
		//add CCS-593 PRM-261 新增客户办卡意愿选项  by rqiao 20150325
		//当前用户所属城市是否为长春或大连
		//eidt by pli 20150409 CCS-609 需求变更：现更新需求，如提单城市为“长春市”的话，“是否有意愿办理哈行信用卡用于还款代扣”选项中的“口”显示为灰色
		//String sCount = Sqlca.getString("select count(*) from user_info ui,code_library cl where ui.city = cl.itemno and cl.codeno = 'AreaCode' and (cl.itemno like '2201%' or  itemno like '2102%') and ui.userid = '"+CurUser.getUserID()+"' ");
		String sCount = Sqlca.getString("select count(*) from user_info ui,code_library cl where ui.city = cl.itemno and cl.codeno = 'AreaCode' and itemno like '2102%'  and ui.userid = '"+CurUser.getUserID()+"' ");
		//end by pli 20150409
		if(Integer.parseInt(sCount)<1)//当前用户所属城市不为长春或大连，“是否有意愿办理哈行信用卡用于还款代扣”为非必输项且只读
		{
			doTemp.setReadOnly("HaCreditCardForRepayment",true);
			doTemp.setRequired("HaCreditCardForRepayment",false);
		}
		//end
		
		if("020".equals(sProductID))//现金贷贷款
		{
			if("060".equals(sContractStatus))//新发生贷款
			{
				dwTemp.setEvent("AfterUpdate","!CustomerManage.CopyCustomerRecord("+sObjectNo+","+sCustomerID+")+!公用方法.UpdateColValue(Business_Contract,IsCashLoanTemp,0,SerialNo='"+sObjectNo+"')");
			}else
			{
				dwTemp.setEvent("AfterUpdate","!公用方法.UpdateColValue(Business_Contract,IsCashLoanTemp,0,SerialNo='"+sObjectNo+"')");
			}
		}else if(!"020".equals(sProductID) && "060".equals(sContractStatus))//非现金贷贷款且为新发生贷款
		{
			dwTemp.setEvent("AfterInsert","!CustomerManage.CopyCustomerRecord("+sObjectNo+","+sCustomerID+")");
			dwTemp.setEvent("AfterUpdate","!CustomerManage.CopyCustomerRecord("+sObjectNo+","+sCustomerID+")");
		}
	}
	//end
	//end
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info04;Describe=定义按钮;]~*/%>
	...
	//依次为：
		...
		//5.事件
		//6.资源图片路径
	boolean isInSpecialCity = sStoreCityName.indexOf("广东省广州市")!=-1 || sStoreCityName.indexOf("北京市")!=-1 ||sStoreCityName.indexOf("江西省赣州市")!=-1 ||sStoreCityName.indexOf("广东省中山市")!=-1;
	boolean hasSpecialRole = Boolean.valueOf(CurUser.hasRole("1006"));
	String sButtons[][] = {
		{"ReadOnly".equals(sRightType) ?"false":"true","All","Button","保存","保存所有修改","saveRecord()",sResourcesPath},
		{sTempSaveFlag.equals("2")?"false":"true","All","Button","暂存","暂时保存所有修改内容","saveRecordTemp()",sResourcesPath},
		{(hasSpecialRole && isInSpecialCity)?"true":"false","All","Button","提交报告查询","提交报告查询","saveReportQuery()",sResourcesPath},
	};
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info06;Describe=定义按钮事件;]~*/%>
	...
	function saveRecord(sPostEvents){
		...
		//取消办公电话的主机与分机号拼接  edit by pli2 20150402
		//setItemValue(0, 0, "WorkTel", sWorkTel);
		//end by pli2 20150402
		var sSpouseWorkTelAreaCode=getItemValue(0,0,"SPOUSEWORKTELAREACODE");
		var sSpouseWorkTelMain=getItemValue(0,0,"SPOUSEWORKTELMAIN");
		var sSpouseWorkTelPlus=getItemValue(0,0,"SPOUSEWORKTELPLUS");
		var sSpouseWorkTel="";
		if(typeof(sSpouseWorkTelMain) != "undefined" && sSpouseWorkTelMain.length!=0){
			sSpouseWorkTel=sSpouseWorkTelMain;
			if(typeof(sSpouseWorkTelAreaCode) != "undefined" && sSpouseWorkTelAreaCode.length!=0){
				sSpouseWorkTel=sSpouseWorkTelAreaCode+"-"+sSpouseWorkTelMain;
				if(typeof(sSpouseWorkTelPlus) != "undefined" && sSpouseWorkTelPlus.length!=0){
					sSpouseWorkTel+="-"+sSpouseWorkTelPlus;
				}
			}
		}
		/*****************月收入总额==家里每月提供生活费+其他收入*********/
	    var monthly_income_total=0;
		var monthly_income_payments=getItemValue(0,getRow(),"SelfMonthIncome");
		var monthly_income_Other=getItemValue(0,getRow(),"OtherRevenue");
		if (parseInt(monthly_income_payments) == 0) {
			alert("个人月收入必须大于0！");
			return;
		}
		
		if(monthly_income_Other == null || monthly_income_Other == "" || monthly_income_Other == "undefined"){
			monthly_income_Other = "0.00";
			setItemValue(0,0,"OtherRevenue",monthly_income_Other);
		}
		
		monthly_income_total=parseInt(monthly_income_payments)+parseInt(monthly_income_Other)
			// if(monthly_income_total){
   			 if(!isNaN(monthly_income_total)){ 
				setItemValue(0,0,"monthly_income_total",monthly_income_total);
   			 }
			// }
		//setItemValue(0,0,"SPOUSEWORKTEL",sSpouseWorkTel);
		//录入数据有效性检查
		if (!ValidityCheck()) return;
		if(checkTelPhone() == false) return;
		
		//add by dahl 2015/08/18 ccs-1008 在特定城市下，职位选择农民时，现居住地址选择临近城市无效，即门店所在城市、户籍地址与居住地址三者需一致
		if(!checkPilotC()) return;
		...
		checkWorkTelHeadShip();
		wait = 0;
		initSendMessage();
		
		if( "7" == "<%=sSubProductType%>" ){
			addStudentCchoolInfo();
		}
		sReturnMessageCode = "";
		cellphoneverifyRemarkChange();//CCS-890 保存的时候显示了其他
		
		setHeadShipByFM();<% /* CCS-1275 农民产品 ??将职务固定为“农民”并匹配产品代码 */ %>
	}
	...
	function saveRecordTemp(){
		...
		checkWorkTelHeadShip();//add by clhuang CCS-703消费贷农民单位电话为选填
		cellphoneverifyRemarkChange();//CCS-890 保存的时候显示了其他
	...
	function saveReportQuery(){
		...
		var sMobile = getItemValue(0,0,"MobileTelephone");
		var sKinshipName = getItemValue(0, 0, "KinshipName");
		var sKinshipTel = getItemValue(0, 0, "KinshipTel");
		var sRelativeType = getItemValue(0, 0, "RelativeType");
		var sOtherContact = getItemValue(0, 0, "OtherContact");
		var sContactTel = getItemValue(0, 0, "ContactTel");
		var sContactrelation = getItemValue(0, 0, "Contactrelation");
		
//		var CurOpenStyle = "width=720,height=540,top=20,left=20,toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,overflow=visible,";
//		var sURL = 'https://www.juxinli.com/org/apply/getApplyByUrl?args={"orgInfo":{"distribute_api_token":"e2262dfa19ef45c5b1dff1b48cc2dadf","customized":[2]},"applyInfo":{"applicant":{"name":"'+sCustomerName+'","id_card_num":"'+sIdentityId+'","cell_phone_num":"'+sMobile+'","home_addr":"shanghaishiyangpuqu","work_tel":"","work_addr":"","home_tel":"","cell_phone_num2":""},"contact":[{"contact_name":"'+sKinshipName+'","contact_tel":"'+sKinshipTel+'","contact_type":"'+sRelativeType+'"},{"contact_name":"'+sOtherContact+'","contact_tel":"'+sContactTel+'","contact_type":"'+sContactrelation+'"}],"loan":{"amount":"","term":"","purpose":""},"question":{"marital_status":"1","kid_cnt":"1","income":"1","own_house":"1","own_car":"1","credit_card":"2","work_time":"2","work_type":"2","live_time":"2","province_travel":"0","oversea_travel":"0","couple_phone_num":""}}}';
	    if(typeof(sMobile) == "undefined" || sMobile.length==0){
	    	alert("手机号码不能为空！");
	    	return false;
	    }
	     if(typeof(sKinshipName) == "undefined" || sKinshipName.length==0){
	    	alert("亲属姓名不能为空！");
	    	return false;
	    }
	    ...
	    if(typeof(sRelativeType) == "undefined" || sRelativeType.length==0){
	    	alert("亲属关系不能为空！");
	    	return false;
	    }
	    if(typeof(sOtherContact) == "undefined" || sOtherContact.length==0){
	    	alert("其他联系人姓名不能为空！");
	    	return false;
	    }
	    ...
	    if(typeof(sContactrelation) == "undefined" || sContactrelation.length==0){
	    	alert("其他联系人关系不能为空！");
	    	return false;
	    } 
		...
		var sOpenStyle = "dialogWidth=730px;dialogHeight=540px;resizable=no;scrollbars=no;status:yes;maximize:no;help:no;";
		var sURL = "/CustomerManage/PhoneServiceValidate.jsp";	
	    var sParam = "phoneNumber="+sMobile+"&customerName="+sCustomerName+"&idCard="+sIdentityId+"&KinshipName="+sKinshipName+"&KinshipTel="+sKinshipTel+"&RelativeType="+sRelativeType+"&OtherContact="+sOtherContact+"&ContactTel="+sContactTel+"&Contactrelation="+sContactrelation;
	    AsControl.PopComp(sURL, sParam, sOpenStyle);
	}          
...
<script type="text/javascript">	
	...
	if("1"=="<%=switch_status%>"){
		checkMobile1();//CCS-1334,二段式提单。因手机号直接带过来需要初始化赋值给手机校验处来源字段
	}
	
	<% /* CCS-1275 农民产品 ??将职务固定为“农民”并匹配产品代码 */ %>
	function setHeadShipByFM() {
		if ("0"!="<%=fmProduct%>") {
			setItemValue(0, 0, "HeadShip", "10");
			setItemDisabled(0, 0, "HeadShip", true);
			checkWorkTelHeadShip();// CCS-703 PRM-333 消费贷农民的单位电话改为选填
		} else {
			// setItemDisabled(0, 0, "HeadShip", false);
		}
	}
	setHeadShipByFM();
	<% /* CCS-1275 农民产品 ??将职务固定为“农民”并匹配产品代码 */ %>
