...
<%
	...
	if(sAssetsType == null) sAssetsType = "03";
%>
<%/*~END~*/%>
...
<%
	...
	String sTempletNo = "ReserveIndPara";
    ...
    doTemp.appendHTMLStyle("LossRate1"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"未逾期减值比例必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("LossRate2"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"逾期1至30天减值比例必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("LossRate3"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"逾期31天至90天减值比例必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("LossRate4"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"逾期91天至360天减值比例必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("LossRate5"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"逾期361天以上减值比例必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("AdjustModulus"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\" 调整系数必须大于等于0，小于等于1！\" ");
    doTemp.appendHTMLStyle("RateofLoss"," myvalid=\"parseFloat(myobj.value,10)>=0 && parseFloat(myobj.value,10)<=1 \" mymsg=\"历史违约率必须大于等于0，小于等于1！\" ");
...
<script type="text/javascript">
	...
	function saveRecord(sPostEvents)
	{
		...
		{			
			...
			//调用页面验证 "03"表示客户类型为个人客户
			sParaString = sAccountMonth + "," + "03" + "," + sAssetsType;
	...
	function goBack()
	{
   		...
   		OpenPage("/Reserve/ReservePara/ReserveIndParaList.jsp","_self","");
