...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "新增经销商借款"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%	
	ASDataObject doTemp = new ASDataObject("DistributorLoadInfo",Sqlca);
...
//    	var fBankName= RunMethod("GetElement","GetElementValue","bankName,account_Information,relativeSerialno='"+sCustomerID+"' and accountType=01"); //放款开户支行名称
//    	var sBankName =RunMethod("GetElement","GetElementValue","bankName,account_Information,relativeSerialno='"+sCustomerID+"' and accountType=02");//收款开户支行名称
//    	var fAccountNo =RunMethod("GetElement","GetElementValue","accountNo,account_Information,relativeSerialno='"+sCustomerID+"' and accountType=01");//放款开户帐号
//   	var sAccountNo =RunMethod("GetElement","GetElementValue","accountNo,account_Information,relativeSerialno='"+sCustomerID+"' and accountType=02");//收款开户帐号
...
//    	setItemValue(0, 0, "loanBankAccount", fAccountNo);
 //   	setItemValue(0, 0, "repaymentBankAccount", sAccountNo);
    ...
    function getRegionCode1(){
    	...
    	var sEntInfoValue1=setObjectValue("SelectDistributorLoadInfo", "customerID,"+sCustomerID, "@bondSum@4", 0, 0, "");
    ...
    function getRegionCode2(){
    	var sEntInfoValue2=setObjectValue("SelectDistributorLoadInfo1","","@productName@1",0,0,"");
...
//    	sInterestRate=sEntInfoValue2[3];     //利率类型
 //   	sFloatingRange=sEntInfoValue2[4];    //浮动幅度
    	sRepriceType=sEntInfoValue2[6];    //利率调整方式
    	setItemValue(0, 0, "BusinessType", sBusinessType); //产品代码
    	setItemValue(0, 0, "RepriceType", sRepriceType); //利率调整方式
    }
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function initRow(){
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0, 0, "productID", "050");
