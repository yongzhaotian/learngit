...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "流动资金贷款详情 "; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function insertTermPara(){
		...
		var repaymentDate = getItemValue(0,getRow(),"repaymentDate");//首期还款日期repaymentDate
		var defaultDate =parseInt(repaymentDate.substr(8), 10);//默认还款日
		...
		RunMethod("ProductManage","initObjectWithProduct","initObjectWithProduct,"+RPTTermID+","+sObjectType+","+sObjectNo);
	 	RunMethod("PublicMethod","UpdateColValue","String@DEFAULTDUEDAY@"+defaultDate+",acct_rpt_segment,String@rpttermid@"+RPTTermID+"@String@ObjectNo@"+sObjectNo);//默认还款日
		//利率
		//罚息 固定为浮动罚息
		...
		var securityServices = getItemValue(0,getRow(),"securityServices");//担保服务费
		var otherCost = getItemValue(0,getRow(),"otherCost");//其他费
		RunMethod("PublicMethod","UpdateColValue","String@DEFAULTVALUE@03,PRODUCT_TERM_PARA,String@paraid@FeePayDateFlag@String@termid@N400@String@ObjectNo@"+sObjectNo);//随交易一次性收取
		RunMethod("PublicMethod","UpdateColValue","String@DEFAULTVALUE@06,PRODUCT_TERM_PARA,String@paraid@FeePayDateFlag@String@termid@QT100@String@ObjectNo@"+sObjectNo);//首次还款日收取
		RunMethod("PublicMethod","UpdateColValue","String@DEFAULTVALUE@05,PRODUCT_TERM_PARA,String@paraid@FeePayDateFlag@String@termid@QT100@String@ObjectNo@"+sObjectNo);//随还款计划
		...
		RunMethod("PublicMethod","UpdateColValue","String@AMOUNT@"+fee+",ACCT_FEE,String@FEEtermid@N400@String@ObjectNo@"+sObjectNo);//手续费
		RunMethod("PublicMethod","UpdateColValue","String@AMOUNT@"+securityServices+",ACCT_FEE,String@FEEtermid@N300@String@ObjectNo@"+sObjectNo);//担保服务费
		RunMethod("PublicMethod","UpdateColValue","String@AMOUNT@"+otherCost+",ACCT_FEE,String@FEEtermid@QT100@String@ObjectNo@"+sObjectNo);//其他费
