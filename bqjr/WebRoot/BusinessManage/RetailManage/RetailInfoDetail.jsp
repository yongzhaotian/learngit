...
function openChildComp(sURL,sParameterString){
	...
	function TreeViewOnClick(){
		...
		if(sCurItemname=='基本信息'){
			AsControl.OpenView("/BusinessManage/RetailManage/RetailApplyInfo.jsp","SerialNo=<%=sSerialNo%>","right");
		}else if(sCurItemname=='附件信息'){
			if(ssSerailNo==""){
				alert("请先填写基本信息！");
				return;
			}else{
				AsControl.OpenView("/BusinessManage/RetailManage/ImageViewFrame.jsp","SerialNo=<%=sSerialNo%>&RegCode=<%=sRegCode%>&PhaseType=<%=sPhaseType%>","right");
			}
		}else{
			return;
		}
