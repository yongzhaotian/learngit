<%@ page contentType="text/html; charset=GBK"%><%@
 include file="/IncludeBegin.jsp"%><%
	/*
		页面说明:示例模块主页面
	 */
	String PG_TITLE = " 移动Pos点管理"; // 浏览器窗口标题 <title> PG_TITLE </title>
	...
	//定义Treeview
	HTMLTreeView tviTemp = new HTMLTreeView(SqlcaRepository,CurComp,sServletURL,"移动Pos点管理","right");
	tviTemp.ImageDirectory = sResourcesPath; //设置资源文件目录（图片、CSS）
	tviTemp.toRegister = false; //设置是否需要将所有的节点注册为功能点及权限点
	tviTemp.TriggerClickEvent=true; //是否选中时自动触发TreeViewOnClick()函数

	//定义树图结构
	tviTemp.insertPage("root","移动Pos点列表","",1);
...
<script type="text/javascript"> 
	...
	function initTreeView(){
		...
		expandNode('root');
		selectItem(1);
		selectItemByName("移动Pos点列表");	//默认打开的(叶子)选项	
	}
