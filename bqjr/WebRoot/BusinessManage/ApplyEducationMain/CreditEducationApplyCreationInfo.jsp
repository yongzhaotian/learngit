...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Info03;Describe=定义数据对象;]~*/%>
	...
	String sTempletNo = "ApplyNewEducationInfo";
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=Info07;Describe=自定义函数;]~*/%>
	...
	function selectProductID()
	{
		...
		var sPaymentSum = getItemValue(0,0,"ProductSum");
		var sShoufuratio1 = getItemValue(0,0,"Shoufuratio");//首付比例
		var sCertID = getItemValue(0,0,"CertID");
	    ...
	    var productname="<%=subProductType%>";
	    var sno="<%=sNo%>";
	    var sParaString = "productName,"+productname+",sno,"+sno+",sum,"+sProductSum+","+"PaymentSum"+","+sPaymentSum+",Shoufuratio,"+sShoufuratio1;
		
	    //设置返回参数 Periods  这个是合同期数
		var result = setObjectValue("SelectApplyEducationInfo",sParaString,"@BusinessType@0@ProductName@1@Periods@2",0,0,""); 
		var arr = new Array();
		arr = result.split("@");
		if(arr.length>0){
			var isupload=arr[3];
			setItemValue(0,0,"uploadFlag",isupload)// 是否上传 
		}
...
//是否投保，联动计算每月还款额
	...
	function initRow()
	{
		if (getRowCount(0)==0) //如果没有找到对应记录，则新增一条，并设置字段默认值
		{
			...
			setItemValue(0,0,"ProductID","030");//add 现金贷需求
			...
			if(sOPERATEMODE=='03'&&sEmployeeType=='02'){
				setItemValue(0,getRow(),"OperatorMode","01");
			}
			if(sOPERATEMODE=='02'&&sEmployeeType=='02'){
				setItemValue(0,getRow(),"OperatorMode","02");
			}
			if(sOPERATEMODE=='01'&&sEmployeeType=='02'){
				setItemValue(0,getRow(),"OperatorMode","04");
			}
            ...
            if(sEmployeeType=='01'){
				setItemValue(0,getRow(),"OperatorMode","03");
				//移动Pos点
