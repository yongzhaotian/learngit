...
<%
    ...
    //CCS-344 贷款人操作中无法添加所有城市    产品类型的参数传递，不同的产品类型可以是同一个贷款人   add by awang 2015/03/11
    String sSubProductType=DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ProductType"));
    ...
    if(sProductType == null) {
    	...
    	sSubProductType = sProductType.replace(",","','");
    } */
    String sLoaner = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("sLoaner"));
    if(sLoaner == null) sLoaner = "";
...
<%
		int num = 0;
        //CCS-344 贷款人操作中无法添加所有城市  产品类型为Temp表示暂存，尚未进行持久化  edit by awang 2015/03/11
		/* String queryStr = " select itemno,itemname from code_library " +
				" where itemno not in (select PC.areacode from ProvidersCity PC where (ProductType in ('"+sSubProductType+"') and SerialNo<>:SerialNo) or (ProductType LIKE 'Temp%' and SerialNo=:SerialNo)) and CodeNo='AreaCode' and IsInuse='1' and (length(sortNo)=4 and substr(SortNo,1,2) not in ('11','12','31','50') or SortNo in ('11','12','31','50'))  order by SortNo ASC"; */
	    String queryStr = " select itemno,itemname from code_library " +
	              " where itemno not in (select PC.AREACODE from service_providers sp, ProvidersCity PC where sp.serialno = PC.Serialno and sp.loaner = '"+sLoaner+"' and PC.PRODUCTTYPE = '"+sSubProductType+"') and CodeNo='AreaCode' and IsInuse='1' and (length(sortNo)=4 and substr(SortNo,1,2) not in ('11','12','31','50') or SortNo in ('11','12','31','50')) order by itemno ASC";
		ASResultSet rs = Sqlca.getASResultSet(new SqlObject(queryStr));
		num = 0;
		while(rs.next()){
			String cityname = rs.getString("itemname");
			String itemno=rs.getString("itemno");
			out.println("availableReportCaptionList[" + num + "] = '" + cityname + "';\r");
		...
		/* queryStr = "  select itemno,itemname from code_library " +
			" where itemno in (select PC.areacode from ProvidersCity PC where SerialNo=:SerialNo and ProductType like 'Temp%') and CodeNo='AreaCode' and IsInuse='1' and (length(sortNo)=4 and substr(SortNo,1,2) not in ('11','12','31','50') or SortNo in ('11','12','31','50'))"; */
		queryStr = "  select itemno,itemname from code_library " +
		" where 1=2 and itemno in (select PC.areacode from ProvidersCity PC where SerialNo=:SerialNo and ProductType='"+sSubProductType+"') and CodeNo='AreaCode' and IsInuse='1' and (length(sortNo)=4 and substr(SortNo,1,2) not in ('11','12','31','50') or SortNo in ('11','12','31','50'))order by itemno ASC";
		ASResultSet rs1 = Sqlca.getASResultSet(new SqlObject(queryStr).setParameter("SerialNo",sSerialNo));
		num = 0;
		while(rs1.next()){
			String cityname = rs1.getString("itemname");
			String itemno = rs1.getString("itemno");
			out.println("selectedReportCaptionList[" + num + "] = '" + cityname + "';\r");
...
<script type="text/javascript">
   ...
   function pushButton(buttonName,push){
	...
	function doQuery(){
		...
		/* var vTemp=""; */
		//CCS-344 贷款人操作中无法添加所有城市    对选择的城市进行暂存(即ProductType为Temp)  edit by awang 2015/03/11
		var vTemp="";
		for(var i=0; i<vReportCount;i++){
			if(i==0){
			vTemp =analyseterm.report_chosen.options[i].value;
			}else{
			vTemp =vTemp+"@"+analyseterm.report_chosen.options[i].value;
			}
		}
		
		RunJavaMethodSqlca("com.amarsoft.app.billions.RetailStoreCommon", "addProviderscity", "serialNo=<%=sSerialNo%>,ProductType="+sSubProductType+",AreaCodes="+vTemp);
