...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "跟进中的业务"; // 浏览器窗口标题 <title> PG_TITLE </title>
	%>
<%/*~END~*/%>
...
<%	
	...
	String sTempletNo = "FollowUpCollectionList"; //模版编号
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List04;Describe=定义按钮;]~*/%>
	...
	String sButtons[][] = {
			...
			{"true","","Button","跟进结果登记","跟进结果登记","addCarRegistRecord()",sResourcesPath},
			{"true","","Button","退回任务","退回任务","colectionTaskGoBack()",sResourcesPath},
		};
	%> 
<%/*~END~*/%>
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	//---------------------定义按钮事件------------------------------------
	/*~[Describe=使用OpenComp打开合同详情;InputParam=sBcSerialNo;OutPutParam=无;]~*/
	...
	/*~[Describe=跟进结果登记  ;InputParam=sBcSerialNo;OutPutParam=无;]~*/
	function addCarRegistRecord(){
		var sColSerialNo = getItemValue(0,getRow(),"SerialNo");//汽车催收任务流水号
		var sBcSerialNo = getItemValue(0,getRow(),"ContractSerialNo");//汽车催收任务合同编号
		
		if (typeof(sColSerialNo)=="undefined" || sColSerialNo.length==0){
			alert(getHtmlMessage('1'));//请选择一条信息！
			return;
		}
	...
	/*~[Describe=退回任务  ;InputParam=sBcSerialNo;OutPutParam=无;]~*/
	function colectionTaskGoBack(){
		var sSerialNo = getItemValue(0,getRow(),"SerialNo");//催收任务号
		if (typeof(sSerialNo)=="undefined" || sSerialNo.length==0){
			alert(getHtmlMessage('1'));//请选择一条信息！
			return;
		...
		var sReturn = AsControl.RunJavaMethodSqlca("com.amarsoft.app.hhcf.after.colection.AfterColectionAction","updateCarColectionStatus","ColSerialNo="+sSerialNo);
		if(sReturn != "Success"){
			alert("催收任务退回状态更新失败!");
			return;
		}
