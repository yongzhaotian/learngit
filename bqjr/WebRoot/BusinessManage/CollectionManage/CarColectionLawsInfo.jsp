...
<%
	...
	// 获得页面参数
	String sColSerialNo =  DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("CollectionSerialNo"));//催收任务编号
	...
	out.println("sColSerialNo:"+sColSerialNo+"sBcSerialNo:"+sBcSerialNo+"sSerialNo:"+sSerialNo);
	...
	String sTempletNo = "CARCOLLECTIONLAWSINFO";//模型编号
	...
	doTemp.setDDDWSql("REGISTTYPE", "select itemno,itemname from code_library where codeno='LawsType'");
	...
	//生成HTMLDataWindow
	Vector vTemp = dwTemp.genHTMLDataWindow(sSerialNo+","+sColSerialNo+","+sBcSerialNo);//传入参数,逗号分割
...
<script type="text/javascript">
	...
	function saveRecord(sPostEvents){
		...
		//alert("RegistSerialNo="+sSerialNo+"&ColSerialNo=<%=sColSerialNo%>&BcSerialNo=<%=sBcSerialNo%>");
		
		var sReturn = AsControl.RunJavaMethodSqlca("com.amarsoft.app.hhcf.after.colection.AfterColectionAction","saveLawsInfo","RegistSerialNo="+sSerialNo+",ColSerialNo=<%=sColSerialNo%>,BCSerialNo=<%=sBcSerialNo%>");
		if(sReturn != "true"){
			alert("该催收任务的诉讼信息登记失败!");
			return;
		}
		//reloadSelf();
	}
	...
	function goBack(){
		AsControl.OpenPage("/BusinessManage/CollectionManage/CarCollectionLawsList.jsp","ContractSerialNo=<%=sBcSerialNo%>&CollectionSerialNo=<%=sColSerialNo%>","_self","");
	}
