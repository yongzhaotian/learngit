...
<%
	...
	String uploadType = (String)myAmarsoftUpload.getRequest().getParameter("uploadType");//文件上传时是覆盖还是追加
	...
	String sFullPath = absPath + File.separator + sDocNo + sFileName.substring(sFileName.lastIndexOf("."));
	
	if (!myAmarsoftUpload.getFiles().getFile(0).isMissing()){
		myAmarsoftUpload.getFiles().getFile(0).saveAs(sFullPath);
	}
	
	sFullPath = sFullPath.replaceAll("\\\\", "/")+"@"+uploadType;
