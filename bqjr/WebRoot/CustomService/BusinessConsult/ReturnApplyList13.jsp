	  ...
	  var sAlertIDString="";
		if(!confirm("退保后该合同将无法再次申请投保，确定要退保？")){
			...
			for(var k=0;k<statusList.length;k++){
				status = statusList[k];
			  if((status !=  "申请退保")){
			    alert("保单号："+policyno[k]+",不能做退保审批！请确认该保单是不是处于申请退保状态。");
			    return;
			  }
			}
			var policynos="";
			...
			if(	typeof(policynos)=="undefined" || policynos.length==0){//单选
		    	...
		    	status = getItemValue(0,getRow(),"status");
					...
					if((status !=  "申请退保")){
					    alert("保单号："+policynos+",不能做退保审批！请确认该保单是不是处于申请退保状态。");
					    return;
					  }
				}
	  ...
	  var sAlertIDString="";
		if(!confirm("是否要取消该笔退保申请？")){
			...
			for(var k=0;k<statusList.length;k++){
				status = statusList[k];
			  if((status !=  "申请退保")){
			    alert("保单号："+policyno[k]+",不能取消退保！请确认该保单是否是申请退保状态");
			    return;
			  }
			}
			var policynos="";
	    	...
	    	status = getItemValue(0,getRow(),"status");
				...
				if((status !=  "申请退保")){
				    alert("保单号："+policynos+",不能取消退保！请确认该保单是否是申请退保状态。");
				    return;
				  }
			}
