...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=List01;Describe=定义页面属性;]~*/%>
	<%
	//jqcao:原objectType 写死的，改为传参，解决空指针异常
	...
	if(objectType == null) throw new Exception("未传入对象类型，请检查！");
	if(objectNo == null) throw new Exception("未传入对象编号，请检查！");
	...
	//if(document == null || !document.getObjectType().equals(BUSINESSOBJECT_CONSTATNTS.business_putout)) throw new Exception("必须传入出账信息，请检查！");
	...
	}else
	{
		...
		if(transactionCode==null||transactionCode.length()==0) 
			throw new Exception("该产品｛"+document.getString("BusinessType")+"-"+ document.getString("ProductVersion")+"｝未配置交易组件｛"+transactionTermID+"｝的参数｛TransactionCode｝的默认值！请确认该类业务是否可使用此功能！");
