...
<%
	...
	sSql = " select di.ObjectType,di.ObjectNo,di.SerialNo,di.DunLetterNo,bc.CustomerName,"+
	...
	sCorpus = DataConvert.toMoney(dCorpus);
	sInterestInSheet = DataConvert.toMoney(dInterestInSheet);
	sInterestOutSheet = DataConvert.toMoney(dInterestOutSheet);
	sElseFee = DataConvert.toMoney(dElseFee);
	sDunSum = DataConvert.toMoney(dDunSum);
...
<html>
	...
	<tr>
		<td colspan='5' align=left class=td1 height='50'> 
		<font style=font-size:11pt;line-height:2> &nbsp &nbsp 根据<%=sArtificialNo%>号（文本合同编号）保证合同，贵单位为<%=sCustomerName%>借款提供连带保证责任,该借款已于<%=sYear%>年<%=sMonth%>月<%=sDay%>日到期。请尽快督促借款人或筹措资金偿还：</font>
		</td>
	</tr>	
	<tr>
		<td colspan='5' align=left class=td1 height='25'> 
	...
	<tr>
		...
		<font style=font-size:11pt;line-height:130%>保证人章(或签收人签字)：</font> 
		</td>
	...
	<tr>
		...
		<tr>
		<td colspan='5' align=left class=td1 height='25'> 
		<font style=font-size:11pt;line-height:130%>注：本通知书一式两份：保证人签收后留存一份，退贷款人一份作为回执。</font> 
		</td>
	</tr>
