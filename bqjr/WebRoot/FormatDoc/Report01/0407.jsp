...
<%
	...
	String sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportscope in (select min(reportscope) from REPORT_RECORD where objectno = '"+sCustomerID+"' and reportdate = '"+sYearN+"'))";
	//out.println(sSql);
	String sRowName[] = {"907","908","905","918","906"};
	String sCol2Value[]={"","","","",""};
	String sCol2Value1[]={"","","","",""};
	String sCol2Value2[]={"","","","",""};
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_1+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportscope in (select min(reportscope) from REPORT_RECORD where objectno = '"+sCustomerID+"' and reportdate = '"+sYearN_1+"'))";
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_2+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9' and reportscope in (select min(reportscope) from REPORT_RECORD where objectno = '"+sCustomerID+"' and reportdate = '"+sYearN_2+"'))";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("RowSubject");
...
<%
	...
	sTemp.append("<form method='post' action='0407.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	//colspan需根据实际情况调整，否则导出时会比较不雅
	sTemp.append("   <td class=td1 align=left colspan='4' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4.7、营运能力分析</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
