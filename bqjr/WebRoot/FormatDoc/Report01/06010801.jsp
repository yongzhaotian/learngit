...
//***********************************************************************	
	rs2 = Sqlca.getResultSet("select GuarantyNo from FORMATDOC_DATA where SerialNo = '"+sSerialNo+"'");
...
//取最新资产负债表日期
	String sSql1 = "";
	sSql1 = " select substr(ReportDate,1,4) as Year,substr(ReportDate,6,2) as Month,ReportNo from REPORT_RECORD "+
		   " where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%1' And  ReportDate = (select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%1')";
...
//年报
	//取最新资产负债表年报日期
	sSql1 = " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
		" where ObjectNo ='"+sGuarantyNo+"' and ModelNo like '%1'"+
		" and  ReportDate in(select ReportDate from CUSTOMER_FSRECORD where CustomerID= '"+ sGuarantyNo +"' and ReportPeriod='04')"+
		" and  ReportDate <>(select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+ sGuarantyNo +"' And ModelNo like '%1')"+
		" order by Year Desc";
	rs2 = Sqlca.getResultSet(sSql1); 
	k = 0;
	while (k < 3){
...
//*****************************************损益表结构和对比*****************************
	//取最近损益表日期
	sSql1 = " select substr(ReportDate,1,4) as Year,substr(ReportDate,6,2) as Month,ReportNo from REPORT_RECORD " +
			" where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%2' And ReportDate = (select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%2')";
	rs2 = Sqlca.getResultSet(sSql1);
	if(rs2.next()){
		sYear = rs2.getString("Year");	//日期
		if(sYear == null){
	...
	rs2 = Sqlca.getResultSet("select Col2value,RowSubject from REPORT_DATA where ReportNo = '"+sNewReportNo2+"' And RowSubject in ('501','502','505','503','507','508','509','515','517') order by RowSubject ");
	while(rs2.next()){
		sRowSubject = rs2.getString("RowSubject");
...
//年报
	sSql1 = " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
		" where ObjectNo ='"+ sGuarantyNo +"' and ModelNo like '%2'"+
		" and  ReportDate in(select ReportDate from CUSTOMER_FSRECORD where CustomerID= '"+ sGuarantyNo +"' and ReportPeriod='04')"+
		" and  ReportDate <>(select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+ sGuarantyNo +"' And ModelNo like '%2')"+
		" order by Year Desc";
	rs2 = Sqlca.getResultSet(sSql1);
	k = 0;
	while (k < 3){
...
//*****************************************现金流量表*****************************
	//取最近现金流量表日期
	sSql1 = " select substr(ReportDate,1,4) as Year,substr(ReportDate,6,2) as Month,ReportNo from REPORT_RECORD " +
			" where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%8' And ReportDate = (select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+sGuarantyNo+"' And ModelNo like '%8')";
	rs2 = Sqlca.getResultSet(sSql1);
	if(rs2.next()){
		sYear = rs2.getString("Year");	//日期
		if(sYear == null){
...
//年报
		sSql1 = " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
		" where ObjectNo ='"+ sGuarantyNo +"' and ModelNo like '%8'"+
		" and  ReportDate in(select ReportDate from CUSTOMER_FSRECORD where CustomerID= '"+ sGuarantyNo +"' and ReportPeriod='04')"+
		" and  ReportDate <>(select max(Reportdate) from REPORT_RECORD Where ObjectNo ='"+ sGuarantyNo +"' And ModelNo like '%8')"+
		" order by Year Desc";
	rs2 = Sqlca.getResultSet(sSql1);
	k = 0;
	while (k < 3){
...
<%
	...
	sTemp.append("	<form method='post' action='06010801.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >"+sNo1[j]+".8 保证人财务分析</font></td>"); 	
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >"+sNo1[j]+".8.1、财务简表分析</font></td>"); 	
	sTemp.append("   </tr>");
	sTemp.append("<tr>");
	sTemp.append("   <td width=18% colspan=12 align=center class=td1 > <br><strong>资产负债简表结构和对比</strong> <br>&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	...
	sTemp.append("<td align=right class=td1 >"+sValue[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sProportion[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sValue1[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sProportion1[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sValue2[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sProportion2[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sValue3[0]+"</td>");
	sTemp.append("<td align=right class=td1 >"+sProportion3[0]+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("   <td colspan=12 align=left class=td1 ><br><strong> 损益表结构和对比 </strong><br>&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td rowspan=2 align=center class=td1 > 项目 </td>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("   <td colspan=12 align=left class=td1 > <br><strong>现金流</strong><br>&nbsp; </td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td rowspan=2 align=center class=td1 > 项目 </td>");
