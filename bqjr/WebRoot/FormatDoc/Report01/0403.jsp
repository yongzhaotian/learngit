...
//取最新资产负债表日期
	String sSql = "";
	sSql = "select substr(ReportDate,1,4) as Year,substr(ReportDate,6,2) as Month,ReportNo from REPORT_RECORD "+
	...
	//年报
	//取最新资产负债表年报日期
	sSql =  " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
			" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%1' And  ReportDate like '%12' order by Year Desc,ReportScope Asc";
...
//*****************************************损益表结构和对比*****************************
	...
	//年报
	sSql =" select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
		  " where ObjectNo ='"+sCustomerID+"' And ModelNo like '%2' And  ReportDate like '%12' order by Year Desc,ReportScope Asc" ;
	rs2 = Sqlca.getResultSet(sSql); 
	k = 0;
	while (k < 3){
		if(rs2.next()){
...
//年报
	sSql =  " select substr(ReportDate,1,4) as Year,ReportNo from REPORT_RECORD "+
			" where ObjectNo ='"+sCustomerID+"' And ModelNo like '%8' And  ReportDate like '%12' order by Year Desc,ReportScope Asc";
	rs2 = Sqlca.getResultSet(sSql); 
	k = 0;
	while (k < 3){
		if(rs2.next()){
...
<%
	...
	sTemp.append("   <td class=td1 align=left colspan='9' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4.3、财务简表分析</font></td>"); 	
	sTemp.append("   </tr>");
	sTemp.append("<tr>");
