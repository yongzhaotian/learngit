...
<%
	...
	//获得应收总额
	ASResultSet rs2 = Sqlca.getResultSet("select sum(FOASum) from ENT_FOA where CustomerID = '"+sCustomerID+"'");
	if(rs2.next())
	{
		dValue = rs2.getDouble(1)/10000;
	}
	...
	if(d010>0)
	{
		...
		while(rs2.next()) 
		{
			...
			double dysValue = 0.0;
			if(sAccountYears.equals("010"))
			{
				sysValue[0] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
				sProportion[0] = DataConvert.toMoney(dysValue/dValue*100);
			}
			else if(sAccountYears.equals("020"))
			{
				sysValue[1] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
				sProportion[1] = DataConvert.toMoney(dysValue/dValue*100);
			}
			else if(sAccountYears.equals("030"))
			{
				sysValue[2] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
	...
	{         
		...
		while(rs2.next())
		{
			...
			if(sAccountYears.equals("010"))
			{
				sysValue1[0] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
				sProportion1[0] = DataConvert.toMoney(dysValue/dValue*100);
			}
			else if(sAccountYears.equals("020"))
			{
				sysValue1[1] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
				sProportion1[1] = DataConvert.toMoney(dysValue/dValue*100);
			}
			else if(sAccountYears.equals("030"))
			{
				sysValue1[2] = DataConvert.toMoney(rs2.getDouble(2)/10000);
				dysValue = rs2.getDouble(2)/10000;
...
<%
	...
	sTemp.append("<form method='post' action='0404.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("  <tr>");	
	//colspan需根据实际情况调整，否则导出时会比较不雅
	sTemp.append("   <td class=td1 align=left colspan='15' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4.4、资产分析</font></td>"); 	
	sTemp.append("  </tr>");	
	//*********************************应收帐款分析**********************************
	...
	sTemp.append("  </tr>");
	sTemp.append("  <tr>");
	sTemp.append("   <td width='12%' colspan='2' align=center class=td1 > 金额 </td>");
    sTemp.append("   <td width=6% align=center class=td1 >%</td>");
    sTemp.append("   <td width='12%' colspan='2' align=center class=td1 > 金额 </td>");
    sTemp.append("   <td width=6% align=center class=td1 >%</td>");
    sTemp.append("   <td width='12%' colspan='2' align=center class=td1 > 金额 </td>");
    sTemp.append("   <td width=6% align=center class=td1 >%</td>");
    sTemp.append("   <td width='22%' colspan='2' align=center class=td1 > 金额 </td>");
    ...
    sTemp.append("   <td colspan='2' align=center class=td1 >金额</td>");
    ...
    String sSql7 = "select RelatedCorpName,FOASum,Remark,RelativeDescribe from ENT_FOA where FOAType='010' and CustomerID = '"+sCustomerID+"' "+"order by FOASum desc";
    rs2 = Sqlca.getResultSet(sSql7);
    int j=0;
    String sRelatedCorpName = ""; //应收帐款大户名单名称
    String sFOASum = "";		  //应收帐款金额
    ...
    if(d010>0)
    {
    	...
    	if(rs2.next())
    	{
    		...
    		sFOASum = DataConvert.toMoney(rs2.getDouble(2)/10000);
    		dFOASum = rs2.getDouble(2)/10000;
    ...
    String sSql8 = "select RelatedCorpName,FOASum,Remark,RelativeDescribe from ENT_FOA where FOAType='015' and CustomerID = '"+sCustomerID+"' "+"order by FOASum desc";
	...
	sTemp.append("  <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 "+myShowTips(sMethod)+" >应收帐款分析");
	sTemp.append("	 </td>");
	sTemp.append("  </tr>");
	sTemp.append("  <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 >");
	sTemp.append(myOutPut("1",sMethod,"name='describe1' style='width:100%; height:150'",getUnitData("describe1",sData)));
	sTemp.append("	 </td>");
	sTemp.append("  </tr>");
	//**********************************存货分析*************************************
	sTemp.append("  <tr align='left'>");
	sTemp.append("   <td colspan='15' class=td1 ><br> <strong>(2) 存货分析</strong> <br>&nbsp;</td>");
    sTemp.append("  </tr>");
	sTemp.append("  <tr>");
	sTemp.append("   <td colspan='3' align=center class=td1 >&nbsp; </td>");
    sTemp.append("   <td colspan='4' align=center class=td1 > 金额 </td>");
    sTemp.append("   <td colspan='2' align=center class=td1 >%</td>");
    sTemp.append("   <td colspan='6' align=center class=td1 > 存货是否正常</td>");
    sTemp.append("  </tr>");
    
    //存货合计
    String sSql91 = "select  sum(ValueSum) from ENT_INVENTORY where InventoryType in('010','020','030') and CustomerID = '"+sCustomerID+"'";
    rs2 = Sqlca.getResultSet(sSql91);
    double sum = 0.0;
    if(rs2.next())
    {
    	sum = rs2.getDouble(1)/10000;
    }
    rs2.getStatement().close();
    
    //产成品
    String sSql9 = "select  ValueSum,getitemname('YesNo',QualityStatus) from ENT_INVENTORY where InventoryType = '030' and CustomerID = '"+sCustomerID+"'";
    rs2 = Sqlca.getResultSet(sSql9);
    String sValueSum = "";			//金额
    String sQualityStatus = "";		//存货是否正常
    if(rs2.next())
    {
    sValueSum = DataConvert.toMoney(rs2.getDouble(1)/10000);
    sQualityStatus = rs2.getString(2);
    if(sQualityStatus == null) sQualityStatus= " ";
    String sProportions="0.0";//占比
    if(sum!=0) sProportions=DataConvert.toMoney(rs2.getDouble(1)/10000/sum*100);
	sTemp.append("  <tr>");
    ...
    //在产品
    String sSql10 = "select  ValueSum,getitemname('YesNo',QualityStatus) from ENT_INVENTORY where InventoryType = '020' and CustomerID = '"+sCustomerID+"'";
	...
	//原材料
	String sSql11 = "select  ValueSum,getitemname('YesNo',QualityStatus) from ENT_INVENTORY where InventoryType = '010' and CustomerID = '"+sCustomerID+"'";
	...
	sTemp.append("  <tr>");
	sTemp.append("   <td colspan='3' align=center class=td1 > 合计</td>");
    sTemp.append("   <td colspan='4' align=center class=td1 >"+sum+"&nbsp;</td>");
    sTemp.append("   <td colspan='8' align=center class=td1 >&nbsp;</td>");
    sTemp.append("  </tr>");
	sTemp.append("  <tr align='left'>");
	sTemp.append("   <td colspan='15' class=td1 "+myShowTips(sMethod)+" >存货分析");
	sTemp.append("	 </td>");
    sTemp.append("  </tr>");
    sTemp.append("  <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 >");
	...
	String sSql121 = "select sum(EvalValue) from ENT_FIXEDASSETS where FixedAssetsType in ('010','020','030','040') and CustomerID = '"+sCustomerID+"'";
	...
	String sSql12 = "select EvalValue,Depreciation,Rate from ENT_FIXEDASSETS where FixedAssetsType = '010' and CustomerID = '"+sCustomerID+"'";
	rs2 = Sqlca.getResultSet(sSql12);
	String sEvalValue = "";				//固定资产金额
    String sDepreciation = "";			//固定资产折旧方法
    double dRate = 0.0;					//固定资产综合成新率
    if(rs2.next())
    {
    sEvalValue = DataConvert.toMoney(rs2.getDouble(1));
	...
	//机器设备
	String sSql13 = "select EvalValue,Depreciation,Rate from ENT_FIXEDASSETS where FixedAssetsType = '020' and CustomerID = '"+sCustomerID+"'";
	...
	//运输工具
	String sSql14 = "select EvalValue,Depreciation,Rate from ENT_FIXEDASSETS where FixedAssetsType = '030' and CustomerID = '"+sCustomerID+"'";
	...
	//其他
	String sSql15 = "select EvalValue,Depreciation,Rate from ENT_FIXEDASSETS where FixedAssetsType = '040' and CustomerID = '"+sCustomerID+"'";
    ...
    sTemp.append("   <td colspan='4' align=center class=td1 > 入帐价值 </td>");
    sTemp.append("   <td colspan='2' align=center class=td1 >%</td>");
	...
	sum = 0.0;
	if(rs2.next()) sum = rs2.getDouble(1)/10000;
	rs2.getStatement().close();
	...
	sAccountValue = DataConvert.toMoney(rs2.getDouble(2)/10000);
	sEvaluateMethod = rs2.getString(3);
	if(sEvaluateMethod == null) sEvaluateMethod = " ";
	String sProportions="0.0";//占比
    if(sum!=0) sProportions=DataConvert.toMoney(rs2.getDouble(2)/10000/sum*100);
	sTemp.append("  <tr>");
    ...
    sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 "+myShowTips(sMethod)+" >无形资产分析");
	sTemp.append("	 </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 >");
	...
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 "+myShowTips(sMethod)+" >对外担保和其他或有负债分析");
	sTemp.append("	 </td>");
    sTemp.append("   </tr>");
    sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 >");
	...
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 "+myShowTips(sMethod)+" > 资产抵、质押情况 <br>");
	...
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' class=td1 "+myShowTips(sMethod)+" > 其他资产情况分析 <br>");
	sTemp.append("	 </td>");
    sTemp.append("   </tr>");
    sTemp.append("   <tr>");
	sTemp.append("   <td colspan='15' align='left' class=td1 >");
