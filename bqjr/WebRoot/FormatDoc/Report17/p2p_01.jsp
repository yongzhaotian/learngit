...
<%
String sCreditID = "";//贷款人
String sStoreCityCode="";//城市
...
//String sDfile = "本人选择银行代扣还款，表明本人同意并授权贷款人可通过银行从本人指定的银行账户（如第49项所示）将每月还款额（如第39项所示）及其它应还款项转入指定还款账户（如第46项所示）。本人同意此扣款授权同时适用于之前已签订的一份或多份个人消费类贷款合同，即贷款人可通过银行从本人指定的上述银行账户内扣划本人在各合同下应偿还的相关款项。本人同意该账户同时可用于因提前还款等引起的资金往来。";
String sDfile = "本人选择银行代扣还款，表明本人同意并授权贷款人可通过银行从本人指定的银行账户（如第59项所示）将每月还款额（如第43项所示）及其它应还款项转入指定还款账户（如第56项所示）。本人同意此扣款授权同时适用于之前已签订的一份或多份个人消费类贷款合同，即贷款人可通过银行从本人指定的上述银行账户内扣划本人在各合同下应偿还的相关款项。本人同意该账户同时可用于因提前还款等引起的资金往来。";
...
ASResultSet rs2 = Sqlca.getResultSet(sSql);
	...
	//查销售顾问手机号
	String sMobileTel = Sqlca.getString(new SqlObject("select mobileTel from user_info where userId=:userId").setParameter("userId", sInputUserID));
	...
	String sCreditCycleFile = ""+bxcompanyName+"： 本人选择参加保险，即表明本人同意深圳市佰仟金融服务有限公司为本人投保“"+bxproductName+"”，并认可其为本人所投保的保险金额"+InsuranceDescription+"。本人指定深圳市佰仟金融服务有限公司为前述“"+bxproductName+"”合同项下身故和残疾保险金的第一受益人，受益金额为依分期购消费贷款三方协议约定仍未偿还的贷款本金。本人同时确认已熟知保险合同及相关条款, 且具备保险合同所约定的所有参保资格条件。";
	
	//String sCreditCycleFile = "民安财产保险有限公司深圳分公司：本人选择参加保险，即表明本人同意深圳市佰仟金融服务有限公司为本人投保“借款人人身意外伤害保险”，并通过身份证号码在民安财产保险有限公司网站上查询下载保险合同，认可其为本人所投保的保险金额。本人指定深圳市佰仟金融服务有限公司为前述“借款人人身意外伤害保险”合同项下保险金的受益人及/或第一受益人，受益金额为索赔当时本人依委托融资咨询服务协议的约定仍未偿还的贷款本金余额。本人同时确认已熟知保险合同及相关条款, 且具备保险合同所约定的所有参保资格条件";
...
<%
	...
	sTemp.append("<table class=table1 align=center  border=0 cellspacing=0 cellpadding=0  width='910' >	");
...
/* 	sTemp.append("<tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >销售点代码："+sStores+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >10.手机：<br>"+sMobileTelephone+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >51.印花税（%）："+sSTAMPTAX+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >52.是否投保："+sCreditCycleName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >53.保险公司名称："+bxcompanyName+"</td>");
	
	sTemp.append("</tr>");

	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 style='height:30px;' >54.是否购买随心还服务包："+sBugPayPkgind+"&nbsp;</td>");
	sTemp.append("<td colspan=8 align=left class=td1 >55.月随心还服务费(元)："+bugPayPkgindFee+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >56.指定还款账户账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentNo+"&nbsp;<br></td>");
	sTemp.append("<td colspan=4 align=left class=td1 >57.开户银行：<br>"+sRepaymentBank+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >58.户名："+sRepaymentName+"&nbsp;</td>");
	//sTemp.append("<td colspan=2 align=left class=td1 style='border-left-style: none;' ></td>");
	sTemp.append("</tr>");
	
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >59.客户银行卡号/账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceAccount+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >60.客户开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sOpenBranch+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >61.客户银行账户名：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >62.银行代扣还款："+repaymentWay+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >63.单位/学校/个体全称："+sWorkCorp+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >64.任职部门/班级："+sEmployRecord+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >65.职位："+sHeadShip+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >66.行业类别："+sIndustryType+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >67.单位性质："+sCellProperty+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >68.单位电话："+sWorkTel+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >69.单位地址："+sWorkAdd+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >70.镇/乡："+sUnitCountryside+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >71.街道/村："+sUnitStreet+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >72.小区/楼盘："+sUnitRoom+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >73.栋/单元/房间号："+sUnitNo+"</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >74.邮寄地址："+sFlag8+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >75.总工作经验/大学学习时间(年)："+sJobTotal+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >76.现单位工作/个体营业时间/距离毕业时间(月)："+sJobTime+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >77.在现单位是否购买社保："+sFlag4+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >78.配偶姓名："+sSpouseName+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >79.配偶移动电话："+sSpouseTel+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >80.配偶单位名称："+sSpouseWorkCorp+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >81.配偶单位电话："+sSpouseWorkTel+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >82.家庭成员名称："+sKinshipName+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >83.家庭成员类型："+sRelativeType+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >84.家庭成员电话："+sKinshipTel+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >85.家庭成员联系地址："+sKinshipAdd+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=2 align=left class=td1 >86.联系人姓名："+sOtherContact+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >87.与申请人关系："+sRelativeType1+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >88.联系电话："+sContactTel+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >89.月收入总额（元）："+sZE+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >90.其他收入（元/月）："+sOtherRevenue+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >91.家庭月收入（元）：<br>&nbsp;&nbsp;"+sFamilyMonthIncome+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >92.个人支出（元/月）："+sCE+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=10 align=left class=td1 ><font style='FONT-FAMILY:宋体; font-size: 8pt;' >本人具有完全民事行为能力签署本申请表并承担相应责任。本人已仔细阅读并完全了解《委托融资咨询服务协议》，并且自愿遵守相关的协议约定。<br>本人保证在此表上填写、确认的内容及向贷款人、佰仟金融提供的所有相关资料全部真实、有效，如本人提供虚假资料，将承担由此引起的一切责任及损失。本人在此同意贷款人和佰仟金融按照相关法律法规的规定，向有权机构报送本人的个人信用信息，包括但不限于本申请表项下的信息，同时，授权贷款人和佰仟金融向任何可能的来源查询本人及配偶的与贷款申请有关的个人信息和个人信用信息，相关查询结果的用途将用于贷款审查和贷款管理。<br>本人同意如因单方面取消申请或不具备借款条件，申请不获批准，本表格及所有已提交的资料无须退还本人，由佰仟金融处理。贷款人有权拒绝本贷款申请而无须给予任何原因解释。</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr style='height:50px;' >");
	sTemp.append("<td colspan=1 rowspan=2 align=left class=td1 >销售顾问姓名:&nbsp;"+sUserName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 rowspan=2 align=left class=td1 >销售顾问联系方式：&nbsp;"+sMobileTel+"&nbsp;</td>");
	...
	sTemp.append("</tr>");
	sTemp.append("</table>");	
	sTemp.append("<div>&nbsp;&nbsp;&nbsp;&nbsp;版本：SQ-XFD-P-20150608-V01</div>");
	sTemp.append("</div>");
	/* 
