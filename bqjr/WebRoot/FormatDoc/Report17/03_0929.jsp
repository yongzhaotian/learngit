...
ASResultSet rs2 = Sqlca.getResultSet(sSql);
	...
	String sSqlbx="SELECT sp.ServiceProvidersType as bxcompanyName,sp.ServiceProvidersName as bxproductName,sp.Casepercent as Cash FROM business_contract bc ,Service_Providers sp where bc.insuranceno=sp.serialno and bc.serialno='"+sObjectNo+"' ";
	ASResultSet rsbx = Sqlca.getResultSet(sSqlbx);
	String bxcompanyName="",bxproductName = "",Cash="";
	if(rsbx.next()){
	...
	if(!"1".equals(sCreditCycle)) sCreditCycleFile = "&nbsp;";
%>
...
/* 	sTemp.append("<tr>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >49.月财务管理费率（%）："+sMANAGEMENTFEESRATE+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >50.月增值服务费率</br>（%）："+sCreditFeeRate+"&nbsp;</td>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >52.指定还款账户账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentNo+"&nbsp;<br></td>");
	sTemp.append("<td colspan=4 align=left class=td1 >53.开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;招商银行深圳四海支行 &nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >54.户名："+sRepaymentName+"&nbsp;</td>");
	//sTemp.append("<td colspan=2 align=left class=td1 style='border-left-style: none;' ></td>");
	sTemp.append("</tr>");
	
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >55.客户银行卡号/账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceAccount+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >56.客户开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sOpenBranch+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >57.客户银行账户名：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >58.银行代扣还款："+repaymentWay+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >59.单位/学校/个体全称："+sWorkCorp+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >60.任职部门/班级："+sEmployRecord+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >61.职位："+sHeadShip+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >62.行业类别："+sIndustryType+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >63.单位性质："+sCellProperty+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >64.单位电话："+sWorkTel+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >65.单位地址："+sWorkAdd+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >66.镇/乡："+sUnitCountryside+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >67.街道/村："+sUnitStreet+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >68.小区/楼盘："+sUnitRoom+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >69.栋/单元/房间号："+sUnitNo+"</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >70.邮寄地址："+sFlag8+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >71.总工作经验/大学学习时间(年)："+sJobTotal+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >72.现单位工作/个体营业时间/距离毕业时间(月)："+sJobTime+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >73.在现单位是否购买社保："+sFlag4+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >74.配偶姓名："+sSpouseName+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >75.配偶移动电话："+sSpouseTel+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >76.配偶单位名称：</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >77.配偶单位电话：</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >78.家庭成员名称："+sKinshipName+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >79.家庭成员类型："+sRelativeType+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >80.家庭成员电话："+sKinshipTel+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >80.家庭成员联系地址："+sKinshipAdd+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=2 align=left class=td1 >82.联系人姓名："+sOtherContact+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >83.与申请人关系："+sRelativeType1+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >84.联系电话："+sContactTel+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=10 align=center class=td1 bgcolor=#aaaaaa ><b>收入及支出信息</b>&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >85.月收入总额（元）："+sZE+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >86.其他收入（元/月）："+sOtherRevenue+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >87.家庭月收入（元）：<br>&nbsp;&nbsp;"+sFamilyMonthIncome+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >88.个人支出（元/月）："+sCE+"</td>");
	sTemp.append("</tr>");
