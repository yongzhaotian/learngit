...
<%
	...
	sTemp.append(" </tr>");
	sTemp.append(" <tr>");	
	sTemp.append(" <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >2.2、在我行的授信业务信息</font></td>"); 	
	sTemp.append(" </tr>");
	sTemp.append(" <tr>");
  	sTemp.append(" <td width=22% align=center class=td1 > 授信业务品种</td>");
    sTemp.append(" <td width=18% align=center class=td1 > 余额（万） </td>");
    sTemp.append(" <td width=10% align=center class=td1 > 币种 </td>");
    sTemp.append(" <td width=22% align=center class=td1 > 保证金比例％</td>");
    sTemp.append(" <td width=16% align=center class=td1 > 月利率‰ </td>");
	...
	{	
		...
		dBalance += rs2.getDouble(2)/10000;
		sBalance = DataConvert.toMoney(rs2.getDouble(2)/10000);
		...
		sTemp.append(" <td width=18% align=right class=td1 >"+sBalance+"&nbsp;</td>");
	...
	sSql = " select getItemName('Currency',BusinessCurrency) as CurrencyName,sum(Balance)"+
           " from BUSINESS_CONTRACT "+
           " where CustomerID = '"+sCustomerID+"' and Balance>0 and DeleteFlag is null and (FinishDate = ' ' or FinishDate is null) group by BusinessCurrency"; 
    ...
    sTemp.append(" <td width=18% align=right class=td1 >"+DataConvert.toMoney(dBalance)+"&nbsp;</td>");
