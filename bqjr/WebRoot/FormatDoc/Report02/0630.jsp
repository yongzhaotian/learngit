...
<%
	...
	sTemp.append("<form method='post' action='0630.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >6.2、抵押方式</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
	...
	sTemp.append("   </tr>");
	sTemp.append("   <tr align=left>");
  	sTemp.append("   <td colspan=4 class=td1 >描述抵押物是否有财产共有人？共有人是否出具同意抵押的书面文件？<br></td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr align=left>");
  	sTemp.append("   <td colspan=4 class=td1 ><p>多次或分割抵押状况</p>");
  	sTemp.append("     <p>1、若以同一抵押物设定若干抵押的，须提交已告知各债权人的书面证明，并提供将来对债权人的清偿顺序；</p>");
  	sTemp.append("     <p>2、若抵押物为整体建筑的一部分，应介绍整体建筑情况和其他房产权属情况；</p>");
  	sTemp.append("     <p>3、若以前做过抵押，应介绍当时抵押情况，债务是否结清。 </p>");
  	sTemp.append(myOutPut("1",sMethod,"name='describe1' style='width:100%; height:150'",getUnitData("describe1",sData)));
	sTemp.append("   <br>");
	sTemp.append("   </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
  	sTemp.append("   <td colspan=4 align=left class=td1 ><p>抵押物的附属情况分析</p>");
  	sTemp.append("     <p>抵押物的附属物名称、面积、性质（是否划拨土地或是否公益性建筑）。 </p><br>");
  	sTemp.append(myOutPut("1",sMethod,"name='describe2' style='width:100%; height:150'",getUnitData("describe2",sData)));
	sTemp.append("   <br>");
	sTemp.append("   </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
  	sTemp.append("   <td colspan=4 align=left class=td1 ><p>抵押物价值稳定性分析</p>");
  	sTemp.append("     <p>对抵押物的成新率(若厂房须考虑折旧率)、地段、市场价格波动性进行分析，抵押物的预期价值上涨还是下降趋势，是否能覆盖贷款金额、不致产生缺口。 </p><br>");
  	sTemp.append(myOutPut("1",sMethod,"name='describe3' style='width:100%; height:150'",getUnitData("describe3",sData)));
	sTemp.append("   <br>");
	sTemp.append("     </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr align=left>");
  	sTemp.append("   <td colspan=4 class=td1 ><p>抵押物变现能力分析</p>");
  	sTemp.append("     <p>对抵押物的市场变现能力做出预测分析：是否可以迅速的出售，市场对抵押物的需求大小，变现清偿时是否会产生一些法律及销售费用等。</p><br>");
  	sTemp.append(myOutPut("1",sMethod,"name='describe4' style='width:100%; height:150'",getUnitData("describe4",sData)));
	sTemp.append("   <br>");
	sTemp.append("     </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr align=left>");
  	sTemp.append("   <td colspan=4 class=td1 ><p>对抵押物的法律控制能力</p>");
  	sTemp.append("     <p>描述抵押物的来源如何，产权是否明晰，有无法律纠纷，是否拥有优先受偿权。</p>");
  	sTemp.append(myOutPut("1",sMethod,"name='describe5' style='width:100%; height:150'",getUnitData("describe5",sData)));
	sTemp.append("   <br>");
  	sTemp.append("     </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr align=left>");
  	sTemp.append("   <td colspan=4 class=td1 ><p>其他情况分析</p>");
  	sTemp.append("     <p>对抵押物其他未尽事项，进行分析。</p>"); 
