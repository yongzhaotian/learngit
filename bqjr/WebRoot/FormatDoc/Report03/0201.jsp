...
<%
	...
	sTemp.append("   <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >2、申请人基本信息</font></td>"); 	
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >2.1、关联关系（单位：万元）</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
    ...
    ASResultSet rs2 = Sqlca.getResultSet("select CustomerName,CertID,getItemName('RelationShip',RelationShip) as RelationShipName,"
							+"OughtSum,InvestmentProp,InvestDate from CUSTOMER_RELATIVE "
							+"where RelationShip like '52%' and length(RelationShip)>2 and EffStatus='1' and CustomerID ='"+sCustomerID+"'");
	while(rs2.next())
	{
	...
	sTemp.append("  <tr>");
  	sTemp.append(" <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' > 申请人对外股权投资情况（合并报表范围内）</font>  </td>");
    sTemp.append("  </tr>");
	sTemp.append("  <tr>");
    ...
    rs2 = Sqlca.getResultSet("select CustomerName,CertID,InvestmentSum,InvestmentProp,InvestYield "
							...
							+"where RelationShip like '02%' and length(RelationShip)>2 and EffStatus='1' and CustomerID ='"+sCustomerID+"'");
	while(rs2.next())
	{
		sCustomerName = rs2.getString(1);
		if(sCustomerName == null) sCustomerName = " ";
	...
	sTemp.append("  <tr>");
  	sTemp.append(" <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' > 申请人其他集团客户信息  </font></td>");
    sTemp.append("  </tr>");
	sTemp.append("  <tr>");
    ...
    String sSql = "select customername,certid,getItemname('relationship',relationship) "+
		          "from customer_relative where customerid in "+
		          "(select relativeid from customer_relative "+
		          "where customerid='"+sCustomerID+"' and relationship='5401') "+
		          "and relativeid not in('"+sCustomerID+"') order by customername ";
	...
	sTemp.append("  <tr>");
  	sTemp.append(" <td class=td1 align=left colspan='6' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' > 申请人的集团关联企业在我行的授信情况  </font></td>");
    sTemp.append("  </tr>");
	sTemp.append("  <tr>");
    ...
    rs2 = Sqlca.getResultSet("select CustomerName,getOrgName(OperateOrgID) as OperateOrgName,"
								...
								+"(select relativeid from customer_relative "
								+"where customerid='"+sCustomerID+"' and relationship='5401') "
								+"and relativeid not in('"+sCustomerID+"') ) " 
