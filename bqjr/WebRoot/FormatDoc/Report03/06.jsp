...
<%
	...
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >6、授信申请用途</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
  	...
  	if(sBusinessType.equals("1020010"))
  		sTemp.append("     承兑汇票贴现：要求说明申请人和其直接前手或出票人之间的贸易关系（贸易标的、金额、付款条件、是否有交易合同、增值税发票、发运单据）；汇票的承兑机构、承兑金额、票据号码、到期日。检查汇票是否经背书转让，背书是否连续。检查汇票、交易合同、增值税发票上的交易合同号码是否一致，日期、金额等要素是否匹配。对于缺少增值税发票复印件的，要求说明原因。");
