...
<%
	...
	String sRowName[] = {"净利润增长率","销售利润率","总资产报酬率","净资产收益率","成本费用利润率"};
	String sCol2Value[]={"","","","",""};
	String sCol2Value1[]={"","","","",""};
	String sCol2Value2[]={"","","","",""};
	
	String sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9'"
				  +" and reportscope in (select min(reportscope) from report_record where reportdate = '"+sYearN+"' and objectno = '"+sCustomerID+"'))";
	ASResultSet rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("RowName").trim();
	...
	sSql = " select * from REPORT_DATA where reportno in(select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_1+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9'"
				  +" and reportscope in (select min(reportscope) from report_record where reportdate = '"+sYearN_1+"' and objectno = '"+sCustomerID+"'))";
	...
	sSql = " select * from REPORT_DATA where reportno in(select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_2+"' and objectno = '"+sCustomerID+"' and ModelNo like '%9'"
				  +" and reportscope in (select min(reportscope) from report_record where reportdate = '"+sYearN_2+"' and objectno = '"+sCustomerID+"'))";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("RowName").trim();
...
<%
	...
	sTemp.append("   <td class=td1 align=left colspan='4' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4、申请人财务分析 </font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan='4' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >4.1、盈利能力分析 </font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
	...
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
  	sTemp.append("   <td colspan='4' align=left class=td1 "+myShowTips(sMethod)+" > 分析：");
	sTemp.append("   </td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
