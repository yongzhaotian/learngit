...
String sDfile = "本人选择银行代扣还款，表明本人同意并授权贷款人和/或担保人可通过银行从本人指定的银行帐户（如第50、51、52项所示）将每月还款额（如第36项所示）及其它应还款项转入指定还款账户（如第47项、第48项及第49项所示）。本人同意此扣款授权同时适用于之前已签订的一份或多份个人消费类贷款合同，即贷款人和/或担保人可通过银行从本人指定的上述银行帐户内扣划本人在各合同下应偿还的相关款项。本人同意该账户同时可用于因提前还款等引起的资金往来。";
...
//学校信息  add by dyh 20150602
String school_name = "";//学校全称
String school_college = "";//所在学院
String school_department = "";//系         
String school_professional_name = "";//专业名称
String school_class = ""; //班级              
String school_student_no = ""; //学号         
String school_learning = ""; //学习形式           
String school_status_student = ""; //学籍状态  
String school_degree_category = "";//学历类别     
String school_length = "";//学制              
String school_level = ""; //层次            
String school_dormitory_telephone = "";//学院/宿舍电话 
String school_counselor_telephone = "";//辅导员电话 
String school_enrollment_date = "";//入学日期     
String school_expected_date = "";//预计毕业日期       
String school_address = "";//学校地址：省/直辖市             
String school_township = ""; //镇/乡           
String school_street = "";//街道/村              
String school_community = ""; //小区/楼盘          
String school_room_no = ""; //栋/单元/房间号            
String school_mailing_address = ""; //邮寄地址   

String monthly_income_Source = "";//每月收入来源
...
String sSql = "select bc.CreditID,bc.StoreCityCode,bc.SubProductType,bc.insurancesum as CreditFeeRate,bc.CreditCycle as CreditCycle,getTypeName(bc.BusinessRange3,bc.BusinessType3) as BusinessType3,bc.BrandType3 as BrandType3,bc.Manufacturer3 as Manufacturer3,bc.Price3 as Price3,bc.FIRSTDRAWINGDATE as FIRSTDRAWINGDATE,bc.CustomerID,bc.CustomerName,bc.stores,bc.inputuserid,bc.inputdate,bc.BusinessType,bc.BusinessSum,bc.InteriorCode,getstorename(bc.Stores) as StoresName, "+
				...
				" getusername(bc.Inputuserid) as UserName,getuserphone(bc.Inputuserid) as userPhone, SUBMITDATETIME as SUBMITDATETIME,suretype as suretype " + 
...
ASResultSet rs2 = Sqlca.getResultSet(sSql);
	if(rs2.next()){
		...
		sApplyType = rs2.getString("ApplyType1");
		...
		userPhone = rs2.getString("userPhone");
		...
		sureType= rs2.getString("suretype");
		...
		if(sureType == null) sManufacturer3 ="&nbsp;";
	...
	if(sRepaymentBank == null){
  	...
  	String sSql5="select ii.certID,ii.Issueinstitution,ii.MaturityDate,ii.sino,getitemname('EducationExperience',ii.EduExperience) as EduExperience,ii.FamilyTel,"
	           ...
	           +"ii.SpouseName,ii.SpouseTel,ii.SpouseWorkTel,ii.SpouseWorkCorp,ii.KinshipName,getItemname('FamilyRelativeMore',ii.RelativeType) as RelativeType, "
	           +"ii.KinshipTel,ii.KinshipAdd,ii.OtherContact,getItemname('RelationshipOther',ii.Contactrelation) as RelativeType1,"
	           +"ii.ContactTel,ii.OtherRevenue+ii.SelfMonthIncome as ZE,ii.OtherRevenue,ii.FamilyMonthIncome,"
	           +"nvl(ii.Alimony,0)+nvl(ii.Houserent,0) as CE,ii.monthly_income_Source,getitemname('FamilyRelativeMore', ii.family_parents_relations) as family_parents_relations, ii.family_parents_company,ii.family_parents_companytel,ii.family_parents_position,ii.spouse_address,ii.spouse_township,ii.spouse_street,ii.spouse_community,ii.spouse_room_no,getitemname('YesNo',ii.spouse_is_education) as spouse_is_education,ii.family_parents_name,ii.family_parents_telephone from ind_info ii where CustomerID='"+sObjectNo.substring(0, 8)+"'";
	...
	if(rs3.next()){
		...
		sEduExperience = (null == rs3.getString("EduExperience"))?"&nbsp;":rs3.getString("EduExperience");
		...
		monthly_income_Source = (null == rs3.getString("monthly_income_Source"))?"&nbsp;":rs3.getString("monthly_income_Source");
		family_parents_relations= (null == rs3.getString("family_parents_relations"))?"&nbsp;":rs3.getString("family_parents_relations");
		family_parents_company= (null == rs3.getString("family_parents_company"))?"&nbsp;":rs3.getString("family_parents_company");
		family_parents_companytel= (null == rs3.getString("family_parents_companytel"))?"&nbsp;":rs3.getString("family_parents_companytel");
		family_parents_position= (null == rs3.getString("family_parents_position"))?"&nbsp;":rs3.getString("family_parents_position");
		spouse_address= (null == rs3.getString("spouse_address"))?"&nbsp;":rs3.getString("spouse_address");
		spouse_township= (null == rs3.getString("spouse_township"))?"&nbsp;":rs3.getString("spouse_township");
		spouse_street= (null == rs3.getString("spouse_street"))?"&nbsp;":rs3.getString("spouse_street");
		spouse_community= (null == rs3.getString("spouse_community"))?"&nbsp;":rs3.getString("spouse_community");
		spouse_room_no= (null == rs3.getString("spouse_room_no"))?"&nbsp;":rs3.getString("spouse_room_no");
		spouse_is_education= (null == rs3.getString("spouse_is_education"))?"&nbsp;":rs3.getString("spouse_is_education");
		family_parents_name= (null == rs3.getString("family_parents_name"))?"&nbsp;":rs3.getString("family_parents_name");
		family_parents_telephone= (null == rs3.getString("family_parents_telephone"))?"&nbsp;":rs3.getString("family_parents_telephone");
	...
	String sSql8="select ssi.school_name,ssi.school_college,ssi.school_department,ssi.school_professional_name,ssi.school_class,ssi.school_student_no,ssi.school_learning,ssi.school_status_student,ssi.school_degree_category,ssi.school_length,getItemName('EducationExperience', ssi.school_level) as school_level,ssi.school_dormitory_telephone,"+
			"ssi.school_counselor_telephone,ssi.school_enrollment_date,ssi.school_expected_date,getItemName('AreaCode',SSI.school_address) as school_address,ssi.school_township,ssi.school_street,ssi.school_community,ssi.school_room_no,ssi.school_mailing_address from student_school_info ssi where ssi.customerid='"+sObjectNo.substring(0, 8)+"'";
	ASResultSet rs8 = Sqlca.getResultSet(sSql8);
	  if(rs8.next()){
		  school_name = (null == rs8.getString("school_name"))?"&nbsp;":rs8.getString("school_name");      
		  school_college = (null == rs8.getString("school_college"))?"&nbsp;":rs8.getString("school_college");     
		  school_department = (null == rs8.getString("school_department"))?"&nbsp;":rs8.getString("school_department");          
		  school_professional_name = (null == rs8.getString("school_professional_name"))?"&nbsp;":rs8.getString("school_professional_name");   
		  school_class = (null == rs8.getString("school_class"))?"&nbsp;":rs8.getString("school_class");             
		  school_student_no = (null == rs8.getString("school_student_no"))?"&nbsp;":rs8.getString("school_student_no");         
		  school_learning = (null == rs8.getString("school_learning"))?"&nbsp;":rs8.getString("school_learning");          
		  school_status_student = (null == rs8.getString("school_status_student"))?"&nbsp;":rs8.getString("school_status_student");     
		  school_degree_category = (null == rs8.getString("school_degree_category"))?"&nbsp;":rs8.getString("school_degree_category");     
		  school_length = (null == rs8.getString("school_length"))?"&nbsp;":rs8.getString("school_length");              
		  school_level = (null == rs8.getString("school_level"))?"&nbsp;":rs8.getString("school_level");
		  school_dormitory_telephone = (null == rs8.getString("school_dormitory_telephone"))?"&nbsp;":rs8.getString("school_dormitory_telephone"); 
		  school_counselor_telephone = (null == rs8.getString("school_counselor_telephone"))?"&nbsp;":rs8.getString("school_counselor_telephone");
		  school_enrollment_date = (null == rs8.getString("school_enrollment_date"))?"&nbsp;":rs8.getString("school_enrollment_date");     
		  school_expected_date = (null == rs8.getString("school_expected_date"))?"&nbsp;":rs8.getString("school_expected_date");       
		  school_address = (null == rs8.getString("school_address"))?"&nbsp;":rs8.getString("school_address");           
		  school_township = (null == rs8.getString("school_township"))?"&nbsp;":rs8.getString("school_township");           
		  school_street = (null == rs8.getString("school_street"))?"&nbsp;":rs8.getString("school_street");              
		  school_community = (null == rs8.getString("school_community"))?"&nbsp;":rs8.getString("school_community");          
		  school_room_no = (null == rs8.getString("school_room_no"))?"&nbsp;":rs8.getString("school_room_no");            
		  school_mailing_address = (null == rs8.getString("school_mailing_address"))?"&nbsp;":rs8.getString("school_mailing_address"); 
	}
...
/* 	sTemp.append("<tr>");
	...
	if("EBF".equals(sureType)){
		sTemp.append("<td class=td1 rowspan=2 align=center colspan=7 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >易佰分-分期购服务申请表</font></td> ");	
	}else{
		sTemp.append("<td class=td1 rowspan=2 align=center colspan=7 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >学生专享优惠分期申请表</font></td> ");	
	}
	...
	sTemp.append("<tr>");
	//sTemp.append("<td colspan=3 align=left class=td1 >合同编号："+sObjectNo+"&nbsp;申请日期："+sInputDate+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >合同编号："+sObjectNo+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >申请类别：学生消费贷&nbsp;</td>");
	...
	sTemp.append("<td colspan=1 rowspan=4 align=left class=td1 ><div><img src='"+xcPage+"' style='width:120px;height:120px;'/></div></td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 style='height:30px;' >9.现居住地址是否与户籍地址相同："+sFlag2+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >12.街道/村："+sStreet+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >13.小区/楼盘："+sCommunity+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >14.栋/单元/房间号："+sCellNo+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >17.街道/村："+sVillagecenter+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >18.小区/楼盘："+sPlot+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >19.栋/单元/房间号："+sRoom+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=1 align=left class=td1 >20.商品类型(1)："+sBusinessType1+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >21.品牌(1)："+sBrandType1+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >22.商品型号(1)："+sManufacturer1+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >23.价格(1)(元)："+sPrice1+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=1 align=left class=td1 >24.商品类型(2)："+sBusinessType2+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >25.品牌(2)："+sBrandType2+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >26.商品型号(2)："+sManufacturer2+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >27.价格(2)(元)："+sPrice2+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=1 align=left class=td1 >28.商品类型(3)："+sBusinessType3+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >29.品牌(3)："+sBrandType3+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >30.商品型号(3)："+sManufacturer3+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >31.价格(3)(元)："+sPrice3+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >32.商品总价（元）："+sTotalPrice+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >33.自付金额（元）："+sTotalSum+"&nbsp;</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >34.贷款本金（元）：<br>"+sBusinessSum+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >37.每月还款日："+sDefaultDueDay+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >38.首次还款额(元)："+dFirstIntrestAmt+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >39.首次还款日："+sFirstDueDate+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >43.月增值服务费率（%）："+sCreditFeeRate+"&nbsp;</td>");
	//sTemp.append("<td colspan=2 align=left class=td1 >44.印花税（%）："+sSTAMPTAX+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >44.随心还服务费(元)："+bugPayPkgindFee+"&nbsp;&nbsp;是否选择随心还："+sBugPayPkgind+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >45.是否投保："+sCreditCycleName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >46.保险公司名称："+bxcompanyName+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >47.指定还款账户账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentNo+"&nbsp;<br></td>");
	sTemp.append("<td colspan=5 align=left class=td1 >48.开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentBank+" &nbsp;</td>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >50.客户银行卡号/账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceAccount+"&nbsp;</td>");
	sTemp.append("<td colspan=5 align=left class=td1 >51.客户开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sOpenBranch+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >52.客户银行账户名：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sReplaceName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >53.银行代扣还款："+repaymentWay+"&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	...
	sTemp.append("<td colspan=1 align=left class=td1 >55.所在学院："+school_college+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >56.系："+school_department+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >57.专业名称："+school_professional_name+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=1 align=left class=td1 >62.学历类别："+school_degree_category+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >65.学院/宿舍电话："+school_dormitory_telephone+"</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >70.镇/乡："+school_township+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >71.街道/村："+school_street+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >72.小区/楼盘："+school_community+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >73.栋/单元/房间号："+school_room_no+"</td>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >75.家庭成员姓名："+sKinshipName+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >76.与申请人关系："+sRelativeType+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >77.家庭成员联系电话："+sKinshipTel+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >78.家庭固话："+sFamilyTel+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >79.家庭成员单位名称："+family_parents_company+"</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >80.家庭成员单位电话："+family_parents_companytel+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >81.家庭成员职位："+family_parents_position+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >82.家庭成员现居住地址：省/直辖市："+"&nbsp;"+sKinshipAdd+"&nbsp;</td>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >87.家庭成员是否知道贷款："+spouse_is_education+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >88.其他亲属姓名："+family_parents_name+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >89.与申请人关系："+family_parents_relations+"&nbsp;</td>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=2 align=left class=td1 >91.联系人姓名："+sOtherContact+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >92.与申请人关系："+sRelativeType1+"</td>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >94.月收入总额（元）："+sZE+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >95.每月收入来源："+monthly_income_Source+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >96.其他收入（元）："+sOtherRevenue+"</td>");
	...
	sTemp.append("<td colspan=1 rowspan=2 align=left class=td1 >销售顾问姓名:&nbsp;"+sUserName+"&nbsp;</td>");
	sTemp.append("<td colspan=2 rowspan=2 align=left class=td1 >销售顾问联系方式:"+userPhone+"</td>");
	sTemp.append("<td colspan=4 rowspan=2 align=left class=td1 >销售顾问签名:&nbsp;</td>");
	sTemp.append("<td colspan=2 rowspan=2 align=left class=td1 >申请人签名:&nbsp;</td>");
	sTemp.append("<td colspan=1 rowspan=2 align=left class=td1 >日期:&nbsp;"+sInputDate+"</td>");
	sTemp.append("</tr>");
	sTemp.append("</table>");
	sTemp.append("<div align='left'>");
	sTemp.append("<font size= '2pt' ><b>&nbsp;&nbsp;版本号: J_XF_XS_ZX_2016030802 </b></font>");
	sTemp.append("</div>");	
	sTemp.append("</div>");
	/* 
