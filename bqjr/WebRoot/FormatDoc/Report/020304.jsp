...
<%
	...
	sTemp.append("<form method='post' action='020304.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	//colspan需根据实际情况调整，否则导出时会比较不雅
	sTemp.append("   <td class=td1 align=left colspan='5' bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >2.3.4、在他行的担保信息（单位：万元）</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
	...
	while(rs2.next())
	{
		...
		sBusinessSum = DataConvert.toMoney(rs2.getDouble(2));
	...
	sSql = "select getitemname('Currency',Currency) as CurrencyName,sum(BusinessSum) "+
           "from CUSTOMER_OACTIVITY where BusinessType = '080' and CustomerID = '"+sCustomerID+"' group by Currency";
    ...
    sTemp.append("   <td colspan=4 align=left class=td1 >"+sSum+"&nbsp;</td>");
    sTemp.append("   </tr>");
