...
<%
	...
	sSql = " select * from REPORT_DATA where ReportNo in (select ReportNo from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN+"')";
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN_1+"')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("Rowsubject");
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"+
		   " where ObjectNo = '"+sGuarantyNo+"' and ModelNo like '%9' and reportdate = '"+sYearN_2+"')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("Rowsubject");
...
<%
	...
	sTemp.append("<form method='post' action='06010803.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=20 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >"+sNo1[j]+".8.3、偿债能力分析</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
