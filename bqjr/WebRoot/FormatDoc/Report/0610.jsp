...
<%
	...
	rs2.getStatement().close();
	sTreeNo = sTreeNo.replaceAll("0","");
	for(int k=0;k<sTreeNo.length();k++)
		sTreeNo += sTreeNo.substring(k,k+1)+".";
	sTreeNo = sTreeNo.substring(0,sTreeNo.length()-1);
	
	String sContractType = "";		//合同类型
	String sGuarantyType = "";		//担保类型
	String sCreditOrgName = "";		//投保人
	String sGuarantyInfo = "";		//受益人
	String sGuarantorName = "";		//承保保险公司
	...
	sSql = "select getItemName('ContractType',ContractType) as ContractType,"+
		   ...
		   "GuarantyInfo,GuarantorName,getItemName('Currency',GuarantyCurrency) as GuarantyCurrency,"+
	...
	if(rs2.next())
	{
		...
		sGuarantyInfo = rs2.getString("GuarantyInfo");
...
<%
	...
	sTemp.append("<form method='post' action='0610.jsp' name='reportInfo'>");	
	sTemp.append("<div id=reporttable>");	
	sTemp.append("<table class=table1 width='640' align=center border=1 cellspacing=0 cellpadding=2 bgcolor=white bordercolor=black bordercolordark=black >	");
	sTemp.append("   <tr>");	
	sTemp.append("   <td class=td1 align=left colspan=13 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >6.10、履约保险保证</font></td>"); 	
	sTemp.append("   </tr>");	
	sTemp.append("   <tr>");
	sTemp.append("   <td width=15% align=left class=td1 > 合同类型</td>");
	sTemp.append("   <td colspan=3 align=left class=td1 >"+sContractType+"&nbsp;</td>");
	sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td align=left class=td1 > 担保类型</td>");
    sTemp.append("   <td colspan=3 align=left class=td1 >"+sGuarantyType+"&nbsp;</td>");
    sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td align=left class=td1 > 投保人</td>");
    sTemp.append("   <td colspan=3 align=left class=td1 >"+sCreditOrgName+"&nbsp; </td>");
    sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td align=left class=td1 > 受益人</td>");
    sTemp.append("   <td colspan=3 align=left class=td1 >"+sGuarantyInfo+"&nbsp; </td>");
    sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td align=left class=td1 > 承保保险公司</td>");
    ...
    sTemp.append("   </tr>");
	sTemp.append("   <tr>");
	sTemp.append("   <td align=left class=td1 > 保险金额</td>");
