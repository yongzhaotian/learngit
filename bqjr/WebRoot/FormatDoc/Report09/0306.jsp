...
<%
	...
	String sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN+"' and objectno = '"+sCustomerID+"' and ReportName ='财务指标表')";
	//out.println(sSql);
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_1+"' and objectno = '"+sCustomerID+"' and ReportName ='财务指标表')";
	...
	sSql = " select * from REPORT_DATA where reportno in (select reportno from REPORT_RECORD"
				  +" where reportdate = '"+sYearN_2+"' and objectno = '"+sCustomerID+"' and ReportName ='财务指标表')";
	rs2 = Sqlca.getResultSet(sSql);
	while(rs2.next())
	{
		String RowName = rs2.getString("RowSubject");
