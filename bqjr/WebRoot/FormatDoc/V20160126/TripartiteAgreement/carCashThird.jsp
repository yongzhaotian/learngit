...
<%
	...
	/* double sumFee = 0.00;//费用总和
	double monthRepayment = 0.00; // 每月还款额
	double sumamount = 0.00;//利息加本金之和 = 每月还款额 - 费用总和 
	int periods = 0; */
	BigDecimal sumFee = new BigDecimal(0.00);
	BigDecimal feeRate = new BigDecimal(0.00);
	BigDecimal businessSum = new BigDecimal(0.00);
	BigDecimal monthRepayment = new BigDecimal(0.00);
	BigDecimal sumamount = new BigDecimal(0.00);
	BigDecimal periods = new BigDecimal(0);
%>

<%@include file="/FormatDoc/IncludePOHeader.jsp"%>
<%
	//获得调查报告数据
	String sql = "SELECT BC.MONTHREPAYMENT, BC.BUSINESSSUM, B.FEERATE, BC.PERIODS FROM BUSINESS_CONTRACT BC INNER JOIN ACCT_FEE B ON B.OBJECTNO = BC.SERIALNO " 
			+ "AND B.OBJECTTYPE = 'jbo.app.BUSINESS_CONTRACT' WHERE BC.SERIALNO = :SERIALNO";
	ASResultSet rs = Sqlca.getResultSet(new SqlObject(sql).setParameter("SERIALNO", sObjectNo));
	while (rs.next()) {
		feeRate = rs.getBigDecimal("FEERATE") == null ? new BigDecimal(0.00) : rs.getBigDecimal("FEERATE");
		periods = rs.getBigDecimal("PERIODS") == null ? new BigDecimal(0) : rs.getBigDecimal("PERIODS");
		businessSum = rs.getBigDecimal("BUSINESSSUM") == null 
				? new BigDecimal(0.00) : rs.getBigDecimal("BUSINESSSUM");
		monthRepayment = rs.getBigDecimal("MONTHREPAYMENT") == null 
				? new BigDecimal(0.00) : rs.getBigDecimal("MONTHREPAYMENT");
		sumFee = sumFee.add(feeRate.multiply(businessSum).divide(periods.multiply(new BigDecimal(100))));
	}
	rs.getStatement().close();
	sumamount = monthRepayment.subtract(sumFee);
	BigDecimal db = sumamount.setScale(2, BigDecimal.ROUND_UP);
%>
