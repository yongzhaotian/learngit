...
String sDfile = "";
String sDfile1 = "本人选择银行代扣还款，表明本人同意并授权深圳市佰仟金融服务有限公司可通过银行从本人指定的借款人银行账户（如第43项所示）将每月还款额（如第29项所示）及其它应还款项转入指定还款账户（如第40、41、42项所示）。本人同意此扣款授权同时适用于之前由深圳市佰仟金融服务有限公司提供服务并已签订的一份或多份贷款合同，即深圳市佰仟金融服务有限公司可通过银行从本人指定的上述银行账户内划扣本人在各合同下应偿还的相关款项。本人同意该账户同时可用于因提前还款等引起的资金往来。";
String sUserName = "";
...
String sSql = "select bc.insurancesum as CreditFeeRate,bc.CreditCycle as CreditCycle,bc.BusinessType3 as BusinessType3,bc.BrandType3 as BrandType3,bc.Manufacturer3 as Manufacturer3,bc.Price3 as Price3,bc.FIRSTDRAWINGDATE as FIRSTDRAWINGDATE,bc.CustomerID,bc.CustomerName,bc.stores,bc.inputuserid,bc.inputdate,bc.BusinessType,getitemname('CashPurpose',bc.Purpose) as Purpose,bc.BusinessSum,bc.InteriorCode,getstorename(bc.Stores) as StoresName, "+
				...
				" getitemname('SubProductType',bc.subproducttype) as ApplyType,"+ //--update 修改申请类别为产品自类别  tangyb 20150909
...
/* 	sTemp.append("<tr>");
	...
	sTemp.append("<td class=td1 rowspan=2 align=center colspan=7 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >个人现金分期贷款申请表</font></td> ");	
	...
	sTemp.append("<td colspan=3 align=left class=td1 >30.是否申请参加保险："+("是".equals(sCreditCycleName)?"是■&nbsp;&nbsp;&nbsp;否□":"是□&nbsp;&nbsp;&nbsp;否■")+"</td>");//update tangyb
	...
	sTemp.append("<td colspan=3 align=left class=td1 >34.首次还款额:"+dFirstIntrestAmt+"</td>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >38.月增值服务费率:"+sCreditFeeRate+"（%）</td>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >40.指定还款账户账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentNo+"&nbsp;<br></td>");
	sTemp.append("<td colspan=6 align=left class=td1 >41.开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;招商银行股份有限公司深圳安联支行 &nbsp;</td>");//update CCS-399(消费贷默认显示为”四海支行“，现金贷默认显示为”安联支行  “;)
	sTemp.append("<td colspan=3 align=left class=td1 >42.户名：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentName+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=9 align=left class=td1 >43.借款人银行账户信息（用于收款、扣款）：<br>&nbsp;&nbsp;&nbsp;&nbsp;户名：&nbsp;"+sReplaceName+"&nbsp;&nbsp;开户行：&nbsp;"+sOpenBranch+"&nbsp;&nbsp;账号： &nbsp;"+sReplaceAccount+"&nbsp;&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >44.银行代扣还款 ：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+("1".equals(repaymentWay)?"是■&nbsp;&nbsp;&nbsp;否□":"是□&nbsp;&nbsp;&nbsp;否■")+"</td>");//update tangyb
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >45.单位/个体全称："+sWorkCorp+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=2 align=left class=td1 >48.行业类别："+sIndustryType+"&nbsp;</td>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >51.单位地址：省/直辖市："+sWorkAdd+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >54.小区/楼盘："+sUnitRoom+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >55.栋/单元/房间号："+sUnitNo+"&nbsp;&nbsp;&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >56.邮寄地址：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+("1".equals(sFlag8)?"居住地址■&nbsp;&nbsp;&nbsp;单位地址□":"居住地址□&nbsp;&nbsp;&nbsp;单位地址■")+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >57.总工作经验（年）："+sJobTotal+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >58.现单位工作/个体营业时间(月)："+sJobTime+"</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >59.在现单位是否购买社保："+sFlag4+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >60.配偶姓名："+sSpouseName+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >61.配偶移动电话："+sSpouseTel+"</td>");
	sTemp.append("<td colspan=5 align=left class=td1 >62.配偶单位名称："+sSpouseWorkCorp+"</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >63.配偶单位电话："+sSpouseWorkTel+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >64.家庭成员名称："+sKinshipName+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >65.家庭成员类型："+sRelativeType+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >66.家庭成员电话："+sKinshipTel+"</td>");
	...
	sTemp.append("<td colspan=1 align=left class=td1 >68.联系人姓名："+sOtherContact+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >69.与申请人关系："+sRelativeType1+"</td>");
	sTemp.append("<td colspan=6 align=left class=td1 >70.联系电话："+sContactTel+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=10 align=center class=td1 bgcolor=#aaaaaa ><b>收入及支出信息</b>&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=1 align=left class=td1 >71.月收入总额（元）："+sZE+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >72.其他收入（元/月）："+sOtherRevenue+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >73.家庭月收入（元）：<br>&nbsp;&nbsp;"+sFamilyMonthIncome+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >74.个人月支出（元/月）："+sCE+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=10 align=left class=td1 ><font style='FONT-FAMILY:宋体; font-size: 8pt;' >本人具有完全民事行为能力签署本申请表并承担相应责任。本人已仔细阅读并完全了解《个人现金分期贷款三方协议》，并且自愿遵守相关的协议规定。<br>本人保证在此表上填写、确认的内容及向贷款人、佰仟金融提供的所有相关资料全部真实、有效，如本人提供虚假资料，将承担由此引起的一切责任及损失。本人在此同意贷款人和佰仟金融按照相关法律法规的规定，向有权机构报送本人的个人信用信息，包括但不限于本申请表项下的信息，同时，授权贷款人和佰仟金融向任何可能的来源查询本人及配偶的与贷款申请有关的个人信息和个人信用信息，相关查询结果的用途将用于贷款审查和贷款管理。<br>本人同意如因单方面取消申请或不具备借款条件，申请不获批准，本表格及所有已提交的资料无须退还本人，由佰仟金融处理。贷款人有权拒绝本贷款申请而无须给予任何原因解释。</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<td colspan=10 rowspan=2 align=left class=td1 >销售顾问姓名:"+sUserName+"&nbsp;销售顾问签名:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;销售顾问联系方式:"+sMobileNum+"&nbsp;&nbsp;申请人签名:&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+year+"年"+month+"月"+day+"日&nbsp</td>");
	sTemp.append("</tr>");
	sTemp.append("</table>");	
	sTemp.append("</div>");
