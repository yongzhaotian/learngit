...
String sDfile1 = "本人选择银行代扣还款，表明本人同意并授权贷款人可通过银行从本人指定的借款人银行账户（如第44项所示）将每月还款额（如第29项所示）及其它应还款项转入指定还款账户（如第41项所示）。本人同意此扣款授权同时适用于之前由佰仟金融提供服务并已签订的一份或多份贷款合同，即贷款人可通过银行从本人指定的上述银行账户内扣划本人在各合同下应偿还的相关款项。本人同意该账户同时可用于因提前还款等引起的资金往来。";
%>
<%!
...
/* 	sTemp.append("<tr>");
	...
	sTemp.append("<td class=td1 rowspan=2 align=center colspan=5 bgcolor=#aaaaaa ><font style=' font-size: 12pt;FONT-FAMILY:宋体;FONT-WEIGHT: bold;color:black;background-color:#aaaaaa' >借钱么车主贷申请表</font></td> ");	
	...
	sTemp.append("<td colspan=3 align=left class=td1 >合同编号："+sObjectNo+"<br/>申请日期："+sInputDate+"&nbsp;</td>");
	sTemp.append("</tr>");
...
/* 	sTemp.append("<tr style='height:30px;'>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=3 align=left class=td1 >27.贷款本金（元）："+sBusinessSum+"&nbsp;</td>");
	...
	//sTemp.append("<td colspan=3 align=left class=td1 style='color: red;' >30.是否购买保险："+sCreditCycleName+"&nbsp;</td>");
	...
	sTemp.append("<td colspan=3 align=left class=td1 >31.每月还款日："+sDefaultDueDay+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >32.月贷款利率（%）："+sCreditRate+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >33.首次还款额（元）："+dFirstIntrestAmt+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >34.首次还款日："+sFirstDueDate+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=3 align=left class=td1 >35.月客户服务费率（%）："+sCUSTOMERSERVICERATES+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >36.月财务管理费率（%）："+sMANAGEMENTFEESRATE+"&nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >37.月增值服务费率</br>（%）："+sCreditFeeRate+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >38.印花税（%）："+sSTAMPTAX+"&nbsp;</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=3 align=left class=td1 >41.指定还款账户账号：<br>&nbsp;&nbsp;&nbsp;&nbsp;"+sRepaymentNo+"&nbsp;<br></td>");
	sTemp.append("<td colspan=4 align=left class=td1 >42.开户银行：<br>&nbsp;&nbsp;&nbsp;&nbsp;招商银行深圳安联支行 &nbsp;</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >43.户名："+sRepaymentName+"&nbsp;</td>");
...
/* 	sTemp.append("<tr>");
	...
	sTemp.append("<tr>");
/* 	sTemp.append("<td colspan=9 align=left class=td1 >42. 借款人银行账户信息（用于收款、扣款）：户名："+sReplaceName
									+"&nbsp;&nbsp;&nbsp;&nbsp;开户行："+sOpenBranch
...
*/
	sTemp.append("<td colspan=9 align=left class=td1 >44. 借款人银行账户信息（用于收款、扣款）：<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;户名："+sReplaceName
			+"&nbsp;&nbsp;&nbsp;&nbsp;开户行："+sOpenBranch+"&nbsp;&nbsp;&nbsp;&nbsp;账号："+sReplaceAccount+"</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >45.银行代扣还款："+("1".equals(repaymentWay)?"是■  否□&nbsp;</td>":"是□  否■&nbsp;</td>"));
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=3 align=left class=td1 >46.单位/个体全称："+sWorkCorp+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >47.任职部门："+sEmployRecord+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >48.职位："+sHeadShip+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >49.行业类别："+sIndustryType+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >50.单位性质："+sCellProperty+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >51.单位电话："+sWorkTel+"&nbsp;</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	sTemp.append("<td colspan=3 align=left class=td1 >52. 单位地址：省/直辖市："+sWorkAdd+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >53.镇/乡："+sUnitCountryside+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >54.街道/村："+sUnitStreet+"&nbsp;</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >55.小区/楼盘："+sUnitRoom+"&nbsp;</td>");
	sTemp.append("<td colspan=1 align=left class=td1 >56.栋/单元/房间号："+sUnitNo+"</td>");
	sTemp.append("</tr>");
	
	sTemp.append("<tr>");
	//sTemp.append("<td colspan=3 align=left class=td1 >55. 邮寄地址："+sFlag8+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1  >57.邮寄地址："+("1".equals(sFlag8)?"居住地址■"+"&nbsp;&nbsp;&nbsp;"+"单位地址□":"居住地址□"+"&nbsp;&nbsp;&nbsp;"+"单位地址■")+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >58.总工作经验(年)："+sJobTotal+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >59.现单位工作/个体营业时间(月)："+sJobTime+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >60.在现单位是否购买社保："+sFlag4+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >61.配偶姓名："+sSpouseName+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >62.配偶移动电话："+sSpouseTel+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >63.配偶单位名称：</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >64.配偶单位电话：</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >65.家庭成员名称："+sKinshipName+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >66.家庭成员类型："+sRelativeType+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >67.家庭成员电话："+sKinshipTel+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >68.家庭成员联系地址："+sKinshipAdd+"</td>");
	sTemp.append("</tr>");
	...
	sTemp.append("</tr>");
	sTemp.append("<tr style='height:20px;'>");
	sTemp.append("<td colspan=3 align=left class=td1 >69.联系人姓名："+sOtherContact+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >70.与申请人关系："+sRelativeType1+"</td>");
	sTemp.append("<td colspan=4 align=left class=td1 >71.联系电话："+sContactTel+"</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=10 align=center class=td1 bgcolor=#aaaaaa ><b>收入及支出信息</b>&nbsp;</td>");
	sTemp.append("</tr>");
	sTemp.append("<tr>");
	sTemp.append("<td colspan=2 align=left class=td1 >72.月收入总额（元）："+sZE+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >73.其他收入（元/月）："+sOtherRevenue+"</td>");
	sTemp.append("<td colspan=3 align=left class=td1 >74.家庭月收入（元）："+sFamilyMonthIncome+"</td>");
	sTemp.append("<td colspan=2 align=left class=td1 >75.个人月支出（元/月）："+sCE+"</td>");
	sTemp.append("</tr>");
...
//	sTemp.append("<td colspan=10 align=left class=td1 ><font style='FONT-FAMILY:宋体; font-size: 8pt;' >本人具有完全民事行为能力签署本申请表并承担相应责任。本人已仔细阅读并完全了解《现金分期消费贷款三方协议》，并且自愿遵守相关的合同规定。<br>本人保证在此表上填写、确认的内容及向贷款人、佰仟金融提供的所有相关资料全部真实、有效，如本人提供虚假资料，将承担由此引起的一切责任及损失。本人在此同意贷款人和佰仟金融按照相关法律法规的规定，向有权机构报送本人的个人信用信息，包括但不限于本申请表项下的信息，同时，授权贷款人和佰仟金融向任何可能的来源查询本人及配偶的与贷款申请有关的个人信息和个人信用信息，相关查询结果的用途将用于贷款审查和贷款管理。<br>本人同意如因单方面取消申请或不具备借款条件，申请不获批准，本表格及所有已提交的资料无须退还本人，由佰仟金融处理。贷款人有权拒绝本贷款申请而无须给予任何原因解释。</td>");
	...
	//sTemp.append("<tr>");
//	sTemp.append("<td colspan=10 align=left class=td1 ><font style='FONT-FAMILY:宋体; font-size: 8pt;' >本人具有完全民事行为能力签署本申请表并承担相应责任。本人已仔细阅读并完全了解《现金贷款三方协议》，并且自愿遵守相关的合同规定。<br>本人保证在此表上填写、确认的内容及向贷款人、佰仟金融提供的所有相关资料全部真实、有效，如本人提供虚假资料，将承担由此引起的一切责任及损失。本人在此同意贷款人和佰仟金融按照相关法律法规的规定，向有权机构报送本人的个人信用信息，包括但不限于本申请表项下的信息，同时，授权贷款人和佰仟金融向任何可能的来源查询本人及配偶的与贷款申请有关的个人信息和个人信用信息，相关查询结果的用途将用于贷款审查和贷款管理。<br/>本人同意如因单方面取消申请或不具备借款条件，申请不获批准，本表格及所有已提交的资料无须退还本人，由佰仟金融处理。贷款人有权拒绝本贷款申请而无须给予任何原因解释。</td>");
//	sTemp.append("</tr>");
